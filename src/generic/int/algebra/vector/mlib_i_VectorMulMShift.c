...
#pragma ident	"@(#)mlib_i_VectorMulMShift.c	9.3	07/11/05 SMI"
	...
	mlib_s32 shift)
{
	...
	for (i = 0; i <= (n - 4); i += 4) {
		mlib_s64 dz0 = 0, dz1 = 0, dz2 = 0, dz3 = 0;

#ifdef __SUNPRO_C
#pragma pipeloop(0)
#endif /* __SUNPRO_C */
		for (j = 0; j < m; j++) {
			mlib_s64 dxj = (mlib_s64)x[j];

			dz0 += dxj * (mlib_s64)y[j * ns + i];
			dz1 += dxj * (mlib_s64)y[j * ns + i + 1];
			dz2 += dxj * (mlib_s64)y[j * ns + i + 2];
			dz3 += dxj * (mlib_s64)y[j * ns + i + 3];
		}
...
#pragma pipeloop(0)
	...
	for (; i < n; i++) {
		...
		for (j = 0; j <= (m - 4); j += 4) {
			dz += ((mlib_s64)x[j]) * ((mlib_s64)y[j * ns + i]);
			dz += ((mlib_s64)x[j + 1]) * ((mlib_s64)y[(j + 1) * ns +
				i]);
			dz += ((mlib_s64)x[j + 2]) * ((mlib_s64)y[(j + 2) * ns +
				i]);
			dz += ((mlib_s64)x[j + 3]) * ((mlib_s64)y[(j + 3) * ns +
				i]);
		}
	...
	mlib_s32 shift)
{
	...
	ns2 = ns << 1;
	for (i = 0; i <= (n - 4); i += 4) {
		mlib_s64 dz0_r = 0, dz0_i = 0, dz1_r = 0, dz1_i = 0;
		mlib_s64 dz2_r = 0, dz2_i = 0, dz3_r = 0, dz3_i = 0;
...
#endif /* __SUNPRO_C */
		for (j = 0; j < m; j++) {
			mlib_s64 dxj_r = (mlib_s64)x[j * 2];
			mlib_s64 dxj_i = (mlib_s64)x[j * 2 + 1];

			mlib_s64 dy0_r = (mlib_s64)addr[0];
			mlib_s64 dy0_i = (mlib_s64)addr[1];
			mlib_s64 dy1_r = (mlib_s64)addr[2];
			mlib_s64 dy1_i = (mlib_s64)addr[3];

			mlib_s64 dy2_r = (mlib_s64)addr[4];
			mlib_s64 dy2_i = (mlib_s64)addr[5];
			mlib_s64 dy3_r = (mlib_s64)addr[6];
			mlib_s64 dy3_i = (mlib_s64)addr[7];
...
#pragma pipeloop(0)
		...
		for (j = 0; j < m; j++) {
			...
			dz_r += ((mlib_s64)x[j * 2]) * ((mlib_s64)y[a]) -
				((mlib_s64)x[j * 2 + 1]) * ((mlib_s64)y[a + 1]);
			dz_i += ((mlib_s64)x[j * 2]) * ((mlib_s64)y[a + 1]) +
				((mlib_s64)x[j * 2 + 1]) * ((mlib_s64)y[a]);
		}
