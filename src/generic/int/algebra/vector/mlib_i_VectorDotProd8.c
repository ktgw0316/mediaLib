...
#pragma ident	"@(#)mlib_i_VectorDotProd8.c	9.3	07/11/05 SMI"
...
#pragma pipeloop(0)
		...
		for (i = 0; i < n >> 2; i++) {
			...
			dot += lx2 * (LONGTYPE)ly2;
			dot += lx3 * (LONGTYPE)ly3;
		}
...
#pragma pipeloop(0)
		...
		for (i = 0; i < n >> 2; i++) {
			...
			dot += px[2] * (LONGTYPE)ly2;
			dot += px[3] * (LONGTYPE)ly3;
			px += 4;
		}
...
#pragma pipeloop(0)
			...
			for (i = 0; i < n >> 1; i++) {
				...
				dot_i += lx1 * (LONGTYPE)ly0 -
					lx0 * (LONGTYPE)ly1;
				dot_r += lx2 * (LONGTYPE)ly2 +
					lx3 * (LONGTYPE)ly3;
				dot_i += lx3 * (LONGTYPE)ly2 -
					lx2 * (LONGTYPE)ly3;
			}
