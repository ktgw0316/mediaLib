...
#pragma ident	"@(#)mlib_i_ImageBlend_ONE_SA.c	9.2	07/11/05 SMI"
...
#pragma pipeloop(0)
		...
		for (i = 0; i < dst_width; i++) {
			...
			SAT_U8_U8C(pdst[4 * i], ((psrc2[4 * i] \
				* alpha) >> 8) + psrc1[4 * i]);
			SAT_U8_U8C(pdst[4 * i + 1], ((psrc2[4 * i \
				+ 1] * alpha) >> 8) + psrc1[4 * i + 1]);
			SAT_U8_U8C(pdst[4 * i + 2], ((psrc2[4 * i \
				+ 2] * alpha) >> 8) + psrc1[4 * i + 2]);
			SAT_U8_U8C(pdst[4 * i + 3], ((psrc2[4 * i \
				+ 3] * alpha) >> 8) + psrc1[4 * i + 3]);
		}
