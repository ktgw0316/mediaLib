 ...
 */
#pragma ident	"@(#)mlib_i_ImageBlend_SA_SA.c	9.2	07/11/05 SMI"
...
#pragma pipeloop(0)
		...
		for (i = 0; i < dst_width; i++) {
			...
			sum0 = ((psrc1[4 * i + 0] + psrc2[4 * i +
			    0]) * alpha) >> 8;
			SAT_U8_U8C(pdst[4 * i + 0], sum0);

			sum1 = ((psrc1[4 * i + 1] + psrc2[4 * i +
			    1]) * alpha) >> 8;

			SAT_U8_U8C(pdst[4 * i + 1], sum1);

			sum2 = ((psrc1[4 * i + 2] + psrc2[4 * i +
			    2]) * alpha) >> 8;

			SAT_U8_U8C(pdst[4 * i + 2], sum2);

			sum3 = ((psrc1[4 * i + 3] + psrc2[4 * i +
			    3]) * alpha) >> 8;

			SAT_U8_U8C(pdst[4 * i + 3], sum3);
