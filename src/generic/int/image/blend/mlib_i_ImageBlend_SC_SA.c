...
#pragma ident	"@(#)mlib_i_ImageBlend_SC_SA.c	9.3	07/11/05 SMI"
 ...
 *      if destination ALPHA value is 1.
 *
 *
 * DESCRIPTION
...
#pragma pipeloop(0)
		...
		for (i = 0; i < dst_width; i++) {
			...
			SAT_U8_U8C(pdst[4 * i + 0], (((psrc1[4 * i + 0]\
				+ alpha) * psrc2[4 * i + 0]) >> 8));

			SAT_U8_U8C(pdst[4 * i + 1], (((psrc1[4 * i + 1]\
				+ alpha) * psrc2[4 * i + 1]) >> 8));

			SAT_U8_U8C(pdst[4 * i + 2], (((psrc1[4 * i + 2]\
				+ alpha) * psrc2[4 * i + 2]) >> 8));

			SAT_U8_U8C(pdst[4 * i + 3], (((psrc1[4 * i + 3]\
				+ alpha) * psrc2[4 * i + 3]) >> 8));
		}
