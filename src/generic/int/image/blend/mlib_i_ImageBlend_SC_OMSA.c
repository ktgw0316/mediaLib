...
#pragma ident	"@(#)mlib_i_ImageBlend_SC_OMSA.c	9.2	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_ImageBlend_SC_OMSA - blend two images with
 *      OMSA source blend factor and SC destination blend factor
 *
 * SYNOPSIS
 *
 *      mlib_status mlib_ImageBlend_SC_OMSA(mlib_image       *dst,
...
#pragma weak mlib_ImageBlend_SC_OMSA = __mlib_ImageBlend_SC_OMSA

#elif defined(__GNUC__)	/* defined(__SUNPRO_C) */
__typeof__(__mlib_ImageBlend_SC_OMSA) mlib_ImageBlend_SC_OMSA
    __attribute__((weak, alias("__mlib_ImageBlend_SC_OMSA")));
...
#pragma pipeloop(0)
		...
		for (i = 0; i < dst_width; i++) {
			...
			alpha = 256 - psrc2[4 * i + off];

			SAT_U8_U8C(pdst[4 * i + 0],
			    (((psrc1[4 * i + 0] + alpha) * psrc2[4 * i +
			    0]) >> 8));

			SAT_U8_U8C(pdst[4 * i + 1],
			    (((psrc1[4 * i + 1] + alpha) * psrc2[4 * i +
			    1]) >> 8));

			SAT_U8_U8C(pdst[4 * i + 2],
			    (((psrc1[4 * i + 2] + alpha) * psrc2[4 * i +
			    2]) >> 8));

			SAT_U8_U8C(pdst[4 * i + 3],
			    (((psrc1[4 * i + 3] + alpha) * psrc2[4 * i +
			    3]) >> 8));
		}
