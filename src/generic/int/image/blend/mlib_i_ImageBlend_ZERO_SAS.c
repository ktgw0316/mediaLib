...
#pragma ident	"@(#)mlib_i_ImageBlend_ZERO_SAS.c	9.2	07/11/05 SMI"
 ...
 *
 * DESCRIPTION
 *      dst = src2 * (f, f, f, 1), f = min(1 - ALPHAsrc1, ALPHAsrc2)
 */
...
#pragma pipeloop(0)
			...
			for (i = 0; i < dst_width; i++) {
				...
				pdst[4 * i + 0] = ((min) * psrc2[4 * i +
				    0]) >> 8;
				pdst[4 * i + 1] = ((min) * psrc2[4 * i +
				    1]) >> 8;
				pdst[4 * i + 2] = ((min) * psrc2[4 * i +
				    2]) >> 8;
				pdst[4 * i + 3] = psrc2[4 * i + 3];
			}
...
#pragma pipeloop(0)
			...
			for (i = 0; i < dst_width; i++) {
				...
				pdst[4 * i + 0] = psrc2[4 * i + 0];
				pdst[4 * i + 1] = ((min) * psrc2[4 * i +
				    1]) >> 8;
				pdst[4 * i + 2] = ((min) * psrc2[4 * i +
				    2]) >> 8;
				pdst[4 * i + 3] = ((min) * psrc2[4 * i +
				    3]) >> 8;
			}
