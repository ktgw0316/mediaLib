...
#pragma ident	"@(#)mlib_i_ImageBlend_DA_SAS.c	9.2	07/11/05 SMI"
...
#pragma pipeloop(0)
			...
			for (i = 0; i < dst_width; i++) {
				...
				alpha1 = dst_alpha;
				min = MIN(src_alpha, 256 - dst_alpha);

				src10 = psrc1[4 * i + 0];
				src20 = psrc2[4 * i + 0];
				SAT_U8_U8C(pdst[4 * i + 0],
				(src10 * alpha1 + src20 * min) >> 8);

				src11 = psrc1[4 * i + 1];
				src21 = psrc2[4 * i + 1];
				SAT_U8_U8C(pdst[4 * i + 1],
				(src11 * alpha1 + src21 * min) >> 8);

				src12 = psrc1[4 * i + 2];
				src22 = psrc2[4 * i + 2];
				SAT_U8_U8C(pdst[4 * i + 2],
				(src12 * alpha1 + src22 * min) >> 8);

				src13 = psrc1[4 * i + 3];
				src23 = psrc2[4 * i + 3];
				SAT_U8_U8C(pdst[4 * i + 3],
				((src13 * alpha1) >> 8) + src23);
			}
