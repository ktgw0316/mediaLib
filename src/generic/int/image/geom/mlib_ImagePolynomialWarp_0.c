...
#pragma ident	"@(#)mlib_ImagePolynomialWarp_0.c	9.3	07/11/05 SMI"
...
mlib_ImagePolynomialWarp_0(
    ...
    mlib_edge edge)
{
	mlib_d64 wx_f, wy_f, Xstart, Ystart, Xstop, Ystop;
	mlib_s64 wx, wy;
	...
	wx_f = xCoeffs[0] * postScaleX - postShiftX;
	wx_f -= (filter != MLIB_NEAREST) ? 0.5 : 0.0;
	DTOLL(wx, wx_f);

	wy_f = yCoeffs[0] * postScaleY - postShiftY;
	wy_f -= (filter != MLIB_NEAREST) ? 0.5 : 0.0;
	DTOLL(wy, wy_f);
	...
	if (!(wx_f >= Xstart && wx_f < Xstop &&
		wy_f >= Ystart && wy_f < Ystop))
		return (MLIB_SUCCESS);
	...
	case MLIB_BYTE:
	    {
		    ...
		    srcData = srcData + (mlib_s32)(wy>>SHIFT) *srcYStride;
		    srcData += (mlib_s32)(wx>>SHIFT) *srcChannels;
...
mlib_ImagePolynomialWarp_0_BL(
    ...
    mlib_s64 wy)
{
	mlib_s64 a0, a1, a2, a3, r0, r1, dx, dy;
	...
	case MLIB_BYTE:
	    {
		    ...
		    for (i = 0; i < srcChannels; i++) {
			    ...
			    r0 = (a0<<SHIFT) + (a1 - a0) * dx;
			    r1 = (a3<<SHIFT) + (a2 - a3) * dx;
			    color[i] = (mlib_s32)(((r0<<SHIFT) +
				(r1 - r0) * dy)>>(SHIFT+SHIFT));
		    }
	    }
