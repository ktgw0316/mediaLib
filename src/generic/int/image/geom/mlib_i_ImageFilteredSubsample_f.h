...
#ifndef _MLIB_I_IMAGEFILTEREDSUBSAMPLE_F_H
#define	_MLIB_I_IMAGEFILTEREDSUBSAMPLE_F_H

#pragma ident	"@(#)mlib_i_ImageFilteredSubsample_f.h	9.2	07/11/05 SMI"
...
#pragma pipeloop(0)
	...
	for (i = 0; i < xsize; i++) {
		...
		ss = (FTYPE) (((mlib_s64)SAT_OFF<<SHIFT) +
			k0 * (buff0[i] + buff1[i]));
...
#pragma pipeloop(0)
	...
	for (i = 0; i < xsize; i++) {
		...
		ss = (val0<<SHIFT) + k0 * buff1[i] + k1 * (buff0[i] + buff2[i]);
		STORE_SAT(dp[i * nchan], buffo[i], ss);
	}
}
...
#pragma pipeloop(0)
	...
	for (i = 0; i < xsize; i++) {
		...
		ss = val0 + k0 * (buff1[i] + buff2[i]) + k1 * (buff0[i] +
		    buff3[i]);
		*/
		ss = (val0<<SHIFT) + k0 * (buff1[i] + buff2[i]) +
			k1 * (buff0[i] + buff3[i]);
		STORE_SAT(dp[i * nchan], buffo[i], ss);
	}
}
...
#pragma pipeloop(0)
	...
	for (i = 0; i < xsize; i++) {
		...
		ss = val0 + k0 * buff2[i] + k1 * (buff1[i] + buff3[i]) +
		    k2 * (buff0[i] + buff4[i]);
		*/
		ss = (val0<<SHIFT) + k0 * buff2[i] +
			k1 * (buff1[i] + buff3[i]) + k2 * (buff0[i] + buff4[i]);
		STORE_SAT(dp[i * nchan], buffo[i], ss);
	}
}
...
/* 0.5*scaleY - 0.5*(kh-1) */
	...
	*/
	for (l = 0; l < hSize; l++)
		DTOLL(hKern[l], hKernel[l]);
	for (l = 0; l < vSize; l++) {
		mlib_d64 temp1;
		temp1 = vKernel[l] * DSCALE;
		DTOLL(vKern[l], temp1);
	}
...
#endif /* _MLIB_I_IMAGEFILTEREDSUBSAMPLE_F_H */
