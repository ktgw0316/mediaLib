...
#pragma ident	"@(#)mlib_i_ImageZoomBlend_BL.c	9.2	07/11/05"

/*
 * Internal functions for mlib_ImageZoomBlend()
 *
 */
...
#define	BLEND2(dst, fres)	dst =        \
		(DTYPE)(((a0*fres + a1*((mlib_s64)dst<<32))/w)>>32)
    ...
    mlib_work_image * param)
{
	...
	width2 = (width + 1) / 2;
	size =
	    7 * width2 * sizeof (mlib_s64) + 3 * width * sizeof (TTYPE) +
...
#undef	RES
#define	RES(N)	res##N = (RTYPE)((r0##N << 16) + fdx * (r1##N - r0##N))
#define	RSLT(x)	RES(x)
...
#define	BLEND34(dst, fres)                                     \
	dst += (DTYPE)(((fres - ((mlib_s64)dst<<32))*w0/w)>>32)

#define	BLEND34z(dst, fres)                                     \
	dst = (DTYPE)(((w0*fres + w1*((mlib_s64)dst<<32))/w)>>32)

/* *********************************************************** */

#define	BLEND44(dst, fres)                                      \
	dst =                                                   \
	    mlib_sat_u8[dst + (mlib_s32)(((MUL255(dalpha) * (fres>>8)) -       \
	    w0 * dst) / w)]

#define	BLEND44z(dst, fres)                                     \
	dst =                                                   \
	    mlib_sat_u8[(mlib_s32)(((MUL255(dalpha) * (fres>>8)) +  \
	    w1 * dst) / w)]
    ...
    mlib_s32 alp)
{
	...
	FP_TYPE a0 = (param->alpha), a1, dalpha = a0;
	BUFF_TYPE *buff;
...
#endif /* __SUNPRO_C */
			for (i = 0; i < width; i++) {
				dp[4 * i] = (DTYPE) (buff[3 * i] >> 32);
				dp[4 * i + 1] = (DTYPE) (buff[3 * i + 1] >> 32);
				dp[4 * i + 2] = (DTYPE) (buff[3 * i + 2] >> 32);
				dp[4 * i + alp] = 255;
			}
    ...
    mlib_s32 alp)
{
	...
	FP_TYPE fdx, fdy, ady, rdy, fdy0, fdy1, fdy2, fdy3;
	mlib_blend blend = param->blend;
	FP_TYPE dalpha = param->alpha, dalpha1 = dalpha;
	FP_TYPE w0, w1, w;
	BUFF_TYPE *buff;
...
#pragma pipeloop(0)
			...
			for (i = 0; i < width; i++) {
				...
				if (w < ((mlib_s64)1<<32)) {
					...
					dp[4 * i] = (DTYPE)
						(buff[4 * i] / w);
					dp[4 * i + 1] = (DTYPE) (buff[4 * i +
						1] / w);
					dp[4 * i + 2] = (DTYPE) (buff[4 * i +
						2] / w);
					dp[4 * i + alp] = (DTYPE)
						(buff[4 * i + 3] >> 32);
				}
...
#pragma pipeloop(0)
			...
			for (i = 0; i < width; i++) {
				...
				if (w < ((mlib_s64)(255*255)<<24)) {
					dp[4 * i] = mlib_sat_u8[dp[4 * i]];
					dp[4 * i + 1] =
						mlib_sat_u8[dp[4 * i + 1]];
					dp[4 * i + 2] =
						mlib_sat_u8[dp[4 * i + 2]];
					dp[4 * i + alp] = 0;
				} else {
...
#pragma pipeloop(0)
			...
			for (i = 0; i < width; i++) {
				...
				if (w < ((mlib_s64)(255*255)<<24)) {
					dp[4 * i] = mlib_sat_u8[0];
					dp[4 * i + 1] = mlib_sat_u8[0];
					dp[4 * i + 2] = mlib_sat_u8[0];
					dp[4 * i + alp] = 0;
				} else {
