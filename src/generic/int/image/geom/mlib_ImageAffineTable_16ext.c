...
#pragma ident	"@(#)mlib_ImageAffineTable_16ext.c	9.3	07/11/05 SMI"
...
#define	LINE_FUNC(KERN)                                          \
	void                                                     \
	mlib_ImageAffineTableLine_16ext_##KERN(FP_TYPE * buff,   \
...
#define	LINE_FUNC(KERN)                                             \
	void                                                        \
	mlib_ImageAffineTableLine_16nw_##KERN(FP_TYPE * buff,       \
	    FILTER_TYPE * filterX, FILTER_TYPE * filterY,           \
    ...
    PARAMS_EXT)
{
	...
	if (type < 4) {
		...
		max_xsize *= b_step;
		sat_off = (mlib_s64)1<<(SHIFT+SHIFT-1);
#endif /* IMG_TYPE == 4 */
