...
#ifndef _MLIB_I_IMAGEZOOMTRANSLATETABLE_F_H
#define	_MLIB_I_IMAGEZOOMTRANSLATETABLE_F_H

#pragma ident	"@(#)mlib_i_ImageZoomTranslateTable_f.h	9.2	07/11/05 SMI"
...
#if !defined(_NO_LONGLONG) && IMG_TYPE < 3
...
#define	SAT_OFF	(1<<AMP_FD>>1)	/* 0.5 */
#define	SRC2FP(x)	(x)	/* U82FP(x) */
#define	NBITS	0
    ...
    mlib_zoom_workspace *ws)
{
	...
	yf0 = FD2LL(*(INT_TYPE *)&fp[0]);
	yf1 = FD2LL(*(INT_TYPE *)&fp[1]);
	yf2 = FD2LL(*(INT_TYPE *)&fp[2]);
	yf3 = FD2LL(*(INT_TYPE *)&fp[3]);
...
#pragma pipeloop(0)
	...
	for (i = 0; i < size2; i++) {
		...
		yy.s32s.s0 += (yd0.s32s.s0 * yf0 + yd1.s32s.s0 * yf1 +
		    yd2.s32s.s0 * yf2 + yd3.s32s.s0 * yf3) >> AMP_FD;
		yy.s32s.s1 += (yd0.s32s.s1 * yf0 + yd1.s32s.s1 * yf1 +
		    yd2.s32s.s1 * yf2 + yd3.s32s.s1 * yf3) >> AMP_FD;
...
#pragma pipeloop(0)
	...
	for (i = 0; i < size; i++) {
		...
		c0 = ((*(INT_TYPE *)&buff0[i] >> NBITS) * yf0 +
			(*(INT_TYPE *)&buff1[i] >> NBITS) * yf1 +
			(*(INT_TYPE *)&buff2[i] >> NBITS) * yf2 +
			(*(INT_TYPE *)&buff3[i] >> NBITS) * yf3)
			>> (AMP_FD - NBITS);
		*(INT_TYPE *)&ybuff[i] += c0;
	}
...
#pragma pipeloop(0)
	...
	for (i = 0; i < (size >> 1); i++) {
		...
		yy.s32s.s0 = (yy.s32s.s0) >> AMP_FD;
		SAT(dPtr[0], ival, yy.s32s.s0);
		dPtr += chan;
		yy.s32s.s0 = SAT_OFF;

		yy.s32s.s1 = (yy.s32s.s1) >> AMP_FD;
		SAT(dPtr[0], ival, yy.s32s.s1);
		dPtr += chan;
		yy.s32s.s1 = SAT_OFF;
...
#pragma pipeloop(0)
	...
	for (i = 0; i < size; i++) {
		...
		*(INT_TYPE *)&ybuff[i] += c0;
		val = (*(INT_TYPE *)&ybuff[i]) >> AMP_FD;
		SAT(dPtr[0], ival, val);
		dPtr += chan;
		*(INT_TYPE *)&ybuff[i] = SAT_OFF;
	}
...
#endif /* __SUNPRO_C */
	for (i = 0; i < size2; i++) {
		s64_2_s32 yd0, yd1, yy;

		yd0.s64 = ((mlib_s64 *)buff0)[i];
		yd1.s64 = ((mlib_s64 *)buff1)[i];
		yy.s64 = ((mlib_s64 *)ybuff)[i];

		yy.s32s.s0 += (yd0.s32s.s0 * yf0 + yd1.s32s.s0 * yf1) >> AMP_FD;
		yy.s32s.s1 += (yd0.s32s.s1 * yf0 + yd1.s32s.s1 * yf1) >> AMP_FD;
...
#pragma pipeloop(0)
	...
	for (i = 0; i < size; i++) {
		...
		xd0.s64 = ((mlib_s64 *)fptr)[0];
		xd1.s64 = ((mlib_s64 *)fptr)[1];
		CALC_SRC_PTRX();

		itmp = GET_SRC(0) * FD2LL(xd0.s32s.s0)
			+ GET_SRC(1) * FD2LL(xd0.s32s.s1)
			+ GET_SRC(2) * FD2LL(xd1.s32s.s0)
			+ GET_SRC(3) * FD2LL(xd1.s32s.s1);
		*(INT_TYPE *)(&pbuff[i]) = (itmp);
...
#pragma pipeloop(0)
	...
	for (i = 0; i < size; i++) {
		MLIB_FILTER_X(x, filterX, 0);
		xf0 = FD2LL(*(INT_TYPE *)&fptr[0]);
		xf1 = FD2LL(*(INT_TYPE *)&fptr[1]);
		xf2 = FD2LL(*(INT_TYPE *)&fptr[2]);
		xf3 = FD2LL(*(INT_TYPE *)&fptr[3]);
		CALC_SRC_PTRX();

		itmp = GET_SRC(0) * xf0 + GET_SRC(1) * xf1
			+ GET_SRC(2) *xf2 + GET_SRC(3) * xf3;
		*(INT_TYPE *)(&pbuff[i]) = (itmp);
...
#pragma pipeloop(0)
	...
	for (i = 0; i < size; i++) {
		...
		*(INT_TYPE *)(&pbuff[i]) = GET_SRC(0) * xf0;
...
#pragma pipeloop(0)
	...
	for (i = 0; i < size; i++) {
		...
		itmp = (*(INT_TYPE *)&pbuff[i]);
		itmp += GET_SRC(0) * xf0 + GET_SRC(1) * xf1 + GET_SRC(2) *xf2;
		*(INT_TYPE *)(&pbuff[i]) = (itmp);
...
#pragma pipeloop(0)
	...
	for (i = 0; i < size; i++) {
		...
		itmp = (*(INT_TYPE *)&ybuff[i]);
		itmp += (c0 * yf0) >> (AMP_FD - NBITS);
		*(INT_TYPE *)(&ybuff[i]) = (itmp);
...
{
	DECL_LINE;
	mlib_s64 c0, c1, xf0, xf1, xf2, xf3;
	mlib_s64 yf0, yf1;
	mlib_s64 itmp;
...
#pragma pipeloop(0)
	...
	for (i = 0; i < size; i++) {
		...
		itmp = (*(INT_TYPE *)&ybuff[i]);
		itmp += ((c0 * yf0) + (c1 * yf1)) >> (AMP_FD - NBITS);
		*(INT_TYPE *)(&ybuff[i]) = (itmp);
...
#pragma pipeloop(0)
	...
	for (i = 0; i < size; i++) {
		...
		itmp = (*(INT_TYPE *)&ybuff[i]);
		itmp += (c0 * yf0 + c1 * yf1 + c2 * yf2) >> (AMP_FD - NBITS);
		*(INT_TYPE *)(&ybuff[i]) = (itmp);
...
#pragma pipeloop(0)
	...
	for (i = 0; i < size; i++) {
		...
		itmp = (*(INT_TYPE *)&ybuff[i]);
		itmp += (c0 * yf0 + c1 * yf1 + c2 * yf2 + c3 * yf3)
			>> (AMP_FD - NBITS);
		*(INT_TYPE *)(&ybuff[i]) = (itmp);
...
#endif /* _MLIB_I_IMAGEZOOMTRANSLATETABLE_F_H */
