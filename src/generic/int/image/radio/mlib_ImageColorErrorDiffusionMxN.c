...
#pragma ident	"@(#)mlib_ImageColorErrorDiffusionMxN.c	9.3	07/11/05 SMI"
...
#define	FUNC_M_ARG                                                        \
	mlib_s64 *buffd, mlib_s64 *perror, mlib_s32 *pkern, mlib_s32 sw

/* *********************************************************** */

#define	FUNC_DM_ARG                                                     \
	void *dl, mlib_s64 *buffd, mlib_s64 *perror, mlib_s32 *pkern,   \
	mlib_s64 dscale, const void *colormap, mlib_s64 *lut_table,     \
	mlib_s32 sw, mlib_s32 dm
...
#define	FUNC_DM(SUFF, STYPE, DTYPE)                                      \
	...
	FUNC(ch3_##STYPE##_##DTYPE##_t##SUFF##_dm0) (FUNC_DM_ARG)        \
	{                                                                \
	    DEF_VARS(STYPE, DTYPE, s64);                                 \
	    DEF_EXTRA_VARS(STYPE, DTYPE, s64)                            \
	    for (i = 0; i < sw; i++) {                                   \
		a = buffd[0];                                            \
		b = buffd[1];                                            \
		c = buffd[2];                                            \
 ...
 */                                                                      \
	...
	{                                                                \
	    DEF_VARS(STYPE, DTYPE, s64);                                 \
	    DEF_EXTRA_VARS(STYPE, DTYPE, s64)                            \
	    DEF_EXTRA_VARS4(STYPE, DTYPE, s64)                           \
	    for (i = 0; i < sw; i++) {                                   \
		a = buffd[0];                                            \
		b = buffd[1];                                            \
		c = buffd[2];                                            \
...
#define	GET_3x3_SRC3(a, b, c)                                   \
	a = (mlib_s64)(sl[0] * dscale);                                    \
	b = (mlib_s64)(sl[1] * dscale);                                    \
	c = (mlib_s64)(sl[2] * dscale)
...
#define	LUT_IND3_s16(ind)	4*ind + 1
#define	LUT_IND3_d64(ind)	3*ind
#define	LUT_IND3_s64(ind)	3*ind

/* *********************************************************** */
#define	GET_COLOR_ERROR3(LUT_TYPE)                                  \
	col_ind = LUT_IND3_##LUT_TYPE(col_ind);                     \
	ae = col0 - (mlib_s32)((mlib_##LUT_TYPE *) lut_table)[col_ind];       \
	be = col1 - (mlib_s32)((mlib_##LUT_TYPE *) lut_table)[col_ind + 1];   \
	ce = col2 - (mlib_s32)((mlib_##LUT_TYPE *) lut_table)[col_ind + 2]
...
#define	GET_COLOR_ERROR4(LUT_TYPE)                                  \
	...
	be = col1 - (mlib_s32)((mlib_##LUT_TYPE *) lut_table)[col_ind + 1];   \
	ce = col2 - (mlib_s32)((mlib_##LUT_TYPE *) lut_table)[col_ind + 2];   \
	de = col3 - (mlib_s32)((mlib_##LUT_TYPE *) lut_table)[col_ind + 3]
...
/* malloc buffers */
	    bsize = (sw + m) * NCHAN;
	dn1 = (dn) ? dn : 1;
	pbuff = __mlib_malloc((dn1 + 1) * bsize * sizeof (mlib_s64) + 32);
...
/* special case */
	if (flag3x3) {
		if (kern[0] == 1.0 && kern[1] == 5.0 &&
		    kern[2] == 3.0 && kern[3] == 7.0 && scale == 16) {
/* FLoyd-Steinberg kernel */
			flag3x3fs = 1;
			func_ind2 = KH_MAX + 2;
		} else {
	...
	if (flag3x3fs) {
		...
		lut_table =
		    (mlib_s64 *)mlib_ImageGetLutDoubleData(colormap) -
		    nchan * offset;
	}
	...
	for (row = 0; row < sh; row++) {
		...
		func_dm(dl, buffd, buff[dn] + dm * NCHAN, pkern,
				dscale, colormap, lut_table, sw, dm);
