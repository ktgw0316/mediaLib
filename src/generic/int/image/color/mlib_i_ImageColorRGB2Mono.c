...
#pragma ident	"@(#)mlib_i_ImageColorRGB2Mono.c	9.3	07/11/05 SMI"
...
#define	ANALYZE_Q_E_D64(src, shift, rsrc)	\
	{					\
	mlib_s64 tmp;				\
	mlib_s64 sb = src >> 63;		\
	shift = ((src >> 52) & 0x7ff) - 1023;	\
	tmp = (src & 0xfffffffffffff) |		\
	0x10000000000000;			\
	rsrc = (tmp ^ sb) - sb;			\
	}
    ...
    const mlib_d64 *weight)
{
	mlib_u8 *sa, *sl;
	mlib_u8 *da, *dl;
	mlib_s64 sf0, sf1, sf2, df1, w[3];
	mlib_s64 w0, w1, w2;
	mlib_s64 sf3, sf4, sf5, df2;
	mlib_s64 sf6, sf7, sf8, df3;
	mlib_s64 sat;
	mlib_s32 i, j, isum, shift;
	mlib_s32 tmp = 0x7FFFFFFF;
	mlib_s32 *outbuf, *pout;
	mlib_s32 num_blocks, leftover_cols;

	ANALYZE_Q_E_D64(*((mlib_s64 *)(&weight[0])), shift, w0);
	w[0] = (shift < -24)? 0 : w0 >> (8 - shift);
	ANALYZE_Q_E_D64(*((mlib_s64 *)(&weight[1])), shift, w1);
	w[1] = (shift < -24)? 0 : w1 >> (8 - shift);
	ANALYZE_Q_E_D64(*((mlib_s64 *)(&weight[2])), shift, w2);
	w[2] = (shift < -24)? 0 : w2 >> (8 - shift);
	...
	for (j = 0; j < ysize; j++) {
		for (i = 0; i < num_blocks; i++) {
			...
			df1 = (w[0] * sf0 + w[1] * sf1 + w[2] * sf2) >> 20;
			df2 = (w[0] * sf3 + w[1] * sf4 + w[2] * sf5) >> 20;
			df3 = (w[0] * sf6 + w[1] * sf7 + w[2] * sf8) >> 20;
    ...
    const mlib_d64 *weight)
{
	...
	for (j = 0; j < ysize; j++) {
		for (i = 0; i < num_blocks; i++) {
			...
			df1 = (w[0] * sf0 + w[1] * sf1 + w[2] * sf2) >> 30;
			df2 = (w[0] * sf3 + w[1] * sf4 + w[2] * sf5) >> 30;
			df3 = (w[0] * sf6 + w[1] * sf7 + w[2] * sf8) >> 30;
    ...
    const mlib_d64 *weight)
{
	...
	ANALYZE_Q_E_D64(*((mlib_s64 *)(&weight[0])), shift, w0);
	w[0] = (shift < -31)? 0 : w0 >> (21 - shift);
	ANALYZE_Q_E_D64(*((mlib_s64 *)(&weight[1])), shift, w1);
	w[1] = (shift < -31)? 0 : w1 >> (21 - shift);
	ANALYZE_Q_E_D64(*((mlib_s64 *)(&weight[2])), shift, w2);
	w[2] = (shift < -31)? 0 : w2 >> (21 - shift);
