...
#pragma ident   "@(#)mlib_ImageConv_u16ext.c	9.2    07/11/05 SMI"
...
CONV_FUNC(5x5)
{
	...
	if (swid > BUFF_LINE) {
		pbuff = __mlib_malloc((KSIZE + 3) * sizeof (ITYPE) * swid);
...
#pragma pipeloop(0)
			...
			for (i = 0; i <= (wid - 2); i += 2) {
				...
				buff5[i + dx_l] = buffi[i];
				buff5[i + dx_l + 1] = buffi[i + 1];
    ...
    mlib_s32 cmask)
{
	...
	if (bsize > BUFF_SIZE) {
		pbuff = __mlib_malloc(sizeof (ITYPE) * bsize);
...
mlib_status CONV_FUNC_MxN
{
	...
	if (m == 1)
		return (mlib_ImageConv1xN_ext
		    (dst, src, k, fscale, n, dy_t, dy_b, cmask));
