...
#pragma ident   "@(#)mlib_ImageConvIndex_16_16nw.c	9.2    07/11/05 SMI"
 ...
 * Files: mlib_ImageConvIndex_16_16nw.c,   mlib_ImageConvIndex_8_16nw.c
 *        mlib_c_ImageConvIndex_16_8nw.c,  mlib_c_ImageConvIndex_8_8nw.c
 */
...
#define	CONV_FUNC(KERN)                                         \
	mlib_conv##KERN##Index_16_16nw(const mlib_image *src,     \
...
#define	GET_ColorTrueFunc()                                     \
	(nchan ==                                               \
	    3) ? mlib_ImageColorTrue2IndexLine_S16_S16_3 :        \
...
#pragma pipeloop(0)
			...
			for (i = 0; i <= (wid - 2); i += 2) {
				...
				sd2.f32s.f0 = (FTYPE) sd.i32s.i0;
				sd2.f32s.f1 = (FTYPE) sd.i32s.i1;
				*(mlib_d64 *)(buff2 + i) = sd2.d64;
...
#endif /* __SUNPRO_C */
		for (i = 0; i < wid + (KSIZE - 1); i++) {
			buff0[i] = (FTYPE) LTBL(sl[i]);
			buff1[i] = (FTYPE) LTBL(sl1[i]);
			buff2[i] = (FTYPE) LTBL(sl2[i]);
		}
	}
...
#pragma pipeloop(0)
			...
			for (i = 0; i <= (wid - 2); i += 2) {
				...
				sd.d64 = *(mlib_d64 *)(buffi + i);
				buff3[i] = (FTYPE) sd.i32s.i0;
				buff3[i + 1] = (FTYPE) sd.i32s.i1;
...
/* last pixels */
			...
			buff4[wid] = (FTYPE) LTBL(sp[0]);
			buff4[wid + 1] = (FTYPE) LTBL(sp[1]);
			buff4[wid + 2] = (FTYPE) LTBL(sp[2]);
		}
...
/* last pixels */
			...
			buff5[wid] = (FTYPE) LTBL(sp[0]);
			buff5[wid + 1] = (FTYPE) LTBL(sp[1]);
			buff5[wid + 2] = (FTYPE) LTBL(sp[2]);
			buff5[wid + 3] = (FTYPE) LTBL(sp[3]);
		}
...
#define	CLAMP_S32(dst, src)                                            \
	dst = (((src) <= MLIB_S32_MIN) ? MLIB_S32_MIN : (((src) >=        \
	    MLIB_S32_MAX) ? MLIB_S32_MAX : (mlib_s32)(src)))
...
#define	LOAD_KERNEL2()                                          \
	mlib_s64 scalef = 1;                                    \
	mlib_s64 k0, k1, k2, k3;                                \
	mlib_s64 p00, p01, p02, p10, p11, p12;                  \
	                                                        \
	while (scalef_expon > 30) {                             \
	    scalef *= (1 << 30);                                \
	    scalef_expon -= 30;                                 \
 ...
 * keep kernel in regs                                          \
 */                                                         \
	k0 = (mlib_s64)kern[0] << DSCALE;                       \
	k1 = (mlib_s64)kern[1] << DSCALE;                       \
	k2 = (mlib_s64)kern[2] << DSCALE;                       \
	k3 = (mlib_s64)kern[3] << DSCALE

/* *********************************************************** */
#define	LOAD_KERNEL3()                                                 \
	mlib_s64 scalef = 1;                                         \
	mlib_s64 k0, k1, k2, k3, k4, k5, k6, k7, k8;                      \
	mlib_s64 p00, p01, p02, p03, p10, p11, p12, p13, p20, p21, p22,   \
	    p23;                                                  \
	while (scalef_expon > 30) {                               \
	    scalef *= (1 << 30);                                  \
	    scalef_expon -= 30;                                   \
 ...
 */                                                          \
	...
	k2 = (mlib_s64)kern[2] << DSCALE;                        \
	k3 = (mlib_s64)kern[3] << DSCALE;                        \
	k4 = (mlib_s64)kern[4] << DSCALE;                        \
	k5 = (mlib_s64)kern[5] << DSCALE;                        \
	k6 = (mlib_s64)kern[6] << DSCALE;                        \
	k7 = (mlib_s64)kern[7] << DSCALE;                        \
	k8 = (mlib_s64)kern[8] << DSCALE
...
#pragma pipeloop(0)
			...
			for (i = 0; i <= (wid - 2); i += 2) {
				...
				mlib_s64 o64;
#endif /* _NO_LONGLONG */
				s64_2x32 sd0, sd1, sd, sd2, dd;

				sd0.s64 = *(mlib_s64 *)(buff0 + i);
				sd1.s64 = *(mlib_s64 *)(buff1 + i);
				p00 = p02;
				p10 = p12;
				p01 = sd0.i32s.i0;
				p11 = sd1.i32s.i0;
				...
				sd.s64 = *(mlib_s64 *)(buffi + i);
				sd2.i32s.i0 = (FTYPE) sd.i32s.i0;
				sd2.i32s.i1 = (FTYPE) sd.i32s.i1;

				*(mlib_s64 *)(buff2 + i) = sd2.s64;

				D2I(dd.i32s.i0,
				    (p00 * k0 + p01 * k1 + p10 * k2 +
					p11 * k3) / scalef);
				D2I(dd.i32s.i1,
				    (p01 * k0 + p02 * k1 + p11 * k2 +
					p12 * k3) / scalef);
				*(mlib_s64 *)(buffo + i) = dd.s64;
...
#pragma pipeloop(0)
	...
	for (j = 0; j < hgt; j++) {
		...
		for (c = 0; c < nchan; c++) {
			...
			s0 = p02 * k0 + p03 * k1 + p12 * k3 +
				p13 * k4 + p22 * k6 + p23 * k7;
			s1 = p03 * k0 + p13 * k3 + p23 * k6;
...
/* #ifdef TYPE_SHORT */
			...
			for (; i < wid; i++) {
				...
				D2I(buffo[i],
				    (p00 * k0 + p01 * k1 + p02 * k2 +
					p10 * k3 + p11 * k4 + p12 * k5 +
					p20 * k6 + p21 * k7 + p22 * k8) /
					scalef);
...
CONV_FUNC(4x4)
{
	FTYPE *buff0, *buff1, *buff2, *buff3, *buff4;
	FTYPE *buffs[8 * (KSIZE + 1)];
	mlib_s64 scalef = 1;
	mlib_s64 k[KSIZE * KSIZE];
	mlib_s64 k0, k1, k2, k3, k4, k5, k6, k7;
	mlib_s64 p00, p01, p02, p03, p04,
	    p10, p11, p12, p13, p14, p20, p21, p22, p23, p30, p31, p32, p33;
	...
	wid1 = (wid + 1) & ~1;
	xsize = wid1 * 4;
	bsize = (KSIZE + 1) * xsize * sizeof (FTYPE) +
		wid1 * sizeof (mlib_s64) + 2 * wid1 * sizeof (mlib_s32) +
		xsize * sizeof (LTYPE);
	pbuff = __mlib_malloc(bsize);
	...
	for (i = 0; i < 4 * (KSIZE + 1); i++)
		buffs[4 * (KSIZE + 1) + i] = buffs[i];
	buffd = (mlib_s64 *)(buffs[4 * (KSIZE + 1) - 1] + wid1);
	buffo = (mlib_s32 *)(buffd + wid1);
	buffi = buffo + wid1;
	bufft = (LTYPE *) (buffi + wid1);
...
#pragma pipeloop(0)
			...
			for (i = 0; i <= (wid - 2); i += 2) {
				...
				dd.s64 = *(mlib_s64 *)(buffi + i);
				buff4[i] = (FTYPE) dd.i32s.i0;
				buff4[i + 1] = (FTYPE) dd.i32s.i1;

				buffd[i] =
				    p00 * k0 + p01 * k1 +
					p02 * k2 + p03 * k3 + p10 * k4 +
					p11 * k5 + p12 * k6 + p13 * k7;
				buffd[i + 1] =
				    p01 * k0 + p02 * k1 +
					p03 * k2 + p04 * k3 + p11 * k4 +
					p12 * k5 + p13 * k6 + p14 * k7;
...
#pragma pipeloop(0)
			...
			for (i = 0; i <= (wid - 2); i += 2) {
				...
				D2I(d0,
				    (p00 * k0 + p01 * k1 + p02 * k2 +
					p03 * k3 + p10 * k4 + p11 * k5 +
					p12 * k6 + p13 * k7 +
				    buffd[i]) / scalef);
				D2I(d1,
				    (p01 * k0 + p02 * k1 + p03 * k2 +
					p04 * k3 + p11 * k4 + p12 * k5 +
					p13 * k6 + p14 * k7 +
				    buffd[i + 1]) / scalef);
...
CONV_FUNC(5x5)
{
	FTYPE *buff0, *buff1, *buff2, *buff3, *buff4, *buff5;
	FTYPE *buffs[8 * (KSIZE + 1)];

	mlib_s64 scalef = 1;
	mlib_s64 k[KSIZE * KSIZE];
	mlib_s64 k0, k1, k2, k3, k4, k5, k6, k7, k8, k9;
	mlib_s64 p00, p01, p02, p03, p04, p05,
	    p10, p11, p12, p13, p14, p15,
	    p20, p21, p22, p23, p24,
	    p30, p31, p32, p33, p34, p40, p41, p42, p43, p44;
...
#pragma pipeloop(0)
			...
			for (i = 0; i <= (wid - 2); i += 2) {
				...
				buffd[i] =
				    p00 * k0 + p01 * k1 +
					p02 * k2 + p03 * k3 + p04 * k4 +
					p10 * k5 + p11 * k6 + p12 * k7 +
				    p13 * k8 + p14 * k9;
				buffd[i + 1] =
				    p01 * k0 + p02 * k1 +
					p03 * k2 + p04 * k3 + p05 * k4 +
					p11 * k5 + p12 * k6 + p13 * k7 +
				    p14 * k8 + p15 * k9;
...
#pragma pipeloop(0)
			...
			for (i = 0; i <= (wid - 2); i += 2) {
				...
				D2I(d0,
				    (p00 * k0 + p01 * k1 + p02 * k2 + p03 * k3 +
				    p04 * k4 + buffd[i]) / scalef);
				D2I(d1,
				    (p01 * k0 + p02 * k1 + p03 * k2 + p04 * k3 +
				    p05 * k4 + buffd[i + 1]) / scalef);
    ...
    const void *colormap)
{
	...
	mlib_s64 *buffd;
	mlib_s64 scalef = 1;
	mlib_s64 k_arr[MAX_MN], *k = k_arr;
	mlib_s64 k0, k1, k2, k3, k4, k5, k6;
	mlib_s64 p0, p1, p2, p3, p4, p5, p6, p7;
	...
	if ((m * n) > MAX_MN) {
		k = __mlib_malloc((m * n) * sizeof (mlib_s64));
	...
	wid1 = (wid + 1) & ~1;
	xsize = wid1 * 4;
	bsize = (n + 1) * xsize * sizeof (FTYPE) + wid1 * sizeof (mlib_s64) +
	    2 * wid1 * sizeof (mlib_s32) + xsize * sizeof (LTYPE);
	pbuff = __mlib_malloc(bsize);
	...
	for (i = 0; i < 4 * (n + 1); i++)
		buffs[4 * (n + 1) + i] = buffs[i];
	buffd = (mlib_s64 *)(buffs[4 * (n + 1) - 1] + wid1);
	buffo = (mlib_s32 *)(buffd + wid1);
	buffi = buffo + wid1;
	bufft = (LTYPE *) (buffi + wid1);
