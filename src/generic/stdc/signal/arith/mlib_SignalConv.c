...
#pragma ident	"@(#)mlib_SignalConv.c	9.2	07/10/09 SMI"
...
__mlib_SignalConv_S16_S16_Sat(
    ...
    mlib_s32 n)
{
	mlib_d64 a_src1[MLIB_FRAME + 2 * 4];
	mlib_d64 a_src2[MLIB_FRAME + 2 * 4];
	mlib_d64 a_dst[MLIB_FRAME * 2 + 4];
...
/* determine and serve shorter as response */
	...
	if ((m <= MLIB_FRAME) && (n <= MLIB_FRAME) &&
	    (s <= 2 * MLIB_FRAME + 4)) {
		mlib_SignalConvKernel_S16(dst, src1, src2,
...
__mlib_SignalConv_S16S_S16S_Sat(
    ...
    mlib_s32 n)
{
	mlib_d64 a_src1[2 * MLIB_FRAME + 8];
	mlib_d64 a_src2[2 * MLIB_FRAME + 16];
	mlib_d64 a_dst[4 * MLIB_FRAME + 8];
	mlib_d64 *dsrc1 = a_src1, *dsrc2 = a_src2, *ddst = a_dst;
