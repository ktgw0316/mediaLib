...
#pragma ident	"@(#)mlib_GraphicsClipLineSet_G.c	9.2	07/10/09 SMI"

/*
 *  FUNCTIONS
 *    mlib_GraphicsClipLineSet_G_8    - clipping function for Gouraud mode
 *    mlib_GraphicsClipLineSet_G_32   - clipping function for Gouraud mode
 *
 *  SYNOPSIS
 *    void  mlib_GraphicsClipLineSet_G_[8|32]
 *                                    (mlib_u8  **rtable,
 *                                     mlib_s32 width,
 *                                     mlib_s32 height,
 *                                     mlib_s32 stride,
	...
	void (*SpamLineSet) ())
{
	...
	mlib_s32 i, x0, y0, x1, y1, c0, c1, count, alllines;
	mlib_s32 zz, adx, ady, clip, clipexist, half = -MLIB_HALF - 1;
	mlib_s32 ll0, ll1, sl = 0;
	mlib_f32 f_div0, f_mul0, cf_mul0;
	mlib_f32 f_div1, f_mul1, cf_mul1;
...
#pragma pipeloop(0)
		...
		if (clipexist) {
			...
			for (i = 0; i < count; i++) {
				if (pa[MLIB_GDS_LENGTHLINES] < 0) {
					mlib_f32 f_x0, f_y0, f_x1, f_y1, fc0,
						f_tB, f_tE, f_ll, f_sl;
					mlib_f32 f_w, f_h;
		...
		SpamLineSet(rtable, stride, ps, pe, width - 1, height - 1);
	...
	void (*SpamLineSet) ())
{
	...
	mlib_s16 *px0_ = x, *px1_ = x + 1, *px0, *px1;
	mlib_s32 *pc0 = c, *pc1 = c + 1;
	mlib_s32 i, x0, y0, x1, y1, c0, c1, c0_, c1_, count, alllines;
	mlib_s32 zz, adx, ady, clip, clipexist, half = -MLIB_HALF - 1;
	mlib_s32 ll0, sl = 0;
	mlib_f32 f_div0, f_mul0, cf_mul0, cf_mul1, cf_mul2;
	mlib_f32 f_width, f_height, f_WIDTH, f_HEIGHT;
	mlib_f32 f_zero = 0.f, f_one = 1.f;
