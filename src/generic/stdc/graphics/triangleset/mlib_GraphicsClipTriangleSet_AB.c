...
#pragma ident	"@(#)mlib_GraphicsClipTriangleSet_AB.c	9.2	07/10/09 SMI"

/*
 *  FUNCTIONS
 *    mlib_GraphicsClipTriangleSet    - clipping function for simple modes
 *
 *  SYNOPSIS
...
void
mlib_GraphicsClipTriangleSet_AB(
    mlib_u8 **rtable,
    mlib_s32 width,
    mlib_s32 height,
    mlib_s32 stride,
    ...
    void (*SpamLineSet) ())
{
	...
	while (all > 0) {
		...
		mlib_GraphicsClipLineSet_AB(rtable, width, height, stride, xl,
		    yl, nlines + (k >> 1), 2, 2, c, a, SpamLineSet);
