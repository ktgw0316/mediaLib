...
#pragma ident	"@(#)mlib_GraphicsFillArc_B.c	9.2	07/10/09 SMI"

/*
 * FUNCTIONS
 *    mlib_GraphicsFillArc_B_[8 | 32] - draw filled arc with alpha blending.
 *
 * SYNOPSIS
 *    mlib_status mlib_GraphicsFillArc_B_8 (mlib_image *buffer,
 *                                          mlib_s16   x,
 *                                          mlib_s16   y,
 *                                          mlib_s32   r,
 *                                          mlib_f32   t1,
 ...
 *                                          mlib_s32   a)
 *
 *    mlib_status mlib_GraphicsFillArc_B_32(mlib_image *buffer,
 *                                          mlib_s16   x,
 *                                          mlib_s16   y,
 *                                          mlib_s32   r,
 *                                          mlib_f32   t1,
 ...
 *
 * DESCRIPTION
 *    Function fills a circle arc of radius r and center (x, y)
...
#pragma weak mlib_GraphicsFillArc_B_32 = __mlib_GraphicsFillArc_B_32
#pragma weak mlib_GraphicsFillArc_B_8 = __mlib_GraphicsFillArc_B_8

#elif defined(__GNUC__)	/* defined(__SUNPRO_C) */
__typeof__(__mlib_GraphicsFillArc_B_32) mlib_GraphicsFillArc_B_32
    __attribute__((weak, alias("__mlib_GraphicsFillArc_B_32")));
__typeof__(__mlib_GraphicsFillArc_B_8) mlib_GraphicsFillArc_B_8
    __attribute__((weak, alias("__mlib_GraphicsFillArc_B_8")));
...
#define	CHECK_CLIPPING(UP, DOWN, LEFT, RIGHT)  {                           \
	if ((x + RIGHT >= width) || (x + LEFT < 0) ||			   \
		(y + DOWN >= height) || (y + UP < 0))			   \
	    clip = 1; 							   \
	else clip = 0;							   \
}
...
#define	DRAW_LINE_8(VAR, ARRAYleft, ARRAYright, SIGN)         \
	{                                                     \
	    MLIB_FILL_ROW_B_8(line, ARRAYleft[VAR],           \
		ARRAYright[VAR], ca, a_1, vca, va_1)          \
	    line = line SIGN stride;                          \
	}

/* *********************************************************** */

#define	DRAW_LINE_CLIP_8(VAR, ARRAYleft, ARRAYright, SIGN)              \
	{                                                               \
	    if (((y - SIGN VAR) >= 0) && ((y - SIGN VAR < height))) {   \
		LEFT(x ARRAYleft[VAR], xb);                             \
		RIGHT(x ARRAYright[VAR], xe);                           \
		MLIB_FILL_ROW_B_8(line - x, xb, xe, ca, a_1, vca, va_1) \
	    }                                                           \
	    line = line SIGN stride;                                    \
	}
...
#define	DRAW_LINE_32(VAR, ARRAYleft, ARRAYright, SIGN) 		\
	{                                                       \
	    MLIB_FILL_ROW_B_32(line, ARRAYleft[VAR],            \
		ARRAYright[VAR], ca1, ca2, ca3, a_1, vca, va_1) \
	    line = line SIGN stride;                            \
	}

/* *********************************************************** */

#define	DRAW_LINE_CLIP_32(VAR, ARRAYleft, ARRAYright, SIGN)              \
	{                                                                \
	    if (((y - SIGN VAR) >= 0) && ((y - SIGN VAR < height))) {    \
		LEFT(x ARRAYleft[VAR], xb);                              \
		RIGHT(x ARRAYright[VAR], xe);                            \
		MLIB_FILL_ROW_B_32(line - x, xb, xe, ca1, ca2, ca3, a_1, \
				vca, va_1)				 \
	    }                                                            \
	    line = line SIGN stride;                                     \
	}
...
#define	DRAW_PART_ARC(END, SIGN_STEP, B_1, B_2, B_3, B_4,            \
	...
	{                                                            \
		...
		DRAW_LINE_##FORM(start, -buf, +buf, SIGN_STEP);       \
	    }                                                        \
	    if (side == 1) {                                         \
		for (; start_1 > start_2; start_1--) {               \
		    DRAW_LINE_##FORM(start_1, B_1, B_2, SIGN_STEP);   \
		}                                                    \
	    } else if (side == 2) {                                  \
		for (; start_1 > start_2; start_1--) {               \
		    DRAW_LINE_##FORM(start_1, B_3, B_4, SIGN_STEP);   \
		}                                                    \
	    }                                                        \
	    for (; start_1 >= (END); start_1--) {                    \
		DRAW_LINE_##FORM(start_1, B_5, B_6, SIGN_STEP);       \
	    }                                                        \
	}
...
#define	DRAW_PART_ARC_CLIP(END, SIGN_STEP, B_1, B_2, B_3,                 \
	...
	{                                                                 \
	    line = line0 - (SIGN_STEP stride * start);                    \
	    for (; start > start_1; start--) {                            \
		DRAW_LINE_CLIP_##FORM(start, -buf, +buf, SIGN_STEP);      \
	    }                                                             \
	    if (side == 1) {                                              \
		for (; start_1 > start_2; start_1--) {                    \
		    DRAW_LINE_CLIP_##FORM(start_1, B_1, B_2, SIGN_STEP);  \
		}                                                         \
	    } else if (side == 2) {                                       \
		for (; start_1 > start_2; start_1--) {                    \
		    DRAW_LINE_CLIP_##FORM(start_1, B_3, B_4, SIGN_STEP);  \
		}                                                         \
	    }                                                             \
	    for (; start_1 >= (END); start_1--) {                         \
		DRAW_LINE_CLIP_##FORM(start_1, B_5, B_6, SIGN_STEP);      \
	    }                                                             \
	}
...
#define	SAVE_LINE_CLIP(VAR, ARRAYleft, ARRAYright, SIGN)              	 \
	{                                                                \
	    mlib_s32 i; 	                                         \
	    if (((y - SIGN VAR) >= 0) && ((y - SIGN VAR < height))) {    \
		LEFT(x ARRAYleft[VAR], xb);                              \
...
#define	RESTORE_LINE_CLIP(VAR, ARRAYleft, ARRAYright, SIGN)              \
	{                                                                \
	    mlib_s32 i; 	                                         \
	    if (((y - SIGN VAR) >= 0) && ((y - SIGN VAR < height))) {    \
		LEFT(x ARRAYleft[VAR], xb);                              \
    ...
    mlib_s32 a)
{
	...
	if (r == 0) {
		if (y < height && y >= 0 && x < width && x >= 0)
			MLIB_GRAPHICS_BLEND_8((data + (stride * y + x)));
		return (MLIB_SUCCESS);
	}
	...
	if (mlib_fabs(t1 - t2) >= PIx2)
		return (__mlib_GraphicsFillCircle_B_8
		    (buffer, xx, yy, r, c, a));
	...
	if (t2 > PIx2) {
		...
		count_repeat = 0;
		if (r > RADMAX) {
			overlap = __mlib_malloc(sizeof (mlib_u8) * r);
			if (!overlap)
				return (MLIB_FAILURE);
		}
	}
	...
	for (; count_repeat < 2; count_repeat++) {
		...
		if (oct2 < oct1) {
			mask =
			    __mlib_GraphicsDrawLine_B_8(buffer, xx, yy,
			    xx + cs1, yy + sn1, c, a);
		...
		if ((sn1 == 0) && (sn2 == 0) && (cs1 >= 0) && (cs2 >= 0) &&
		    (oct2 - oct1 > 4))
			return __mlib_GraphicsFillCircle_B_8(buffer, xx, yy, r,
			    c, a);
		...
		if (repeat == 1) {
			...
			if (start_sin1 > 0 && sn2 > 0) {
				...
				for (; (start >= end); start--) {
					SAVE_LINE_CLIP(start, +line_fill_2,
					    +line_fill_1, +);
				}
				...
				line = line0 + stride * start;
				for (; (start >= end); start--) {
					SAVE_LINE_CLIP(start, +line_fill_1,
					    +line_fill_2, -);
				}
			}
		}
	...
	if (repeat == 1) {
		if (start_sin1 > 0 && sn2 > 0) {
			...
			line = line0 - stride * start;
			for (; (start >= end); start--) {
				RESTORE_LINE_CLIP(start, +line_fill_2,
				    +line_fill_1, +);
			}
		} else if (start_sin1 < 0 && sn2 < 0) {
			start = (start_sin1 > sn2) ? -start_sin1 : -sn2;
			...
			line = line0 + stride * start;
			for (; (start >= end); start--) {
				RESTORE_LINE_CLIP(start, +line_fill_1,
				    +line_fill_2, -);
			}
		}
	}
	...
	if ((y | (height - 1 - y)) >= 0) {
		MLIB_FILL_ROW_B_8(line0 - x, xb_last, xe_last, ca, a_1,
				vca, va_1);
	}
    ...
    mlib_s32 a)
{
	...
	mlib_s32 line_fill_1_0[RADMAX], line_fill_2_0[RADMAX];
	mlib_s32 *line_fill_1 = NULL, *line_fill_2 = NULL, *point_line = NULL;
	mlib_s32 overlap_buf[RADMAX], *overlap = overlap_buf;
	mlib_s32 start, start_1, start_2, end, help;
	mlib_s32 xb, xe, clip, side, count_repeat, repeat;
	mlib_f32 help_t2, start_sin1;
	mlib_s32 x = xx, y = yy, xb_last, xe_last;
	...
	if (r == 0) {
		if (y < height && y >= 0 && x < width && x >= 0)
			MLIB_GRAPHICS_BLEND_32((data + (stride * y + x)))
		return (MLIB_SUCCESS);
	}
	...
	if (mlib_fabs(t1 - t2) >= PIx2)
		return (__mlib_GraphicsFillCircle_B_32(buffer, xx, yy,
							r, c, a));
	...
	if (t2 > PIx2) {
		...
		count_repeat = 0;
		if (r > RADMAX) {
			overlap = __mlib_malloc(sizeof (mlib_s32) * r);
			if (!overlap)
				return (MLIB_FAILURE);
		}
	}
	...
	for (; count_repeat < 2; count_repeat++) {
		...
		if (oct2 < oct1) {
			mask =
			    __mlib_GraphicsDrawLine_B_32(buffer, xx, yy,
			    xx + cs1, yy + sn1, c, a);
		...
		if ((sn1 == 0) && (sn2 == 0) && (cs1 >= 0) && (cs2 >= 0) &&
		    (oct2 - oct1 > 4))
			return __mlib_GraphicsFillCircle_B_32(buffer, xx, yy, r,
			    c, a);
	...
	if ((y | (height - 1 - y)) >= 0) {
		MLIB_FILL_ROW_B_32(line0 - x, xb_last, xe_last,
