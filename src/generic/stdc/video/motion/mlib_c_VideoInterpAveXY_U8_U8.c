...
#pragma ident	"@(#)mlib_c_VideoInterpAveXY_U8_U8.c	9.2	07/10/09 SMI"
...
#define	MLIB_C_VIDEOINTERPAVG(width, height)                         \
	...
	for (y = 0; y < height; y++) {                               \
		...
		mlib_u32 d0 = *(mlib_u32 *)dd;                       \
	                                                             \
		dd[0] = (mlib_u32)(((d0 & 0xff) << 2) + (sd[0] +     \
			sd2[0]) + (sd[1] + 6 + sd2[1])) >> 3;        \
		dd[1] = (mlib_u32)(((d0 >> 6) & 0x3fc) + (sd[2] +    \
			sd2[2]) + (sd[1] + 6 + sd2[1])) >> 3;        \
		dd[2] = (mlib_u32)(((d0 >> 14) & 0x3fc) + (sd[2] +   \
			sd2[2]) + (sd[3] + 6 + sd2[3])) >> 3;        \
		dd[3] = (mlib_u32)(((d0 >> 22) & 0x3fc) + (sd[4] +   \
...
#define	MLIB_C_VIDEOINTERPAVG(width, height)                         \
	...
	for (y = 0; y < height; y++) {                               \
		...
		mlib_u32 d0 = *(mlib_u32 *)dd;                       \
	                                                             \
		dd[0] = (mlib_u32)(((d0 >> 22) & 0x3fc) + (sd[0] +   \
			sd2[0]) + (sd[1] + 6 + sd2[1])) >> 3;        \
		dd[1] = (mlib_u32)(((d0 >> 14) & 0x3fc) + (sd[2] +   \
			sd2[2]) + (sd[1] + 6 + sd2[1])) >> 3;        \
		dd[2] = (mlib_u32)(((d0 >> 6) & 0x3fc) + (sd[2] +    \
			sd2[2]) + (sd[3] + 6 + sd2[3])) >> 3;        \
		dd[3] = (mlib_u32)(((d0 & 0xff) << 2) + (sd[4] +     \
			sd2[4]) + (sd[3] + 6 + sd2[3])) >> 3;        \
		sd += 4;                                             \
		sd2 += 4;                                            \
		dd += 4;                                             \
