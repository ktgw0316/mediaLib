...
#pragma ident	"@(#)mlib_c_VideoInterpX_U8_U8.c	9.2	07/10/09 SMI"
...
#define	MLIB_C_VIDEOINTERP(width, height)                       \
	...
	for (y = 0; y < height; y++) {                          \
		...
		dd[1] = (mlib_u32)(sd[2] + (sd[1] + 1)) >> 1;   \
		dd[2] = (mlib_u32)(sd[2] + (sd[3] + 1)) >> 1;   \
		dd[3] = (mlib_u32)(sd[4] + (sd[3] + 1)) >> 1;   \
		dd[4] = (mlib_u32)(sd[4] + (sd[5] + 1)) >> 1;   \
		dd[5] = (mlib_u32)(sd[6] + (sd[5] + 1)) >> 1;   \
		dd[6] = (mlib_u32)(sd[6] + (sd[7] + 1)) >> 1;   \
		dd[7] = (mlib_u32)(sd[8] + (sd[7] + 1)) >> 1;   \
		sd += 8;                                        \
		dd += 8;                                        \
	    }                                                   \
