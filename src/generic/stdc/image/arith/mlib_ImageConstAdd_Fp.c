...
#pragma ident	"@(#)mlib_ImageConstAdd_Fp.c	9.2	07/10/09 SMI"

/*
 * FUNCTION
 *      mlib_ImageConstAdd_Fp - add an image with constant
 *
 * SYNOPSIS
 ...
 *      consts  array of constants to be added to each channel of the pixels
 *
 * RESTRICTION
...
static void mlib_ImageConstAdd_F32(
    mlib_f32 *dl,
    mlib_f32 *sl,
    mlib_s32 xsize,
    mlib_s32 ysize,
...
static void mlib_ImageConstAdd_D64(
    mlib_d64 *dl,
    mlib_d64 *sl,
    mlib_s32 xsize,
    mlib_s32 ysize,
    ...
    const mlib_d64 *consts)
{
	...
	switch (type) {
	case MLIB_FLOAT:
		mlib_ImageConstAdd_F32(pdst, psrc, xsize, ysize, nchan, slb / 4,
		    dlb / 4, consts);
		return (MLIB_SUCCESS);

	case MLIB_DOUBLE:
		mlib_ImageConstAdd_D64(pdst, psrc, xsize, ysize, nchan, slb / 8,
		    dlb / 8, consts);
		return (MLIB_SUCCESS);
