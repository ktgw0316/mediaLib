...
#pragma ident	"@(#)mlib_ImageMul_Fp.c	9.2	07/10/09 SMI"

/*
 * FUNCTION
 *      mlib_ImageMul_Fp - multiply two images with shifting
 *
 * SYNOPSIS
...
#define	ARITH_FUN(dst, src1, src2)	(dst) = (src1) * (src2)
...
#pragma pipeloop(0)
				...
				for (; pd < (pend - 1);
					...
					s1.d64 = *(mlib_d64 *)ps1;
					s2.d64 = *(mlib_d64 *)ps2;
					ARITH_FUN(d.f32s.f0, val, s2.f32s.f0);
					ARITH_FUN(d.f32s.f1, s1.f32s.f0,
					    s2.f32s.f1);
					*(mlib_d64 *)pd = d.d64;
					val = s1.f32s.f1;
				}
...
#pragma pipeloop(0)
				...
				for (; pd < (pend - 1);
					...
					s1.d64 = *(mlib_d64 *)ps1;
					s2.d64 = *(mlib_d64 *)ps2;
					ARITH_FUN(d.f32s.f0, val, s1.f32s.f0);
					ARITH_FUN(d.f32s.f1, s2.f32s.f0,
					    s1.f32s.f1);
					*(mlib_d64 *)pd = d.d64;
					val = s2.f32s.f1;
				}
...
#pragma pipeloop(0)
				...
				for (; pd < (pend - 1);
					...
					s1.d64 = *(mlib_d64 *)ps1;
					s2.d64 = *(mlib_d64 *)ps2;
					ARITH_FUN(*pd, s1.f32s.f0, s2.f32s.f0);
					ARITH_FUN(*(pd + 1), s1.f32s.f1,
					    s2.f32s.f1);
				}
			} else {
/* all aligned */
...
#pragma pipeloop(0)
				...
				for (; pd < (pend - 1);
					...
					s1.d64 = *(mlib_d64 *)ps1;
					s2.d64 = *(mlib_d64 *)ps2;
					ARITH_FUN(d.f32s.f0, s1.f32s.f0,
					    s2.f32s.f0);
					ARITH_FUN(d.f32s.f1, s1.f32s.f1,
					    s2.f32s.f1);
					*(mlib_d64 *)pd = d.d64;
				}
			}
		...
		for (j = 0; j < ysize; j++) {
			mlib_d64 *ps1 = (mlib_d64 *)psrc1,
			    *ps2 = (mlib_d64 *)psrc2, *pd = (mlib_d64 *)pdst;
#ifdef __SUNPRO_C
#pragma pipeloop(0)
#endif /* __SUNPRO_C */
			for (i = 0; i < xsize; i++) {
