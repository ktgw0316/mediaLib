...
#pragma ident	"@(#)mlib_c_ImageBlendColor_Inp.c	9.2	07/10/09 SMI"
...
#define	FUNC_NAME	mlib_ImageBlendColor_Inp_U8
#define	SHIFT	8
...
#define	BLEND_COLOR(src, color)                                 \
	(mlib_s32)(fa * src + (fone - fa) * color) >> SHIFT

#include <mlib_ImageBlendColor.h>

/* *********************************************************** */

#define	FUNC_NAME	mlib_ImageBlendColor_Inp_S16
#define	SHIFT	15
#define	MAX_VALUE	((1u << SHIFT) - 1)
...
#define	FUNC_NAME	mlib_ImageBlendColor_Inp_U16
#define	SHIFT	16
#define	MAX_VALUE	((1u << SHIFT) - 1)
#define	ONE	1.0/(1 << SHIFT)
...
/* for VIS 1-D vector only if aligned */
	...
	case MLIB_BYTE:
		SATUR_COLOR(U8);
		return mlib_ImageBlendColor_Inp_U8(NULL, ddata, 0, dstride,
		    width, height, channel, alpha, color1);

	case MLIB_SHORT:
		SATUR_COLOR(S16);
		return mlib_ImageBlendColor_Inp_S16(NULL, ddata, 0, dstride,
		    width, height, channel, alpha, color1);

	case MLIB_USHORT:
		SATUR_COLOR(U16);
		return mlib_ImageBlendColor_Inp_U16(NULL, ddata, 0, dstride,
		    width, height, channel, alpha, color1);
