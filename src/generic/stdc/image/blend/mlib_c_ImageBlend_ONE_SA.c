...
#pragma ident	"@(#)mlib_c_ImageBlend_ONE_SA.c	9.2	07/10/09 SMI"
...
#pragma pipeloop(0)
		...
		for (i = 0; i < dst_width; i++) {
			...
			pdst[4 * i] =
			    FLOAT2INT_CLAMP(mlib_c_blend_u8_sat[psrc2[4 * i]] *
			    alpha + mlib_c_blend_u8_sat[psrc1[4 * i]]) >> 23;
			pdst[4 * i + 1] =
			    FLOAT2INT_CLAMP(mlib_c_blend_u8_sat[psrc2[4 * i +
			    1]] * alpha + mlib_c_blend_u8_sat[psrc1[4 * i +
			    1]]) >> 23;
			pdst[4 * i + 2] =
			    FLOAT2INT_CLAMP(mlib_c_blend_u8_sat[psrc2[4 * i +
			    2]] * alpha + mlib_c_blend_u8_sat[psrc1[4 * i +
			    2]]) >> 23;
			pdst[4 * i + 3] =
			    FLOAT2INT_CLAMP(mlib_c_blend_u8_sat[psrc2[4 * i +
			    3]] * alpha + mlib_c_blend_u8_sat[psrc1[4 * i +
			    3]]) >> 23;
		}
