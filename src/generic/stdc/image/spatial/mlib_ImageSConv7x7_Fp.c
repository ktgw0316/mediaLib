...
#pragma ident	"@(#)mlib_ImageSConv7x7_Fp.c	9.2	07/10/09 SMI"

/*
 * FUNCTION
 *      mlib_ImageSConv7x7_Fp - separable convolve of an image with a 7x7 kernel
 *
 * SYNOPSIS
 *      mlib_status mlib_ImageSConv7x7_Fp(mlib_image       *dst,
...
#pragma weak mlib_ImageSConv7x7_Fp = __mlib_ImageSConv7x7_Fp

#elif defined(__GNUC__)	/* defined(__SUNPRO_C) */
__typeof__(__mlib_ImageSConv7x7_Fp) mlib_ImageSConv7x7_Fp
    __attribute__((weak, alias("__mlib_ImageSConv7x7_Fp")));
    ...
    mlib_edge edge)
{
	...
	if (edge != MLIB_EDGE_SRC_EXTEND) {
		if (mlib_ImageGetWidth(src_i) >= ksize &&
			...
			switch (type) {
			case MLIB_FLOAT:
				mlib_sconv7x7nw_f32(dst_i, src_i, hkernel,
				    vkernel, cmask);
				break;

			case MLIB_DOUBLE:
				mlib_sconv7x7nw_d64(dst_i, src_i, hkernel,
				    vkernel, cmask);
				break;
...
/* adjust src_e image */
		...
		switch (type) {
		case MLIB_FLOAT:
			mlib_sconv7x7ext_f32(dst_e, src_e, dx_l, dx_r, dy_t,
			    dy_b, hkernel, vkernel, cmask);
			break;
		case MLIB_DOUBLE:
			mlib_sconv7x7ext_d64(dst_e, src_e, dx_l, dx_r, dy_t,
			    dy_b, hkernel, vkernel, cmask);
			break;
