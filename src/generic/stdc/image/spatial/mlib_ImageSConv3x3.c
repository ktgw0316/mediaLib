...
#pragma ident	"@(#)mlib_ImageSConv3x3.c	9.2	07/10/09 SMI"
		...
		if (mlib_ImageGetWidth(src_i) >= ksize &&
			...
			case MLIB_BYTE:
				...
				if (scale < 24 || scale > 31)
					return (MLIB_FAILURE);
				mlib_sconv3x3nw_u8(dst_i, src_i, hkernel,
				    vkernel, scale, cmask);
				break;
			...
			case MLIB_SHORT:
				...
				if (scale < 26 || scale > 33)
					return (MLIB_FAILURE);
				mlib_sconv3x3nw_s16(dst_i, src_i, hkernel,
				    vkernel, scale, cmask);
				break;
			...
			case MLIB_USHORT:
				...
				if (scale < 26 || scale > 33)
					return (MLIB_FAILURE);
				mlib_sconv3x3nw_u16(dst_i, src_i, hkernel,
				    vkernel, scale, cmask);
				break;
			...
			case MLIB_INT:
				...
				if (scale < 0)
					return (MLIB_FAILURE);
				mlib_sconv3x3nw_s32(dst_i, src_i, hkernel,
				    vkernel, scale, cmask);
				break;
