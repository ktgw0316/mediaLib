...
#pragma ident	"@(#)mlib_ImageConvCopyEdge_Bit.c	9.2	07/10/09 SMI"
 ...
 *
 * RESTRICTION
 *      The src and the dst must be the MLIB_BIT type, same width, same height
 *      and have same number of channels (1). The unselected channels are not
	...
	if (bitoffd == bitoffs) {
		...
		if (dx_l > 0) {
			if (bitoffd + dx_l <= 8) {
				...
				for (i = dy_t; i < (img_height - dy_b); i++) {
					pd[i * img_strided + amount - 1] =
					    (pd[i * img_strided + amount -
					    1] & ~mask) | (ps[i * img_strides +
					    amount - 1] & mask);
				}
			}
		}
		...
		for (i = 0; i < dy_t; i++) {
			...
			pd[i * img_strided] =
			    ...
			    img_strided] & mask);
			pd[i * img_strided + amount - 1] =
			    (tmp_end & (~mask_end)) | (pd[i * img_strided +
			    amount - 1] & mask_end);
		}
		...
		amount1 = (bitoff_end && shift2 >= bitoff_end) ? 1 : 0;
		...
		for (i = 0; i < dy_t; i++) {
			tmp_start = pd[i * img_strided];
			tmp_end = pd[i * img_strided + amount - 1];
			if (!test) s0 = ps[i * img_strides]; else s0 = 0;
			for (j = 0; j < amount - amount1; j++) {
