...
#pragma ident	"@(#)mlib_ImageConv_32nw.c	9.2	07/10/09 SMI"
    ...
    mlib_s32 cmask)
{
	...
	if (wid > BUFF_LINE) {
		pbuff = __mlib_malloc((KSIZE + 1) * sizeof (mlib_d64) * wid);
    ...
    mlib_s32 cmask)
{
	...
	if (wid > BUFF_LINE) {
		pbuff = __mlib_malloc((KSIZE + 2) * sizeof (mlib_d64) * wid);
    ...
    mlib_s32 cmask)
{
	...
	if ((bsize > BUFF_SIZE) || (n > MAX_N)) {
		pbuff =
		    __mlib_malloc(sizeof (mlib_d64) * bsize +
		    sizeof (mlib_d64 *) * 2 * (n + 1));
