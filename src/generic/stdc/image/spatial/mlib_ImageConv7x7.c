...
#pragma ident	"@(#)mlib_ImageConv7x7.c	9.2	07/10/09 SMI"
		...
		if (mlib_ImageGetWidth(src_i) >= ksize &&
			...
			case MLIB_BYTE:
				...
				if (scale < 16 || scale > 31)
					return (MLIB_FAILURE);
				mlib_conv7x7nw_u8(dst_i, src_i, kernel, scale,
				    cmask);
				break;
			case MLIB_INT:
				...
				if (scale < 0)
					return (MLIB_FAILURE);
				mlib_conv7x7nw_s32(dst_i, src_i, kernel, scale,
				    cmask);
				break;
			default:
				return (MLIB_FAILURE);
...
/* adjust src_e image */
		...
		case MLIB_BYTE:
			...
			if (scale < 16 || scale > 31)
				return (MLIB_FAILURE);
			mlib_conv7x7ext_u8(dst_e, src_e, dx_l, dx_r, dy_t, dy_b,
			    kernel, scale, cmask);
			break;
		case MLIB_INT:
			...
			if (scale < 0)
				return (MLIB_FAILURE);
			mlib_conv7x7ext_s32(dst_e, src_e, dx_l, dx_r, dy_t,
