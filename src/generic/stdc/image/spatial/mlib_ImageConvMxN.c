...
#pragma ident	"@(#)mlib_ImageConvMxN.c	9.2	07/10/09 SMI"
    ...
    mlib_edge edge)
{
	...
	return mlib_ImageConvMxN_f(dst, src, kernel, m, n, dm, dn, scale, cmask,
	    edge);
}
    ...
    mlib_edge edge)
{
	...
	if (edge != MLIB_EDGE_SRC_EXTEND) {
		if (mlib_ImageGetWidth(dst_i) >= m &&
			...
			case MLIB_BYTE:
				ret =
				    mlib_convMxNnw_u8(dst_i, src_i, kernel, m,
				    n, dm, dn, scale, cmask);
				break;
			case MLIB_SHORT:
#ifdef __sparc
				ret =
				    mlib_convMxNnw_s16(dst_i, src_i, kernel, m,
				    n, dm, dn, scale, cmask);
#else /* __sparc */

				if (mlib_ImageConvVersion(m, n, scale,
				    type) == 0)
					ret =
...
#ifdef __sparc
				ret =
				    mlib_convMxNnw_u16(dst_i, src_i, kernel, m,
				    n, dm, dn, scale, cmask);
#else /* __sparc */
...
/* adjust src_e image */
		...
		case MLIB_BYTE:
			ret =
			    mlib_convMxNext_u8(dst_e, src_e, kernel, m, n, dx_l,
			    dx_r, dy_t, dy_b, scale, cmask);
			break;
		case MLIB_SHORT:
#ifdef __sparc
			ret =
			    mlib_convMxNext_s16(dst_e, src_e, kernel, m, n,
...
#ifdef __sparc
			ret =
			    mlib_convMxNext_u16(dst_e, src_e, kernel, m, n,
			    dx_l, dx_r, dy_t, dy_b, scale, cmask);
#else /* __sparc */
