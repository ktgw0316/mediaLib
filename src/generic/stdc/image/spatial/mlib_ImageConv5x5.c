...
#pragma ident	"@(#)mlib_ImageConv5x5.c	9.2	07/10/09 SMI"
		...
		if (mlib_ImageGetWidth(src_i) >= ksize &&
			...
			case MLIB_BYTE:
				...
				if (scale < 16 || scale > 31)
					return (MLIB_FAILURE);
				mlib_conv5x5nw_u8(dst_i, src_i, kernel, scale,
				    cmask);
				break;
			...
			case MLIB_SHORT:
				...
				if (scale < 17 || scale > 32)
					return (MLIB_FAILURE);
#ifdef __sparc
				mlib_conv5x5nw_s16(dst_i, src_i, kernel, scale,
				    cmask);
#else /* __sparc */

				if (mlib_ImageConvVersion(5, 5, scale,
				    type) == 0)
					mlib_conv5x5nw_s16(dst_i, src_i, kernel,
					    scale, cmask);
				else
					mlib_i_conv5x5nw_s16(dst_i, src_i,
					    kernel, scale, cmask);
#endif /* __sparc */
				break;
			...
			case MLIB_USHORT:
				...
				if (scale < 17 || scale > 32)
					return (MLIB_FAILURE);
#ifdef __sparc
				mlib_conv5x5nw_u16(dst_i, src_i, kernel, scale,
				    cmask);
#else /* __sparc */

				if (mlib_ImageConvVersion(5, 5, scale,
				    type) == 0)
					mlib_conv5x5nw_u16(dst_i, src_i, kernel,
					    scale, cmask);
				else
					mlib_i_conv5x5nw_u16(dst_i, src_i,
					    kernel, scale, cmask);
#endif /* __sparc */
				break;
			...
			case MLIB_INT:
				...
				if (scale < 0)
					return (MLIB_FAILURE);
				mlib_conv5x5nw_s32(dst_i, src_i, kernel, scale,
				    cmask);
				break;
...
/* adjust src_e image */
		...
		case MLIB_BYTE:
			...
			if (scale < 16 || scale > 31)
				return (MLIB_FAILURE);
			mlib_conv5x5ext_u8(dst_e, src_e, dx_l, dx_r, dy_t, dy_b,
			    kernel, scale, cmask);
			break;
		case MLIB_SHORT:
			...
			if (scale < 17 || scale > 32)
				return (MLIB_FAILURE);
#ifdef __sparc
			mlib_conv5x5ext_s16(dst_e, src_e, dx_l, dx_r, dy_t,
		...
		case MLIB_USHORT:
			...
			if (scale < 17 || scale > 32)
				return (MLIB_FAILURE);
#ifdef __sparc
			mlib_conv5x5ext_u16(dst_e, src_e, dx_l, dx_r, dy_t,
			    dy_b, kernel, scale, cmask);
#else /* __sparc */
