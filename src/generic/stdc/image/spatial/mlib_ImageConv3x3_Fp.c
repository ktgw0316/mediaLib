...
#pragma ident	"@(#)mlib_ImageConv3x3_Fp.c	9.2	07/10/09 SMI"
    ...
    mlib_edge edge)
{
	...
	if (edge != MLIB_EDGE_SRC_EXTEND) {
		if (mlib_ImageGetWidth(src_i) >= ksize &&
			...
			switch (type) {
			case MLIB_FLOAT:
				mlib_conv3x3nw_f32(dst_i, src_i, kernel, cmask);
				break;
			case MLIB_DOUBLE:
				mlib_conv3x3nw_d64(dst_i, src_i, kernel, cmask);
				break;
			default:
				return (MLIB_FAILURE);
			}
...
/* adjust src_e image */
		...
		switch (type) {
		case MLIB_FLOAT:
			mlib_conv3x3ext_f32(dst_e, src_e, dx_l, dx_r, dy_t,
			    dy_b, kernel, cmask);
			break;
		case MLIB_DOUBLE:
			mlib_conv3x3ext_d64(dst_e, src_e, dx_l, dx_r, dy_t,
			    dy_b, kernel, cmask);
			break;
		default:
			return (MLIB_FAILURE);
