...
#pragma ident	"@(#)mlib_ImageConv2x2.c	9.2	07/10/09 SMI"
		...
		if (mlib_ImageGetWidth(src_i) >= ksize &&
			...
			case MLIB_BYTE:
				...
				if (scale < 16 || scale > 31)
					return (MLIB_FAILURE);
				mlib_conv2x2nw_u8(dst_i, src_i, kernel, scale,
				    cmask);
				break;
			case MLIB_SHORT:
				...
				if (scale < 17 || scale > 32)
					return (MLIB_FAILURE);
				mlib_conv2x2nw_s16(dst_i, src_i, kernel, scale,
				    cmask);
				break;
			case MLIB_USHORT:
				...
				if (scale < 17 || scale > 32)
					return (MLIB_FAILURE);
				mlib_conv2x2nw_u16(dst_i, src_i, kernel, scale,
				    cmask);
				break;
			case MLIB_INT:
				...
				if (scale < 0)
					return (MLIB_FAILURE);
				mlib_conv2x2nw_s32(dst_i, src_i, kernel, scale,
				    cmask);
				break;
			default:
				return (MLIB_FAILURE);
...
/* MLIB_EDGE_SRC_EXTEND */
/* adjust src_e image */
		mlib_ImageSetSubimage(src_e, src_e, dx_l, dy_t,
		...
		case MLIB_BYTE:
			...
			if (scale < 16 || scale > 31)
				return (MLIB_FAILURE);
			mlib_conv2x2ext_u8(dst_e, src_e, dx_l, dx_r, dy_t, dy_b,
			    kernel, scale, cmask);
			break;
		case MLIB_SHORT:
			...
			if (scale < 17 || scale > 32)
				return (MLIB_FAILURE);
			mlib_conv2x2ext_s16(dst_e, src_e, dx_l, dx_r, dy_t,
		...
		case MLIB_USHORT:
			...
			if (scale < 17 || scale > 32)
				return (MLIB_FAILURE);
			mlib_conv2x2ext_u16(dst_e, src_e, dx_l, dx_r, dy_t,
			    dy_b, kernel, scale, cmask);
			break;
		case MLIB_INT:
