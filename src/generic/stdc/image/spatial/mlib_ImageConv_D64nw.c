...
#pragma ident	"@(#)mlib_ImageConv_D64nw.c	9.2	07/10/09 SMI"
    ...
    mlib_s32 cmask)
{
	...
	if (max_hsize > BUFF_SIZE) {
		pbuff = __mlib_malloc(sizeof (DTYPE) * max_hsize);
	}
    ...
    mlib_s32 cmask)
{
	...
	if (n * m > MAX_NM) {
		k = __mlib_malloc(n * m * sizeof (mlib_f32));
