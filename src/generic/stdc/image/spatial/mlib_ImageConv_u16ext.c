...
#pragma ident	"@(#)mlib_ImageConv_u16ext.c	9.2	07/10/09 SMI"
...
#define	LOAD_BUFF(buff)                                                 \
	* (mlib_s64 *)(buff + i) =                                      \
	    (((mlib_s64)sp[chan1]) << 32) | S64TOS32((mlib_s64)sp[0])
...
CONV_FUNC(3x3)
{
	FTYPE buff[(KSIZE + 2) * BUFF_LINE], *buff0, *buff1, *buff2, *buff3,
	    *buffT;
	DEF_VARS(DTYPE);
	DTYPE *sl2;
	...
	if (swid > BUFF_LINE) {
		pbuff = __mlib_malloc((KSIZE + 2) * sizeof (FTYPE) * swid);
...
#pragma pipeloop(0)
			...
			for (i = 0; i <= (wid - 2); i += 2) {
				...
				d1 = D2I(s1 + p02 * k1 + p03 * k2 + p12 * k4 +
...
CONV_FUNC(4x4)
{
	...
	if (swid > BUFF_LINE) {
		pbuff = __mlib_malloc((KSIZE + 3) * sizeof (FTYPE) * swid);
