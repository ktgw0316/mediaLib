...
#pragma ident	"@(#)mlib_ImageSConv5x5_Fp.c	9.2	07/10/09 SMI"
    ...
    mlib_edge edge)
{
	...
	if (edge != MLIB_EDGE_SRC_EXTEND) {
		if (mlib_ImageGetWidth(src_i) >= ksize &&
			...
			switch (type) {
			case MLIB_FLOAT:
				mlib_sconv5x5nw_f32(dst_i, src_i, hkernel,
				    vkernel, cmask);
				break;

			case MLIB_DOUBLE:
				mlib_sconv5x5nw_d64(dst_i, src_i, hkernel,
				    vkernel, cmask);
				break;
...
/* adjust src_e image */
		...
		switch (type) {
		case MLIB_FLOAT:
			mlib_sconv5x5ext_f32(dst_e, src_e, dx_l, dx_r, dy_t,
			    dy_b, hkernel, vkernel, cmask);
			break;
		case MLIB_DOUBLE:
			mlib_sconv5x5ext_d64(dst_e, src_e, dx_l, dx_r, dy_t,
			    dy_b, hkernel, vkernel, cmask);
			break;
