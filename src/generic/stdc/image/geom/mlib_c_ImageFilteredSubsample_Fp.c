...
#pragma ident	"@(#)mlib_c_ImageFilteredSubsample_Fp.c	9.2	07/10/09 SMI"

/*
 * FUNCTION
 *
 *      mlib_ImageFilteredSubsample_Fp - Antialias filter and subsample an image
 *
 * SYNOPSIS
 *
 ...
 *      The images can be of type MLIB_FLOAT or MLIB_DOUBLE.
 *
...
#pragma weak mlib_ImageFilteredSubsample_Fp = \
	__mlib_ImageFilteredSubsample_Fp

#elif defined(__GNUC__)	/* defined(__SUNPRO_C) */
__typeof__(__mlib_ImageFilteredSubsample_Fp) mlib_ImageFilteredSubsample_Fp
    __attribute__((weak, alias("__mlib_ImageFilteredSubsample_Fp")));
    ...
    mlib_edge edge)
{
	...
	if (kh >= 2 * scaleY) {
		switch (type) {
		case MLIB_FLOAT:
			return mlib_ImageFilteredSubsample_f32nw_2(dst, src,
			    scaleX, scaleY, transX, transY, hKernel, vKernel,
			    hSize, vSize, hParity, vParity, edge);
		case MLIB_DOUBLE:
			return mlib_ImageFilteredSubsample_d64nw_2(dst, src,
			    scaleX, scaleY, transX, transY, hKernel, vKernel,
			    hSize, vSize, hParity, vParity, edge);
		default:
			return (MLIB_FAILURE);
		...
		switch (type) {
		case MLIB_FLOAT:
			return mlib_ImageFilteredSubsample_f32nw_1(dst, src,
			    scaleX, scaleY, transX, transY, hKernel, vKernel,
			    hSize, vSize, hParity, vParity, edge);
		case MLIB_DOUBLE:
			return mlib_ImageFilteredSubsample_d64nw_1(dst, src,
			    scaleX, scaleY, transX, transY, hKernel, vKernel,
			    hSize, vSize, hParity, vParity, edge);
		default:
			return (MLIB_FAILURE);
