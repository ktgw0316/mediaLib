...
#pragma ident	"@(#)mlib_c_ImageAffineIndex_BL.c	9.3	07/10/09 SMI"
...
#define	COUNT_BL_U8(ind)                                        \
	pix0_##ind =                                            \
		a00_##ind + fdy * (a10_##ind - a00_##ind);      \
	pix1_##ind =                                            \
		a01_##ind + fdy * (a11_##ind - a01_##ind);      \
	res##ind =                                              \
...
#define	COUNT_BL_S16(ind)                                       \
	...
	pix1_##ind =                                            \
		a01_##ind + fdy * (a11_##ind - a01_##ind);      \
	res##ind =                                              \
		pix0_##ind + fdx * (pix1_##ind - pix0_##ind)
...
#define	FUNC_AFFINEINDEX_BL_0(ITYPE, LTYPE, NCHAN)                     \
	...
	{                                                              \
	    DECLAREVAR_IND();                                          \
	    mlib_##LTYPE *dp, buff_lcl[NCHAN * MLIB_LIMIT], *pbuff =   \
		    buff_lcl;                                          \
		...
		pbuff = __mlib_malloc(NCHAN *                          \
			sizeof (mlib_##LTYPE) * max_xsize);            \
		if (pbuff == NULL)                                     \
		    return (MLIB_FAILURE);                             \
	    }                                                          \
