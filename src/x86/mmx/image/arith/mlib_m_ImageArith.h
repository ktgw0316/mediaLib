...
#ifndef _MLIB_M_IMAGEARITH_H
#define	_MLIB_M_IMAGEARITH_H

#pragma ident	"@(#)mlib_m_ImageArith.h	9.2	07/11/05 SMI"

/*
 * Macro definitions for MMX version image arithmetic functions.
 */
...
/* *********************************************************** */
static void mlib_m_ImageArith_U8(
    mlib_u8 *da1,
    mlib_u8 *sa1,
    mlib_u8 *sa2,
    mlib_s32 dlb1,
...
static void mlib_m_ImageArith_S16(
    mlib_s16 *da1,
    mlib_s16 *sa1,
    mlib_s16 *sa2,
    mlib_s32 dlb1,
...
static void mlib_m_ImageArith_U16(
    mlib_u16 *da1,
    mlib_u16 *sa1,
    mlib_u16 *sa2,
    mlib_s32 dlb1,
...
static void mlib_m_ImageArith_S32(
    mlib_s32 *da1,
    mlib_s32 *sa1,
    mlib_s32 *sa2,
    mlib_s32 dlb1,
    ...
    const mlib_image *src2)
{
	...
	if (dtype == MLIB_BYTE) {
		...
		mlib_m_ImageArith_U8((mlib_u8 *)da, sa1, sa2, dlb, slb1, slb2,
		    dw, dh);
		return (MLIB_SUCCESS);
		...
		mlib_m_ImageArith_S16((mlib_s16 *)da, sa1, sa2, dlb, slb1, slb2,
		    dw, dh);
		return (MLIB_SUCCESS);
		...
		mlib_m_ImageArith_U16((mlib_u16 *)da, sa1, sa2, dlb, slb1, slb2,
		    dw, dh);
		return (MLIB_SUCCESS);
		...
		mlib_m_ImageArith_S32((mlib_s32 *)da, sa1, sa2, dlb, slb1, slb2,
		    dw, dh);
		return (MLIB_SUCCESS);
	}
...
/* row loop */
	for (; dh > 0; dh--) {
		...
		if (emask != 0xff) {
			...
			sd1 = (*sa1++);
			sd2 = (*sa2++);
			MLIB_M_IMAGE_ARITH_U8(dd1, sd1, sd2);
...
/* row loop */
	for (; dh > 0; dh--) {
		...
		if (emask != 0xf) {
			...
			sd1 = (*sa1++);
			sd2 = (*sa2++);
			MLIB_M_IMAGE_ARITH_S16(dd1, sd1, sd2);
...
/* row loop */
	for (; dh > 0; dh--) {
		...
		if (emask != 0xf) {
			...
			sd1 = (*sa1++);
			sd2 = (*sa2++);
			MLIB_M_IMAGE_ARITH_U16(dd1, sd1, sd2);
			mmx_pst_16(dd1, da1, emask);
			n1 += 4;
			da1++;
		}
...
/* row loop */
	for (; dh > 0; dh--) {
		...
		if (emask != 0x3) {
			...
			sd1 = (*sa1++);
			sd2 = (*sa2++);
			MLIB_M_IMAGE_ARITH_S32(dd1, sd1, sd2);
