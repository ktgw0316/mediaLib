...
#pragma ident	"@(#)mlib_m_ImagePolynomialWarp_Fp.c	9.4	07/11/05 SMI"
 ...
 *
 * SYNOPSIS
 *      mlib_status mlib_ImagePolynomialWarp_Fp(mlib_image  *dst,
...
#pragma weak mlib_ImagePolynomialWarp_Fp = __mlib_ImagePolynomialWarp_Fp

#elif defined(__GNUC__)  /* defined(__SUNPRO_C) */

__typeof__(__mlib_ImagePolynomialWarp_Fp) mlib_ImagePolynomialWarp_Fp
    __attribute__((weak, alias("__mlib_ImagePolynomialWarp_Fp")));
...
#define	FREE_MEMORY                                             \
	mlib_free(memory);                                      \
	mlib_free(xCoeffs_new);                                 \
	MLIB_SETCW(fpstatus, MLIB_RC_MASK, fpstatus)

/* *********************************************************** */

mlib_status
__mlib_ImagePolynomialWarp_Fp(
    mlib_image *dst,
    const mlib_image *src,
    const mlib_d64 *xCoeffs,
    const mlib_d64 *yCoeffs,
...
/* check for obvious errors */
	...
	if (n == 0)
		return mlib_ImagePolynomialWarp_0_Fp(dst, src,
		    xCoeffs, yCoeffs,
		    postShiftX, postShiftY, postScaleX,
		    postScaleY, filter, edge);

	if (n == 1)
		return mlib_ImagePolynomialWarp_1(dst, src,
		    xCoeffs, yCoeffs,
		    preShiftX, preShiftY, postShiftX,
		    postShiftY, preScaleX, preScaleY,
		    postScaleX, postScaleY, filter, edge);

	len = mlib_ImageGetWidth(dst) + 1;
	memory = mlib_malloc((sizeof (mlib_s32) * 3 +
	    sizeof (mlib_d64) * 2 +
	...
	border = (filter == MLIB_BILINEAR) ? 0.5 :
	    ((filter == MLIB_BICUBIC) ? 1.5 : 0);
	...
	if (edge == MLIB_EDGE_SRC_PADDED) {
		...
		if (pws.SrcStartX < paddings[0])
			pws.SrcStartX = paddings[0];
		if (pws.SrcStartY < paddings[1])
			pws.SrcStartY = paddings[1];
		if (pws.SrcStopX > (mlib_ImageGetWidth(src) - paddings[2]))
			pws.SrcStopX = mlib_ImageGetWidth(src) - paddings[2];
		if (pws.SrcStopY > (mlib_ImageGetHeight(src) - paddings[3]))
			pws.SrcStopY = mlib_ImageGetHeight(src) - paddings[3];
	}
	...
	MLIB_SETCW(MLIB_RC_ZERO, MLIB_RC_MASK, fpstatus);

	if (n < 6) {
		mlib_ImagePolynomialWarp_2_5_Fp(dst, src,
		    lineAddr, &pws,
	...
	for (i = 0; i < mlib_ImageGetHeight(src); i++) {
		lineAddr[i] = srcData;
