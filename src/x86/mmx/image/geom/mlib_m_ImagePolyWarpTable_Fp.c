...
#pragma ident	"@(#)mlib_m_ImagePolyWarpTable_Fp.c	9.5	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_ImagePolynomialWarpTable_Fp - polynomial-based image warp
 *
 * SYNOPSIS
...
#pragma weak mlib_ImagePolynomialWarpTable_Fp = \
		__mlib_ImagePolynomialWarpTable_Fp

#elif defined(__GNUC__)  /* defined(__SUNPRO_C) */

__typeof__(__mlib_ImagePolynomialWarpTable_Fp) mlib_ImagePolynomialWarpTable_Fp
    __attribute__((weak, alias("__mlib_ImagePolynomialWarpTable_Fp")));
...
/* check for obvious errors */
	...
	type = (itype == MLIB_FLOAT) ? 4 : 8;
	num_func = (itype == MLIB_FLOAT) ? 4 : 5;

	if (itype != MLIB_FLOAT && itype != MLIB_DOUBLE)
		return (MLIB_FAILURE);
	...
	if (n == 0) {
		return mlib_ImagePolynomialWarpTable_0_Fp(dst, src,
	...
	mlib_free(memory);
	mlib_free(xCoeffs_new);
	mlib_free(pws.list_call_function);
	return (MLIB_SUCCESS);
}
