...
#pragma ident	"@(#)mlib_m_ImageAffine_NN_Fp.c	9.2	07/11/05 SMI"
    ...
    mlib_affine_param *param)
{
	...
	if (((srcYStride & 7) == 0) && (srcYStride <= 0x7FFF8)) {
	...
	if (((srcYStride & 7) == 0) && (srcYStride <= 0x7FFF8)) {
		...
		for (j = yStart; j <= yFinish; j++) {
			...
			for (; dstPixelPtr < dstLineEnd; dstPixelPtr += 4) {
				pix0 = srcPixelPtr0[ind0];
				dstPixelPtr[0] = pix0;
				pix0 = srcPixelPtr0[ind0 + 1];
				dstPixelPtr[1] = pix0;
				pix0 = srcPixelPtr0[ind1];
				dstPixelPtr[2] = pix0;
				pix0 = srcPixelPtr0[ind1 + 1];
				dstPixelPtr[3] = pix0;
			...
			if (dstPixelPtr == dstLineEnd) {
				dstPixelPtr[0] = srcPixelPtr0[ind0];
				dstPixelPtr[1] = srcPixelPtr0[ind0 + 1];
			}
		}
	} else {
		for (j = yStart; j <= yFinish; j++) {
	...
	if (srcYStride <= 0xFFFF) {
		...
		for (j = yStart; j <= yFinish; j++) {
			...
			for (; dstPixelPtr < dstLineEnd; dstPixelPtr += 6) {
				...
				pix0 = srcPixelPtr0[ind0 + 1];
				dstPixelPtr[1] = pix0;
				pix0 = srcPixelPtr0[ind0 + 2];
				dstPixelPtr[2] = pix0;
				pix0 = srcPixelPtr0[ind1];
				dstPixelPtr[3] = pix0;
				pix0 = srcPixelPtr0[ind1 + 1];
				dstPixelPtr[4] = pix0;
				pix0 = srcPixelPtr0[ind1 + 2];
				dstPixelPtr[5] = pix0;
    ...
    mlib_affine_param *param)
{
	...
	if (srcYStride <= 0xFFFF) {
		...
		for (j = yStart; j <= yFinish; j++) {
			...
			tmp0 = _mm_madd_pi16(x64, four);
			tmp1 = _mm_madd_pi16(y64, Stride64);
			tmp2 = _mm_add_pi32(tmp0, tmp1);
			ind0 = mmx_read_lo(tmp2);
			ind1 = mmx_read_hi(tmp2);

			for (; dstPixelPtr < dstLineEnd; dstPixelPtr += 8) {
				pix0 = srcPixelPtr0[ind0];
				dstPixelPtr[0] = pix0;
				pix0 = srcPixelPtr0[ind0 + 1];
				dstPixelPtr[1] = pix0;
		...
		for (j = yStart; j <= yFinish; j++) {
			...
			X += dX;
			srcPixelPtr =
			    MLIB_POINTER_GET(lineAddr, ySrc) + 4 * xSrc;
			pix0 = srcPixelPtr[0];
			pix1 = srcPixelPtr[1];
			pix2 = srcPixelPtr[2];
			pix3 = srcPixelPtr[3];
