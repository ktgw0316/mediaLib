...
#pragma ident	"@(#)mlib_m_ImageConv3x3_8nw_2.c	9.2	07/11/05 SMI"
...
#define	GET_KERN()                                                   \
	...
	*(mlib_s16 *)&_rnd = (shift > 0) ? (1 << (shift - 1)) : 0;   \
	_rnd = _mm_unpacklo_pi16(_rnd, _rnd);                        \
	_rnd = _mm_unpacklo_pi16(_rnd, _rnd)
...
#define	PREP_3x3()                                              \
	d0 = d1;                                                \
	UNPACK_SRC(d1, lo);                                     \
	sum0 = _mm_add_pi16(_rnd, _mm_mulhi_pi16(d0, ker1));    \
	sum1 = _mm_add_pi16(_rnd, _mm_mulhi_pi16(d0, ker4));    \
	aa = ALGN(d0, d1, 32);                                  \
	sum0 = _mm_add_pi16(sum0, _mm_mulhi_pi16(aa, ker2));    \
	sum1 = _mm_add_pi16(sum1, _mm_mulhi_pi16(aa, ker5));    \
	sum0 = _mm_add_pi16(sum0, _mm_mulhi_pi16(d1, ker3));    \
	sum1 = _mm_add_pi16(sum1, _mm_mulhi_pi16(d1, ker6));    \
	buff1[i] = sum0;                                        \
	buff2[i] = sum1
...
#define	CONV_3x3(xx, i)                                         \
	...
	sum0 = _mm_add_pi16(sum0, _mm_mulhi_pi16(d1, ker3));    \
	sum1 = _mm_add_pi16(sum1, _mm_mulhi_pi16(d1, ker6));    \
	sum2 = _mm_add_pi16(sum2, _mm_mulhi_pi16(d1, ker9));    \
    ...
    mlib_s32 scalef_expon)
{
	...
	__m64 s0, d0, d1, prev0, prev1, sum0, sum1, sum2, aa, res_hi, res_lo;
	__m64 zero = _m_zero, _rnd;
	mlib_s32 shift;
	mlib_s32 *sp;
