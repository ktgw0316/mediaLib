...
#pragma ident	"@(#)mlib_m_ImageConv5x5_8nw_1.c	9.2	07/11/05 SMI"
...
#define	PREP_5x5()                                                  \
	...
	sum0 = _mm_add_pi16(sum0, _mm_mulhi_pi16(d1, ker[0][4]));   \
	sum1 = _mm_add_pi16(sum1, _mm_mulhi_pi16(d1, ker[1][4]));   \
	sum2 = _mm_add_pi16(sum2, _mm_mulhi_pi16(d1, ker[2][4]));   \
	sum3 = _mm_add_pi16(sum3, _mm_mulhi_pi16(d1, ker[3][4]));   \
	buff0[i] = sum0;                                            \
	buff1[i] = sum1;                                            \
	buff2[i] = sum2;                                            \
	buff3[i] = sum3
...
#define	CONV_5x5(xx, i)                                             \
	...
	sum2 = _mm_add_pi16(sum2, _mm_mulhi_pi16(d1, ker[2][4]));   \
	sum3 = _mm_add_pi16(sum3, _mm_mulhi_pi16(d1, ker[3][4]));   \
	sum4 = _mm_add_pi16(sum4, _mm_mulhi_pi16(d1, ker[4][4]));   \
	prev0 = buff0[i];                                           \
	prev1 = buff1[i];                                           \
	prev2 = buff2[i];                                           \
	prev3 = buff3[i];                                           \
...
#define	CONV_U8_S16(xx)                                         \
	d1 = _mm_unpack##xx##_pi8(s0, zero);                    \
	d1 = _mm_slli_pi16(d1, 7)
    ...
    mlib_s32 scalef_expon)
{
	...
	__m64 ker[5][5];
	__m64 s0, d0, d1, prev0, prev1, prev2, prev3;
