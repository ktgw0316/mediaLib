...
#pragma ident	"@(#)mlib_m_ImageConstXor.c	9.6	07/11/05 SMI"
...
#define	MMX_CONST_LOGIC(d, s, c)	d = _mm_xor_si64(s, c);
    ...
    const mlib_s32 *c)
{
	...
	if (mlib_ImageGetType(src) == MLIB_BIT) {
		return (mlib_ImageConstXor_Bit(dst, src, c));
	}
