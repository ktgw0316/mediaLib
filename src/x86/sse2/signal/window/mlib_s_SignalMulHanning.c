...
#pragma ident	"@(#)mlib_s_SignalMulHanning.c	9.2	07/11/05 SMI"
...
/* check for obvious errors */
	...
	if (CACHE_SIZE > sizeof (mlib_s16) * n) {
		...
		res0 = __mlib_SignalGenHanning_S16(pwin, n);
...
/* check for obvious errors */
	...
	if (CACHE_SIZE > sizeof (mlib_s16) * nn) {
		...
		res0 = mlib_SignalGenHanning_S16_i(pwin - 2 * i0,
			pwin2 + 2 * i0, n, i0, i1);
		if (i1 == n / 4) {
...
#pragma pipeloop(0)
			...
			for (i = 0, j = 0; i <= n2; i++, j += 2) {
				mlib_d64 src0 = src[j], src1 = src[j + 1];
				mlib_d64 src2 = src[e0 - j], src3 = src[e1 - j];
...
__mlib_SignalMulHanning_S16(
    ...
    mlib_s32 n)
{
	return (__mlib_SignalMulHanning_S16_S16(data, data, n));
}
...
__mlib_SignalMulHanning_S16S(
    ...
    mlib_s32 n)
{
	return (__mlib_SignalMulHanning_S16S_S16S(data, data, n));
}
