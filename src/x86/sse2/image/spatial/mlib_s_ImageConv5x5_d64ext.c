...
#pragma ident   "@(#)mlib_s_ImageConv5x5_d64ext.c	9.3	07/11/05 SMI"
...
#define	GET_KERN()                                                    \
	for (j = 0; j < 5; j++) {                                  \
	    for (i = 0; i < 5; i++) {                              \
			ker[j][i] = _mm_set1_pd((mlib_d64)kern[5 * j + i]);\
	    }                                                      \
	}                                                          \
...
#define	PREP_5x5(NCHAN, l)                                       \
	...
	sum0 = _mm_mul_pd(d0, ker[0][0]);                      \
	sum1 = _mm_mul_pd(d0, ker[1][0]);                      \
	sum2 = _mm_mul_pd(d0, ker[2][0]);                      \
	sum3 = _mm_mul_pd(d0, ker[3][0]);                      \
	aa = _mm_loadu_pd((void *)(sbuff + NCHAN));            \
	sum0 = _mm_add_pd(sum0, _mm_mul_pd(aa, ker[0][1]));    \
	sum1 = _mm_add_pd(sum1, _mm_mul_pd(aa, ker[1][1]));    \
	sum2 = _mm_add_pd(sum2, _mm_mul_pd(aa, ker[2][1]));    \
	sum3 = _mm_add_pd(sum3, _mm_mul_pd(aa, ker[3][1]));    \
	bb = _mm_loadu_pd((void *)(sbuff + 2 * NCHAN));        \
	sum0 = _mm_add_pd(sum0, _mm_mul_pd(bb, ker[0][2]));    \
	sum1 = _mm_add_pd(sum1, _mm_mul_pd(bb, ker[1][2]));    \
	sum2 = _mm_add_pd(sum2, _mm_mul_pd(bb, ker[2][2]));    \
	sum3 = _mm_add_pd(sum3, _mm_mul_pd(bb, ker[3][2]));    \
	cc = _mm_loadu_pd((void *)(sbuff + 3 * NCHAN));        \
	sum0 = _mm_add_pd(sum0, _mm_mul_pd(cc, ker[0][3]));    \
	sum1 = _mm_add_pd(sum1, _mm_mul_pd(cc, ker[1][3]));    \
	sum2 = _mm_add_pd(sum2, _mm_mul_pd(cc, ker[2][3]));    \
	sum3 = _mm_add_pd(sum3, _mm_mul_pd(cc, ker[3][3]));    \
	dd = _mm_loadu_pd((void *)(sbuff + 4 * NCHAN));        \
	sum0 = _mm_add_pd(sum0, _mm_mul_pd(dd, ker[0][4]));    \
	sum1 = _mm_add_pd(sum1, _mm_mul_pd(dd, ker[1][4]));    \
	sum2 = _mm_add_pd(sum2, _mm_mul_pd(dd, ker[2][4]));    \
	sum3 = _mm_add_pd(sum3, _mm_mul_pd(dd, ker[3][4]));    \
	_mm_store_pd((mlib_d64 *)(buff0 + l), sum0);           \
	_mm_store_pd((mlib_d64 *)(buff1 + l), sum1);           \
	_mm_store_pd((mlib_d64 *)(buff2 + l), sum2);           \
	_mm_store_pd((mlib_d64 *)(buff3 + l), sum3);           \
	sbuff += 2;                                            \
...
#define	CONV_5x5(NCHAN, l)                                    \
	...
	sum2 = _mm_mul_pd(d0, ker[2][0]);                      \
	sum3 = _mm_mul_pd(d0, ker[3][0]);                      \
	sum4 = _mm_mul_pd(d0, ker[4][0]);                      \
	aa = _mm_loadu_pd((void *)(sbuff + NCHAN));            \
	sum0 = _mm_add_pd(sum0, _mm_mul_pd(aa, ker[0][1]));    \
	sum1 = _mm_add_pd(sum1, _mm_mul_pd(aa, ker[1][1]));    \
	sum2 = _mm_add_pd(sum2, _mm_mul_pd(aa, ker[2][1]));    \
	sum3 = _mm_add_pd(sum3, _mm_mul_pd(aa, ker[3][1]));    \
	sum4 = _mm_add_pd(sum4, _mm_mul_pd(aa, ker[4][1]));    \
	bb = _mm_loadu_pd((void *)(sbuff + 2 * NCHAN));        \
	sum0 = _mm_add_pd(sum0, _mm_mul_pd(bb, ker[0][2]));    \
	sum1 = _mm_add_pd(sum1, _mm_mul_pd(bb, ker[1][2]));    \
	sum2 = _mm_add_pd(sum2, _mm_mul_pd(bb, ker[2][2]));    \
	sum3 = _mm_add_pd(sum3, _mm_mul_pd(bb, ker[3][2]));    \
	sum4 = _mm_add_pd(sum4, _mm_mul_pd(bb, ker[4][2]));    \
	cc = _mm_loadu_pd((void *)(sbuff + 3 * NCHAN));        \
	sum0 = _mm_add_pd(sum0, _mm_mul_pd(cc, ker[0][3]));    \
	sum1 = _mm_add_pd(sum1, _mm_mul_pd(cc, ker[1][3]));    \
	sum2 = _mm_add_pd(sum2, _mm_mul_pd(cc, ker[2][3]));    \
	sum3 = _mm_add_pd(sum3, _mm_mul_pd(cc, ker[3][3]));    \
	sum4 = _mm_add_pd(sum4, _mm_mul_pd(cc, ker[4][3]));    \
	dd = _mm_loadu_pd((void *)(sbuff + 4 * NCHAN));        \
	sum0 = _mm_add_pd(sum0, _mm_mul_pd(dd, ker[0][4]));    \
	sum1 = _mm_add_pd(sum1, _mm_mul_pd(dd, ker[1][4]));    \
	sum2 = _mm_add_pd(sum2, _mm_mul_pd(dd, ker[2][4]));    \
	sum3 = _mm_add_pd(sum3, _mm_mul_pd(dd, ker[3][4]));    \
	sum4 = _mm_add_pd(sum4, _mm_mul_pd(dd, ker[4][4]));    \
	prev0 = _mm_load_pd((mlib_d64 *)(buff0 + l));          \
	prev1 = _mm_load_pd((mlib_d64 *)(buff1 + l));          \
	prev2 = _mm_load_pd((mlib_d64 *)(buff2 + l));          \
	prev3 = _mm_load_pd((mlib_d64 *)(buff3 + l));          \
	sum4 = _mm_add_pd(sum4, prev0);                        \
	sum4 = _mm_add_pd(sum4, prev1);                        \
	sum4 = _mm_add_pd(sum4, prev2);                        \
	sum4 = _mm_add_pd(sum4, prev3);                        \
	_mm_store_pd((mlib_d64 *)(buff0 + l), sum0);           \
	_mm_store_pd((mlib_d64 *)(buff1 + l), sum1);           \
	_mm_store_pd((mlib_d64 *)(buff2 + l), sum2);           \
	_mm_store_pd((mlib_d64 *)(buff3 + l), sum3);           \
