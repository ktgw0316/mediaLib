...
#pragma ident   "@(#)mlib_s_ImageConv5x5Index.c	9.2	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_ImageConv5x5Index - indexed color image convolution
 *
 * SYNOPSIS
 *      mlib_status mlib_ImageConv5x5Index(mlib_image       *dst,
...
#pragma weak mlib_ImageConv5x5Index = __mlib_ImageConv5x5Index

#elif defined(__GNUC__)	/* defined(__SUNPRO_C) */
__typeof__(__mlib_ImageConv5x5Index) mlib_ImageConv5x5Index
    __attribute__((weak, alias("__mlib_ImageConv5x5Index")));
...
static mlib_status mlib_s_conv5x5Index_8_8nw(
    const mlib_image *src,
    mlib_image *dst,
    const mlib_s32 *kern,
    mlib_s32 scale,
...
static mlib_status mlib_s_conv5x5Index_8_16nw(
    const mlib_image *src,
    mlib_image *dst,
    const mlib_s32 *kern,
    mlib_s32 scale,
...
static mlib_status mlib_s_conv5x5Index_8_8ext(
    const mlib_image *src,
    mlib_image *dst,
    const mlib_s32 *kern,
    mlib_s32 scale,
...
static mlib_status mlib_s_conv5x5Index_8_16ext(
    const mlib_image *src,
    mlib_image *dst,
    const mlib_s32 *kern,
    mlib_s32 scale,
    ...
    const void *colormap)
{
	...
	case MLIB_BYTE:
		...
		case MLIB_BYTE:
			...
			case MLIB_EDGE_DST_FILL_ZERO:
				stat =
				    mlib_s_conv5x5Index_8_8nw(src, dst, kernel,
				    scale, colormap);
			...
			case MLIB_EDGE_SRC_EXTEND:
				stat =
				    mlib_s_conv5x5Index_8_8ext(src, dst, kernel,
				    scale, colormap);
				break;
	...
	case MLIB_SHORT:	/* convert wid from pixels to bytes */
		...
		case MLIB_BYTE:
			...
			case MLIB_EDGE_DST_FILL_ZERO:
				stat =
				    mlib_s_conv5x5Index_8_16nw(src, dst, kernel,
				    scale, colormap);
			...
			case MLIB_EDGE_SRC_EXTEND:
				stat =
				    mlib_s_conv5x5Index_8_16ext(src, dst,
    ...
    const void *colormap)
{
	if (mlib_ImageConvVersion(5, 5, scale, MLIB_BYTE) == 0)
		return (mlib_conv5x5Index_8_8nw
		    (src, dst, kern, scale, colormap));

	if (mlib_ImageGetLutChannels(colormap) == 3) {
		return mlib_convMxN_Index3_8_8nw(dst, src, 5, 5, 2, 2, kern,
		    scale, colormap);
	} else {
		return mlib_convMxN_Index4_8_8nw(dst, src, 5, 5, 2, 2, kern,
		    scale, colormap);
	}
}
...
{
	if (mlib_ImageConvVersion(5, 5, scale, MLIB_BYTE) == 0)
		return mlib_conv5x5Index_8_16nw(src, dst, kern, scale,
		    colormap);

	if (mlib_ImageGetLutChannels(colormap) == 3) {
		return mlib_convMxN_Index3_8_16nw(dst, src, 5, 5, 2, 2, kern,
		    scale, colormap);
	} else {
		return mlib_convMxN_Index4_8_16nw(dst, src, 5, 5, 2, 2, kern,
		    scale, colormap);
	}
}
...
{
	if (mlib_ImageConvVersion(5, 5, scale, MLIB_BYTE) == 0)
		return mlib_conv5x5Index_8_8ext(src, dst, kern, scale,
		    colormap);

	if (mlib_ImageGetLutChannels(colormap) == 3) {
		return mlib_convMxN_Index3_8_8ext(dst, src, 5, 5, 2, 2, 2, 2,
		    kern, scale, colormap);
	} else {
		return mlib_convMxN_Index4_8_8ext(dst, src, 5, 5, 2, 2, 2, 2,
		    kern, scale, colormap);
	}
}
