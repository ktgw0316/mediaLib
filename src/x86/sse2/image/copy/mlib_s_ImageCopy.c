...
#pragma ident	"@(#)mlib_s_ImageCopy.c	9.3	07/11/05 SMI"
...
/* size in bytes */
	...
	j = 0;
	if (b_size > 64) {	/* use 128-bit ld/st for large buffers only */
		n = (mlib_addr)da & 15;
		if (n != 0) {
			n = 16 - n;
#ifdef __SUNPRO_C
...
#endif /* __SUNPRO_C */
		for (; j <= (b_size - 8); j += 8) {
			_mm_storel_epi64((void *)da,
					_mm_loadl_epi64((void *)sa));
			da += 8;
			sa += 8;
		}
	} else if ((((mlib_addr)sa & 3) | ((mlib_addr)da & 3)) == 0) {
		mlib_u32 *pws, *pwd;
