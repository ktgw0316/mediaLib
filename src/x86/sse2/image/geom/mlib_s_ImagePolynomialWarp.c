...
#pragma ident   "@(#)mlib_s_ImagePolynomialWarp.c	9.3	07/11/05 SMI"
 ...
 *  The images can be in MLIB_BYTE, MLIB_SHORT, MLIB_USHORT or MLIB_INT data
 *  type.
...
/* check for obvious errors */
	...
	memory = mlib_malloc(sizeof (mlib_s32) * 3 * len +
	    sizeof (mlib_d64) * 2 * mlib_ImageGetWidth(dst) +
	...
	pws.flags = 0;
	pws.filter_table = (void *)mlib_filters_s16_bc;
	pws.filter_table_vis = (void *)mlib_filters_s16_bc_4;

	if (channels == 3) {
		pws.filter_table = (void *)mlib_filters_s16_bc_3;
		pws.filter_table_vis = (void *)mlib_filters_s16_bc_4;
	}

	if (filter == MLIB_BICUBIC2) {
		pws.flags = 1;
		pws.filter_table = (void *)mlib_filters_s16_bc2;
		pws.filter_table_vis = (void *)mlib_filters_s16_bc2_4;
		filter = MLIB_BICUBIC;
		if (channels == 3) {
			pws.filter_table = (void *)mlib_filters_s16_bc2_3;
			pws.filter_table_vis = (void *)mlib_filters_s16_bc2_4;
		}
	}
	...
	for (i = 0; i < mlib_ImageGetHeight(dst); i++) {
		...
		len = func_clip(&pws, y, preShiftX, mlib_ImageGetWidth(dst), n);
