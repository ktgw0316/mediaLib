...
#pragma ident	"@(#)mlib_s_ImageZoom_BC_S32.c	9.2	07/11/05 SMI"
...
#define	STORE(mem, x)	mem = (mlib_s32) x
...
#define	S_LD    _mm_loadu_ps
#define	S_STO   _mm_storeu_si128
#define	S_SET   _mm_set1_ps
#define	S_MUL   _mm_mul_ps
#define	S_ADD   _mm_add_ps
...
#pragma pipeloop(0)
		...
		for (i = 0; i < NCHAN * width - SNUM + 1; i += SNUM) {
			...
			sk00 = S_MUL(sk00, sxf[0]);
			sk10 = S_MUL(sk10, sxf[1]);
			sk20 = S_MUL(sk20, sxf[2]);
			sk30 = S_MUL(sk30, sxf[3]);

			sk00 = S_ADD(sk00, sk10);
			sk20 = S_ADD(sk20, sk30);
			sk00 = S_ADD(sk00, sk20);
			__m128i std = _mm_cvtps_epi32(sk00);
			_mm_storeu_si128((__m128i *)(dl + i), std);
		}
