...
#pragma ident   "@(#)mlib_s_ImageFlipAntiDiag.c	9.6	07/11/05 SMI"
...
/* *********************************************************** */
    ...
    void mlib_s_ImageFlipAntiDiag_U8_1(
    mlib_image *dst,
    const mlib_image *src);
    void mlib_c_ImageFlipAntiDiag_U8_3(
    mlib_image *dst,
    const mlib_image *src);

    void mlib_s_ImageFlipAntiDiag_S16_1(
    mlib_image *dst,
    const mlib_image *src);
    void mlib_c_ImageFlipAntiDiag_S16_3(
    mlib_image *dst,
    const mlib_image *src);
...
/*  handle MLIB_BIT data type of image  */
	case MLIB_BIT:
		...
		case 1:
			return (mlib_ImageFlipMainDiag_BIT_1(dst, src, -1, -1));
		default:
			return (MLIB_FAILURE);
		}
...
/*  handle MLIB_BYTE data type of image  */
	case MLIB_BYTE:
		switch (mlib_ImageGetChannels(src)) {
		case 1:
			mlib_s_ImageFlipAntiDiag_U8_1(dst, src);
			break;
		case 2:
			mlib_s_ImageFlipAntiDiag_S16_1(dst, src);
			break;
		case 3:
			mlib_c_ImageFlipAntiDiag_U8_3(dst, src);
			break;
		case 4:
...
/*  handle MLIB_SHORT data type of image  */
	...
	case MLIB_USHORT:
		...
		case 2:
			...
			break;
		case 3:
			mlib_c_ImageFlipAntiDiag_S16_3(dst, src);
			break;
		case 4:
...
#define	STRIP_1(pd, ps, w, h, data_type)                          \
	{                                                         \
	    mlib_s32 x, y;                                        \
	    mlib_s32 _w = (w), _h = (h);                          \
	    data_type *_ps = (void *)(ps);                        \
	    data_type *_pd = (void *)(pd);                        \
	    data_type *spix = _ps;                                \
	    data_type *dpix = _pd;                                \
	                                                          \
	    for (x = 1; x <= _w; ++x) {                           \
...
#define	BRAK_CLOSE(xpix, ypix, data_type, num_ch)                       \
	if (d_head)                                                     \
		STRIPH_##num_ch(dststart, srcstart,                         \
		(sw - s_tail), d_head, data_type);                          \
	if (s_head)                                                     \
    ...
    const mlib_image *src)
{
	...
	if (sw < 16 || sh < 16) {
		STRIP_1(dststart, srcstart, sw, sh, mlib_u8);
	} else {
		mlib_s32 s_head = 0;
		mlib_s32 d_head = 0;
...
void
mlib_s_ImageFlipAntiDiag_S16_1(
    mlib_image *dst,
    const mlib_image *src)
{
	PREPAREVARS(mlib_u16, 1)
	if (sw < 8 || sh < 8) {
		STRIP_1(dststart, srcstart, sw, sh, mlib_u16);
	} else {
		mlib_s32 s_head = 0;
		mlib_s32 d_head = 0;
...
void
mlib_ImageFlipAntiDiag_S32_1(
    mlib_image *dst,
    const mlib_image *src)
{
	PREPAREVARS(mlib_u32, 1)
	if (sw < 4 || sh < 4) {
		STRIP_1(dststart, srcstart, sw, sh, mlib_u32);
	} else {
		mlib_s32 s_head = 0;
		mlib_s32 d_head = 0;
...
void
mlib_s_ImageFlipAntiDiag_D64_1(
    mlib_image *dst,
    const mlib_image *src)
{
	PREPAREVARS(mlib_d64, 1)
	if (sw < 2 || sh < 2) {
		STRIP_1(dststart, srcstart, sw, sh, mlib_d64);
	} else {
		mlib_s32 s_head = 0;
		mlib_s32 d_head = 0;
...
void
mlib_s_ImageFlipAntiDiag_D64_2(
    mlib_image *dst,
    const mlib_image *src)
{
	PREPAREVARS(mlib_d64, 2)
	    STRIP_SSE2_FP_2(dststart, srcstart, sw, sh);
}

/* *********************************************************** */

void
mlib_s_ImageFlipAntiDiag_D64_3(
    mlib_image *dst,
    const mlib_image *src)
{
	...
	if (numhorzsq <= 0 || numvertsq <= 0) {
		STRIP_3(dststart, srcstart, sw, sh, mlib_d64);
		return;
	}
	for (ysq = 1; ysq <= numhorzsq; ++ysq) {
		mlib_s32 xsq;
...
void
mlib_s_ImageFlipAntiDiag_D64_4(
    mlib_image *dst,
    const mlib_image *src)
{
	PREPAREVARS(mlib_d64, 4)
	    STRIP_SSE2_FP_4(dststart, srcstart, sw, sh);
}
...
#define	BRAK_CLOSE(xpix, ypix, data_type, num_ch)                          \
	if (d_head)                                                     \
		STRIPH_##num_ch(dststart, srcstart, (sw - s_tail), d_head,  \
		data_type);                                                 \
	if (s_head)                                                     \
		STRIP_##num_ch(dststart + num_ch * d_head,                  \
    ...
    const mlib_image *src)
{
	...
	if (!((sstride | dstride) & 3)) {
		...
		if (numhorzsq <= 0 || numvertsq <= 0) {
			STRIP_3(dststart, srcstart, sw, sh, mlib_u8);
			return;
		}
		for (ysq = 1; ysq <= numhorzsq; ++ysq) {
			mlib_s32 xsq;
...
void
mlib_c_ImageFlipAntiDiag_S16_3(
    mlib_image *dst,
    const mlib_image *src)
{
	...
	if (!((sstride | dstride) & 1)) {
		...
		if (numhorzsq <= 0 || numvertsq <= 0) {
			STRIP_3(dststart, srcstart, sw, sh, mlib_u16);
			return;
		}
		for (ysq = 1; ysq <= numhorzsq; ++ysq) {
			mlib_s32 xsq;
...
void
mlib_s_ImageFlipAntiDiag_S32_3(
    mlib_image *dst,
    const mlib_image *src)
{
	...
	if (!((sstride | dstride) & 3)) {
		...
		if (numhorzsq <= 0 || numvertsq <= 0) {
			STRIP_3(dststart, srcstart, sw, sh, mlib_u32);
			return;
		}
		for (ysq = 1; ysq <= numhorzsq; ++ysq) {
			mlib_s32 xsq;
