...
#pragma ident	"@(#)mlib_s_ImageNotXor.c	9.2	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_ImageNotXor   - notXor two images  (C version)
 *
 * SYNOPSIS
 *      mlib_status mlib_ImageNotXor(mlib_image       *dst,
...
#pragma weak mlib_ImageNotXor = __mlib_ImageNotXor

#elif defined(__GNUC__)	/* defined(__SUNPRO_C) */
__typeof__(__mlib_ImageNotXor) mlib_ImageNotXor
    __attribute__((weak, alias("__mlib_ImageNotXor")));
...
#define	OPERATION_SSE2(a1, a2)	_mm_andnot_si128(_mm_xor_si128(a1, a2), ones)
    ...
    const mlib_image *src2)
{
	...
	if ((mlib_ImageGetType(src1) == MLIB_BYTE) ||
		...
		return (mlib_ImageNotXor_Bit(dst, src1, src2));
	} else
		return (MLIB_FAILURE);
}
