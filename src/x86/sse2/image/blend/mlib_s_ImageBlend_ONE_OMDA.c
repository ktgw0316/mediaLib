...
#pragma ident	"@(#)mlib_s_ImageBlend_ONE_OMDA.c	9.2	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_ImageBlend_ONE_OMDA - blend two images with
 *      OMDA source blend factor and ONE destination blend factor
 *
 * SYNOPSIS
 *
 *      mlib_status mlib_ImageBlend_ONE_OMDA(mlib_image       *dst,
...
#pragma weak mlib_ImageBlend_ONE_OMDA = __mlib_ImageBlend_ONE_OMDA

#elif defined(__GNUC__)	/* defined(__SUNPRO_C) */
__typeof__(__mlib_ImageBlend_ONE_OMDA) mlib_ImageBlend_ONE_OMDA
    __attribute__((weak, alias("__mlib_ImageBlend_ONE_OMDA")));
...
{                                                         \
	...
	sdst = _mm_adds_epu16(dalpha1, ssrc1);                \
}
...
{                                                         \
	for (k = i; k < dst_width; k++) {                     \
		...
		a = 256 - psrc1[4 * k + alpha];                   \
                                                          \
		d_s0 += (psrc2[4 * k + 1] * a) >> 8;              \
		d_s1 += (psrc2[4 * k + 2] * a) >> 8;              \
		d_s2 += (psrc2[4 * k + help_alpha] * a) >> 8;     \
                                                          \
