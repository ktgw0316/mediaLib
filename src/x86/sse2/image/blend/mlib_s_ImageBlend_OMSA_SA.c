...
#pragma ident	"@(#)mlib_s_ImageBlend_OMSA_SA.c	9.2	07/11/05 SMI"
...
{                                                         \
	...
	dalpha2 = _mm_sub_epi16(dalpha2, dalpha1);            \
                                                          \
	dalpha3 = _mm_mullo_epi16(dalpha3, ssrc2);            \
	dalpha2 = _mm_adds_epu16(dalpha2, dalpha3);           \
...
{                                                         \
	...
	for (k = i; k < dst_width; k++) {                     \
		...
		d_s3 = psrc1[4 * k + 3];                          \
                                                          \
		d_s0 += (((psrc2[4 * k + 0] - d_s0) * a2) >> 8);  \
		d_s1 += (((psrc2[4 * k + 1] - d_s1) * a2) >> 8);  \
		d_s2 += (((psrc2[4 * k + 2] - d_s2) * a2) >> 8);  \
		d_s3 += (((psrc2[4 * k + 3] - d_s3) * a2) >> 8);  \
                                                          \
