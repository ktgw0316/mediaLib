...
#pragma ident	"@(#)mlib_s_ImageBlend_OMSA_SAS.c	9.2	07/11/05 SMI"
...
{                                                         \
	...
	dalpha1 = _mm_min_epi16(dalpha1, dalpha3);            \
	dalpha1 = _mm_insert_epi16(dalpha1, 0x100, 0);        \
	dalpha1 = _mm_insert_epi16(dalpha1, 0x100, 4);        \
                                                          \
	dalpha1 = _mm_mullo_epi16(dalpha1, ssrc2);            \
                                                          \
	dalpha3 = _mm_xor_si128(dalpha3, df_f);               \
	dalpha3 = _mm_add_epi16(dalpha3, done_one);           \
	dalpha3 = _mm_mullo_epi16(dalpha3, ssrc1);            \
                                                          \
	dalpha3 = _mm_adds_epu16(dalpha1, dalpha3);           \
	sdst = _mm_srli_epi16(dalpha3, 8);                    \
                                                          \
}
...
{                                                         \
	...
	dalpha1 = _mm_add_epi16(dalpha1, done_one);           \
                                                          \
	dalpha1 = _mm_min_epi16(dalpha1, dalpha3);            \
	dalpha1 = _mm_insert_epi16(dalpha1, 0x100, 3);        \
	dalpha1 = _mm_insert_epi16(dalpha1, 0x100, 7);        \
                                                          \
	dalpha1 = _mm_mullo_epi16(dalpha1, ssrc2);            \
                                                          \
	dalpha3 = _mm_xor_si128(dalpha3, df_f);               \
...
{                                                         \
	...
	for (k = i; k < dst_width; k++) {                     \
		src_alpha = psrc2[4 * k + 0];                     \
		dst_alpha = 256 - psrc1[4 * k + 0];               \
		alpha2 = src_alpha;                               \
		...
		d_s3 = psrc1[4 * k + 3];                          \
                                                          \
		d_s0 = (d_s0 * (256 - src_alpha) +                \
				psrc2[4 * k + 0] * 256) >> 8;             \
		d_s1 = (d_s1 * (256 - src_alpha) +                \
				psrc2[4 * k + 1] * min) >> 8;             \
		d_s2 = (d_s2 * (256 - src_alpha) +                \
				psrc2[4 * k + 2] * min) >> 8;             \
		d_s3 = (d_s3 * (256 - src_alpha) +                \
				psrc2[4 * k + 3] * min) >> 8;             \
                                                          \
...
{                                                         \
	...
	mlib_s32 min, alpha2;                                 \
	for (k = i; k < dst_width; k++) {                     \
		src_alpha = psrc2[4 * k + 3];                     \
		dst_alpha = 256 - psrc1[4 * k + 3];               \
		alpha2 = src_alpha;                               \
		min = MIN(src_alpha, dst_alpha);                  \
		d_s0 = psrc1[4 * k + 0];                          \
		d_s1 = psrc1[4 * k + 1];                          \
    ...
    mlib_s32 cmask)
{
	...
	if (channels == 4) {
		mlib_s32 d_s0, d_s1, d_s2, d_s3, a1, a2;
		int k;
		__m128i *px, *py, *pz;
		__m128i dx, dy;
/* upper - 1 lower - 0 */
