...
#pragma ident	"@(#)mlib_s_ImageBlend_SA_OMDA.c	9.2	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_ImageBlend_SA_OMDA - blend two images with
 *      OMDA source blend factor and SA destination blend factor
 *
 * SYNOPSIS
 *
 *      mlib_status mlib_ImageBlend_SA_OMDA(mlib_image       *dst,
...
#pragma weak mlib_ImageBlend_SA_OMDA = __mlib_ImageBlend_SA_OMDA

#elif defined(__GNUC__)	/* defined(__SUNPRO_C) */
__typeof__(__mlib_ImageBlend_SA_OMDA) mlib_ImageBlend_SA_OMDA
    __attribute__((weak, alias("__mlib_ImageBlend_SA_OMDA")));
...
{                                                         \
	...
	for (k = i; k < dst_width; k++) {                     \
		...
		d_s3 = psrc1[4 * k + 3];                          \
                                                          \
		d_s0 = (d_s0 * a2 + psrc2[4 * k + 0] * a1) >> 8;  \
		d_s1 = (d_s1 * a2 + psrc2[4 * k + 1] * a1) >> 8;  \
		d_s2 = (d_s2 * a2 + psrc2[4 * k + 2] * a1) >> 8;  \
		d_s3 = (d_s3 * a2 + psrc2[4 * k + 3] * a1) >> 8;  \
                                                          \
