...
#pragma ident	"@(#)mlib_s_ImageBlend_OMSC_OMDA.c	9.2	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_ImageBlend_OMSC_OMDA - blend two images with
 *      OMDA source blend factor and OMSC destination blend factor
 *
 * SYNOPSIS
 *
 *      mlib_status mlib_ImageBlend_OMSC_OMDA(mlib_image       *dst,
...
#pragma weak mlib_ImageBlend_OMSC_OMDA = __mlib_ImageBlend_OMSC_OMDA

#elif defined(__GNUC__)	/* defined(__SUNPRO_C) */
__typeof__(__mlib_ImageBlend_OMSC_OMDA) mlib_ImageBlend_OMSC_OMDA
    __attribute__((weak, alias("__mlib_ImageBlend_OMSC_OMDA")));
...
{                                                         \
	...
	dalpha1 = _mm_unpacklo_epi64(dalpha1, dalpha2);       \
                                                          \
	dalpha3 = _mm_mullo_epi16(dalpha1, ssrc2);            \
	dalpha2 = _mm_slli_epi16(ssrc2, 8);                   \
	dalpha1 = _mm_sub_epi16(dalpha2, dalpha3);            \
                                                          \
...
{                                                         \
	for (k = i; k < dst_width; k++) {                     \
		...
		a = psrc1[4 * k + alpha];                         \
                                                          \
		d_s0 += psrc2[4 * k + 1] -                        \
			((psrc2[4 * k + 1] * (a + d_s0)) >> 8);       \
		d_s1 += psrc2[4 * k + 2] -                        \
			((psrc2[4 * k + 2] * (a + d_s1)) >> 8);       \
		d_s2 += psrc2[4 * k + help_alpha] -               \
			((psrc2[4 * k + help_alpha] *                 \
			(a + d_s2)) >> 8);                            \
                                                          \
		SAT_U8_U8C(pdst[4 * k + alpha], a +               \
			psrc2[4 * k + alpha] -                        \
			((psrc2[4 * k + alpha] * (a << 1)) >> 8));    \
