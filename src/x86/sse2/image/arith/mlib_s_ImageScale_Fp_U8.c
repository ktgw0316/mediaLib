...
#pragma ident "@(#)mlib_s_ImageScale_Fp_U8.c	9.3	07/11/05 SMI"

/*
 * FUNCTION
 *	mlib_s_ImageScale_Fp_u8_f32	- linear scaling
 *	mlib_s_ImageScale_Fp_u8_d64	- linear scaling
 *
 * SYNOPSIS
 *	void mlib_s_ImageScale_Fp_u8_f32(
 *				mlib_u8 *dst,
 *				mlib_s32 dlb,
 *				const mlib_f32 *src,
 *				mlib_s32 slb,
 ...
 *				const mlib_d64 *beta);
 *	void mlib_s_ImageScale_Fp_u8_d64(
 *				mlib_u8 *dst,
 *				mlib_s32 dlb,
 *				const mlib_d64 *src,
 *				mlib_s32 slb,
...
#define	MLIB_C_IMAGESCALE_U8_F32(d, s, a, b)			\
	{							\
		mlib_f32 t;					\
		t = ((s) * (a) + (b));				\
		if (t >= MLIB_U8_MAX) {				\
...
#define	MLIB_S_IMAGESCALE_U8_F32(D_STORE, S_LOAD)		\
	{							\
		__m128 dsts, srcs;				\
		__m128i s32sh, s32sl, s16sh, s16sl, s8s;	\
		...
		s16sl = _mm_packs_epi32(s32sl, s32sh);		\
		...
		dsts = _mm_max_ps(dsts, dmin);			\
		s32sh = _mm_cvtps_epi32(dsts);			\
		s16sh = _mm_packs_epi32(s32sl, s32sh);		\
		s8s = _mm_packus_epi16(s16sl, s16sh);		\
		D_STORE((void *)&(dst[i]), s8s);		\
	}
...
#define	MLIB_S_IMAGESCALE3_F32_F32(D_STORE, S_LOAD)		\
	{							\
		...
		s32sh = _mm_cvtps_epi32(dsts);			\
		s16sl = _mm_packs_epi32(s32sl, s32sh);		\
		srcs = S_LOAD((void *)&(src[i + 24]));		\
		dsts = _mm_mul_ps(alphas, srcs);		\
		dsts = _mm_add_ps(betas, dsts);			\
		dsts = _mm_min_ps(dsts, dmax);			\
		dsts = _mm_max_ps(dsts, dmin);			\
		s32sl = _mm_cvtps_epi32(dsts);			\
		srcs = S_LOAD((void *)&(src[i + 28]));		\
		dsts = _mm_mul_ps(alphau, srcs);		\
		dsts = _mm_add_ps(betau, dsts);			\
		dsts = _mm_min_ps(dsts, dmax);			\
		dsts = _mm_max_ps(dsts, dmin);			\
		...
		s16sh = _mm_packs_epi32(s32sl, s32sh);		\
		s8s = _mm_packus_epi16(s16sl, s16sh);		\
		D_STORE((void *)&(dst[i + 16]), s8s);		\
		srcs = S_LOAD((void *)&(src[i + 32]));		\
		dsts = _mm_mul_ps(alphav, srcs);		\
		dsts = _mm_add_ps(betav, dsts);			\
		dsts = _mm_min_ps(dsts, dmax);			\
		dsts = _mm_max_ps(dsts, dmin);			\
		s32sl = _mm_cvtps_epi32(dsts);			\
		srcs = S_LOAD((void *)&(src[i + 36]));		\
		dsts = _mm_mul_ps(alphas, srcs);		\
		dsts = _mm_add_ps(betas, dsts);			\
		dsts = _mm_min_ps(dsts, dmax);			\
		dsts = _mm_max_ps(dsts, dmin);			\
		s32sh = _mm_cvtps_epi32(dsts);			\
		s16sl = _mm_packs_epi32(s32sl, s32sh);		\
		srcs = S_LOAD((void *)&(src[i + 40]));		\
		dsts = _mm_mul_ps(alphau, srcs);		\
		dsts = _mm_add_ps(betau, dsts);			\
		dsts = _mm_min_ps(dsts, dmax);			\
		dsts = _mm_max_ps(dsts, dmin);			\
		s32sl = _mm_cvtps_epi32(dsts);			\
		srcs = S_LOAD((void *)&(src[i + 44]));		\
		dsts = _mm_mul_ps(alphav, srcs);		\
		dsts = _mm_add_ps(betav, dsts);			\
		dsts = _mm_min_ps(dsts, dmax);			\
		dsts = _mm_max_ps(dsts, dmin);			\
	...
	const mlib_d64 *beta)
{
	...
	dmax = _mm_set1_ps((mlib_f32)MLIB_U8_MAX);
	dmin = _mm_set1_ps((mlib_f32)MLIB_U8_MIN);
...
#pragma pipeloop(0)
				...
				for (i = 0; i <= (nsize - 16); i += 16) {
					MLIB_S_IMAGESCALE_U8_F32(
						_mm_store_si128,
						_mm_load_ps);
				}
				for (; i < nsize; i ++) {
					MLIB_C_IMAGESCALE_U8_F32(
						dst[i], src[i],
						alpha0, beta0);
				}
...
#define	MLIB_C_IMAGESCALE_U8_D64(d, s, a, b)			\
	{							\
		mlib_d64 t;					\
		t = ((s) * (a) + (b));				\
		if (t >= MLIB_U8_MAX) {			\
...
#define	MLIB_S_IMAGESCALE_U8_D64(D_STORE, S_LOAD)		\
	{							\
		__m128d dsts, srcs;				\
		__m128i s32s0, s32s1;				\
		__m128i s32sh, s32sl, s16sh, s16sl, s8s;	\
	...
	const mlib_d64 *beta)
{
	...
	dmax = _mm_set1_pd((mlib_d64)MLIB_U8_MAX);
	dmin = _mm_set1_pd((mlib_d64)MLIB_U8_MIN);
...
#endif /* __SUNPRO_C */
				for (i = 0; i <= (nsize - 16); i += 16) {
					MLIB_S_IMAGESCALE_U8_D64(
						_mm_store_si128,
						_mm_load_pd);
				}
				for (; i < nsize; i ++) {
					MLIB_C_IMAGESCALE_U8_D64(
						dst[i], src[i],
						alpha0, beta0);
				}
