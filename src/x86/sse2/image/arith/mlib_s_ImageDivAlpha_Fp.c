...
#pragma ident	"@(#)mlib_s_ImageDivAlpha_Fp.c	9.2	07/11/05 SMI"
...
#pragma pipeloop(0)
		...
		for (i = 0; i < height; i++) {
			for (j = 0; j < width - 9; j += 12) {
				...
				xdst1 = _mm_div_ps(xsrc1, xalp1);
				xdst2 = _mm_div_ps(xsrc2, xalp2);
				xdst3 = _mm_div_ps(xsrc3, xalp3);
				_mm_storeu_ps((ptr_dst + j), xdst1);
...
#pragma pipeloop(0)
		...
		for (i = 0; i < height; i++) {
			for (j = 0; j < width; j += 4) {
				...
				xtrc1 = _mm_div_pd(xsrc2, xsrc1);
				xtrc2 = _mm_div_pd(xsrc3, xsrc1);
