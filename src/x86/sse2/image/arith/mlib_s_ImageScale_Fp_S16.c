...
#pragma ident "@(#)mlib_s_ImageScale_Fp_S16.c	9.3	07/11/05 SMI"

/*
 * FUNCTION
 *	mlib_s_ImageScale_Fp_s16_f32	- linear scaling
 *	mlib_s_ImageScale_Fp_s16_d64	- linear scaling
 *
 * SYNOPSIS
 *	void mlib_s_ImageScale_Fp_s16_f32(
 *				mlib_s16 *dst,
 *				mlib_s32 dlb,
 *				const mlib_f32 *src,
 *				mlib_s32 slb,
 ...
 *				const mlib_d64 *beta);
 *	void mlib_s_ImageScale_Fp_s16_d64(
 *				mlib_s16 *dst,
 *				mlib_s32 dlb,
 *				const mlib_d64 *src,
 *				mlib_s32 slb,
...
#define	MLIB_C_IMAGESCALE_S16_F32(d, s, a, b)			\
	{							\
		mlib_f32 t;					\
		t = ((s) * (a) + (b));				\
		if (t >= MLIB_S16_MAX) {			\
...
#define	MLIB_S_IMAGESCALE_S16_F32(D_STORE, S_LOAD)		\
	{							\
		__m128 dsts, srcs;				\
	...
	const mlib_d64 *beta)
{
	...
	dmax = _mm_set1_ps((mlib_f32)MLIB_S16_MAX);
	dmin = _mm_set1_ps((mlib_f32)MLIB_S16_MIN);
...
#pragma pipeloop(0)
				...
				for (i = 0; i <= (nsize - 8); i += 8) {
					MLIB_S_IMAGESCALE_S16_F32(
						_mm_store_si128,
						_mm_load_ps);
				}
				for (; i < nsize; i ++) {
					MLIB_C_IMAGESCALE_S16_F32(
						dst[i], src[i],
						alpha0, beta0);
				}
...
#define	MLIB_C_IMAGESCALE_S16_D64(d, s, a, b)			\
	{							\
		mlib_d64 t;					\
		t = ((s) * (a) + (b));				\
		if (t >= MLIB_S16_MAX) {			\
...
#define	MLIB_S_IMAGESCALE_S16_D64(D_STORE, S_LOAD)		\
	{							\
		__m128d dsts, srcs;				\
		__m128i s32s0, s32s1;				\
		__m128i s32sh, s32sl, s16s;			\
	...
	const mlib_d64 *beta)
{
	...
	dmax = _mm_set1_pd((mlib_d64)MLIB_S16_MAX);
	dmin = _mm_set1_pd((mlib_d64)MLIB_S16_MIN);
...
#endif /* __SUNPRO_C */
				for (i = 0; i <= (nsize - 8); i += 8) {
					MLIB_S_IMAGESCALE_S16_D64(
						_mm_store_si128,
						_mm_load_pd);
				}
				for (; i < nsize; i ++) {
					MLIB_C_IMAGESCALE_S16_D64(
						dst[i], src[i],
						alpha0, beta0);
				}
