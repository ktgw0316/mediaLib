...
#pragma ident "@(#)mlib_s_ImageBlend_Fp_SA_F32.c	9.3	07/11/05 SMI"

/*
 * FUNCTION
 *	mlib_s_ImageBlendSA_Fp_f32	- blend two images
 *
 * SYNOPSIS
 *	mlib_s_ImageBlendSA_Fp_f32(mlib_f32 *dst,
 *				mlib_s32 dlb,
 *				const mlib_f32 *src1,
 *				mlib_s32 slb1,
 *				const mlib_f32 *src2,
...
#define	MLIB_S_IMAGEBLENDSA2_F32(D_STORE, S1_LOAD, S2_LOAD)	\
	{							\
		__m128 dsts, srcs1, srcs2;			\
		__m128 alphas, betas, part1, part2;		\
		mlib_f32 alpha0, alpha1;			\
		alpha0 = alpha[i];				\
		alpha1 = alpha[i + 1];				\
		alphas = _mm_set_ps(alpha1, alpha1,		\
					alpha0, alpha0);	\
...
#define	MLIB_S_IMAGEBLENDSA3_F32(D_STORE, S1_LOAD, S2_LOAD)	\
	{							\
		__m128 dsts, srcs1, srcs2;			\
		...
		alpha2 = alpha[i + 2];				\
		alpha3 = alpha[i + 3];				\
		alphas = _mm_set_ps(alpha1, alpha0,		\
					alpha0, alpha0);	\
		alphau = _mm_set_ps(alpha2, alpha2,		\
					alpha1, alpha1);	\
		alphav = _mm_set_ps(alpha3, alpha3,		\
					alpha3, alpha2);	\
		srcs1  = S1_LOAD((void *)&(src1[k]));		\
		srcs2  = S2_LOAD((void *)&(src2[k]));		\
		betas  = _mm_sub_ps(ones, alphas);		\
		...
		betas  = _mm_sub_ps(ones, alphau);		\
		part1  = _mm_mul_ps(alphau, srcs1);		\
		part2  = _mm_mul_ps(betas, srcs2);		\
		dsts   = _mm_add_ps(part1, part2);		\
		D_STORE((void *)&(dst[k + 4]), dsts);		\
		srcs1  = S1_LOAD((void *)&(src1[k + 8]));	\
		srcs2  = S2_LOAD((void *)&(src2[k + 8]));	\
		betas  = _mm_sub_ps(ones, alphav);		\
		part1  = _mm_mul_ps(alphav, srcs1);		\
		part2  = _mm_mul_ps(betas, srcs2);		\
		dsts   = _mm_add_ps(part1, part2);		\
		D_STORE((void *)&(dst[k + 8]), dsts);		\
	}

#define	MLIB_S_IMAGEBLENDSA4_F32(D_STORE, S1_LOAD, S2_LOAD)	\
	{							\
		__m128 dsts, srcs1, srcs2, alphas;		\
		__m128 betas, part1, part2;			\
		alphas = _mm_set1_ps(alpha[i]);			\
		srcs1  = S1_LOAD((void *)&(src1[k]));		\
		srcs2  = S2_LOAD((void *)&(src2[k]));		\
		betas  = _mm_sub_ps(ones, alphas);		\
		part1  = _mm_mul_ps(alphas, srcs1);		\
