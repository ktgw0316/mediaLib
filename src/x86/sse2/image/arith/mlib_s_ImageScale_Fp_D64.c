...
#pragma ident "@(#)mlib_s_ImageScale_Fp_D64.c	9.3	07/11/05 SMI"

/*
 * FUNCTION
 *	mlib_s_ImageScale_Fp_d64_f32	- linear scaling
 *	mlib_s_ImageScale_Fp_d64_d64	- linear scaling
 *
 * SYNOPSIS
 *	void mlib_s_ImageScale_Fp_d64_f32(
 *				mlib_d64 *dst,
 *				mlib_s32 dlb,
 *				const mlib_f32 *src,
 *				mlib_s32 slb,
 ...
 *				const mlib_d64 *beta);
 *	void mlib_s_ImageScale_Fp_d64_d64(
 *				mlib_d64 *dst,
 *				mlib_s32 dlb,
 *				const mlib_d64 *src,
 *				mlib_s32 slb,
...
#define	MLIB_C_IMAGESCALE_D64_F32(s, a, b)	((s) * (a) + (b))

#define	MLIB_S_IMAGESCALE_D64_F32(D_STORE, S_LOAD)		\
	{							\
		__m128 dsts, srcs;				\
		__m128d d64s;					\
		...
		d64s = _mm_cvtps_pd(dsts);			\
		D_STORE((void *)&(dst[i]), d64s);		\
		dsts = _mm_movehl_ps(dsts, dsts);		\
		d64s = _mm_cvtps_pd(dsts);			\
		D_STORE((void *)&(dst[i + 2]), d64s);		\
	}

#define	MLIB_S_IMAGESCALE3_D64_F32(D_STORE, S_LOAD)		\
	{							\
		__m128 dsts, srcs;				\
		__m128d d64s;					\
		srcs = S_LOAD((void *)&(src[i]));		\
		...
		d64s = _mm_cvtps_pd(dsts);			\
		D_STORE((void *)&(dst[i + 4]), d64s);		\
		dsts = _mm_movehl_ps(dsts, dsts);		\
		d64s = _mm_cvtps_pd(dsts);			\
		D_STORE((void *)&(dst[i + 6]), d64s);		\
		srcs = S_LOAD((void *)&(src[i + 8]));		\
		dsts = _mm_mul_ps(alphav, srcs);		\
		dsts = _mm_add_ps(betav, dsts);			\
		d64s = _mm_cvtps_pd(dsts);			\
		D_STORE((void *)&(dst[i + 8]), d64s);		\
		dsts = _mm_movehl_ps(dsts, dsts);		\
		d64s = _mm_cvtps_pd(dsts);			\
		D_STORE((void *)&(dst[i + 10]), d64s);		\
	}
...
#pragma pipeloop(0)
				...
				for (i = 0; i <= (nsize - 4); i += 4) {
					MLIB_S_IMAGESCALE_D64_F32(
						_mm_store_pd,
						_mm_load_ps);
				}
				for (; i < nsize; i ++) {
					dst[i] = MLIB_C_IMAGESCALE_D64_F32(
						src[i], alpha0, beta0);
				}
...
#pragma pipeloop(0)
				...
				for (; i < nsize; i += 2) {
					dst[i] = MLIB_C_IMAGESCALE_D64_F32(
						src[i], alpha0, beta0);
					dst[i + 1] = MLIB_C_IMAGESCALE_D64_F32(
...
#pragma pipeloop(0)
				...
				for (; i < nsize; i += 3) {
					...
					dst[i + 1] = MLIB_C_IMAGESCALE_D64_F32(
						src[i + 1], alpha1, beta1);
					dst[i + 2] = MLIB_C_IMAGESCALE_D64_F32(
...
#pragma pipeloop(0)
				...
				for (; i < nsize; i += 4) {
					...
					dst[i + 2] = MLIB_C_IMAGESCALE_D64_F32(
						src[i + 2], alpha2, beta2);
					dst[i + 3] = MLIB_C_IMAGESCALE_D64_F32(
...
#define	MLIB_C_IMAGESCALE_D64_D64(s, a, b)	((s) * (a) + (b))

#define	MLIB_S_IMAGESCALE_D64_D64(D_STORE, S_LOAD)		\
	{							\
		__m128d dsts, srcs;				\
		srcs = S_LOAD((void *)&(src[i]));		\
...
#define	MLIB_S_IMAGESCALE3_D64_D64(D_STORE, S_LOAD)		\
	{							\
		__m128d dsts, srcs;				\
		srcs = S_LOAD((void *)&(src[i]));		\
		dsts = _mm_mul_pd(alphas, srcs);		\
...
#define	MLIB_S_IMAGESCALE4_D64_D64(D_STORE, S_LOAD)		\
	{							\
		__m128d dsts, srcs;				\
		srcs = S_LOAD((void *)&(src[i]));		\
		dsts = _mm_mul_pd(alphas, srcs);		\
...
#pragma pipeloop(0)
				...
				for (i = 0; i <= (nsize - 2); i += 2) {
					MLIB_S_IMAGESCALE_D64_D64(
						_mm_store_pd,
						_mm_load_pd);
				}
				for (; i < nsize; i ++) {
					dst[i] = MLIB_C_IMAGESCALE_D64_D64(
						src[i], alpha0, beta0);
				}
...
#pragma pipeloop(0)
				...
				for (; i < nsize; i += 2) {
					dst[i] = MLIB_C_IMAGESCALE_D64_D64(
						src[i], alpha0, beta0);
					dst[i + 1] = MLIB_C_IMAGESCALE_D64_D64(
						src[i + 1], alpha1, beta1);
				}
				src  = (mlib_d64 *)((mlib_u8 *)src + slb);
				dst  = (mlib_d64 *)((mlib_u8 *)dst + dlb);
...
#pragma pipeloop(0)
				...
				for (; i < nsize; i += 3) {
					...
					dst[i + 1] = MLIB_C_IMAGESCALE_D64_D64(
						src[i + 1], alpha1, beta1);
					dst[i + 2] = MLIB_C_IMAGESCALE_D64_D64(
						src[i + 2], alpha2, beta2);
				}
				src  = (mlib_d64 *)((mlib_u8 *)src + slb);
				dst  = (mlib_d64 *)((mlib_u8 *)dst + dlb);
...
#pragma pipeloop(0)
				...
				for (; i < nsize; i += 4) {
					...
					dst[i + 2] = MLIB_C_IMAGESCALE_D64_D64(
						src[i + 2], alpha2, beta2);
					dst[i + 3] = MLIB_C_IMAGESCALE_D64_D64(
						src[i + 3], alpha3, beta3);
				}
				src  = (mlib_d64 *)((mlib_u8 *)src + slb);
				dst  = (mlib_d64 *)((mlib_u8 *)dst + dlb);
