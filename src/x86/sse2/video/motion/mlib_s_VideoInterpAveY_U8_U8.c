...
#pragma ident	"@(#)mlib_s_VideoInterpAveY_U8_U8.c	9.2	07/11/05 SMI"
...
#define	MLIB_S_VIDEOINTERPAVG8(height)				\
	for (y = 0; y < height; y++) {                          \
		...
		txmm4 = _mm_avg_epu8(txmm3, txmm0);		     \
		_mm_storel_epi64((__m128i *)dd, txmm4);		     \
...
#define	MLIB_S_VIDEOINTERPAVG16(height)				\
	for (y = 0; y < height; y++) {                          \
		...
		txmm3 = _mm_avg_epu8(txmm1, txmm2);		     \
		txmm4 = _mm_avg_epu8(txmm3, txmm0);		     \
		_mm_storeu_si128((__m128i *)dd, txmm4);		     \
		sl = sd = sl + field_stride;                         \
		dl = dd = dl + field_stride;                         \
	}
