...
#pragma ident	"@(#)mlib_s_VideoColorJFIFYCC2ABGR444.c	9.6	07/11/05 SMI"
...
/* *********************************************************** */
mlib_status
mlib_VideoColorJFIFYCC2ABGR444_aligned(
	mlib_u8 *abgr,
	const mlib_u8 *y,
	const mlib_u8 *cb,
	const mlib_u8 *cr,
	...
	mlib_s32 n)
{
	...
	if (0 == (0xf & ((mlib_addr)abgr | (mlib_addr)y |
		...
		return mlib_VideoColorJFIFYCC2ABGR444_aligned(
			abgr, y, cb, cr, n);
	} else {
		return mlib_VideoColorJFIFYCC2ABGR444_naligned(
			abgr, y, cb, cr, n);
	}
}
...
#pragma pipeloop(0)
	...
	for (; i <= n - 16; i += 16)	{
		...
		_mm_store_si128(px_abgr++, x_abgrl);

		x_abgrh = _mm_unpackhi_epi16(x_abl, x_grl);
		_mm_store_si128(px_abgr++, x_abgrh);
