...
#pragma ident	"@(#)mlib_s_VideoColorYUV2RGB411.c	9.6	07/11/05 SMI"
...
static mlib_status
mlib_VideoColorYUV2RGB411_align(
	mlib_u8 *rgb,
	const mlib_u8 *y,
	const mlib_u8 *u,
	const mlib_u8 *v,
...
static mlib_status
mlib_VideoColorYUV2RGB411_nalign(
	mlib_u8 *rgb,
	const mlib_u8 *y,
	const mlib_u8 *u,
	const mlib_u8 *v,
	...
	mlib_s32 uv_stride)
{
	...
	if ((((mlib_addr)y & 0xf) == 0) &&
		(((y_stride & 0xf) == 0) || (height == 1))) {
		return mlib_VideoColorYUV2RGB411_align(
			rgb, y, u, v, width, height, rgb_stride,
			y_stride, uv_stride);
	} else {
		return mlib_VideoColorYUV2RGB411_nalign(rgb, y, u, v,
...
#pragma pipeloop(0)
		...
		for (; iW <= width - 32; iW += 32) {
			...
			drgl = _mm_unpacklo_epi8(dr, dg);
			drgh = _mm_unpackhi_epi8(dr, dg);
			dbzl = _mm_unpacklo_epi8(db, db);
			dbzh = _mm_unpackhi_epi8(db, db);
			...
			/* create rgb sequences : db1/dr1/dg1 */
