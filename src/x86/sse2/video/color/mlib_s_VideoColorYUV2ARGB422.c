...
#pragma ident	"@(#)mlib_s_VideoColorYUV2ARGB422.c	9.4	07/11/05 SMI"
...
mlib_VideoColorYUV2ARGB422_aligned(
	...
	mlib_s32 uv_stride);
mlib_status
mlib_VideoColorYUV2ARGB422_naligned(
	mlib_u8 *argb,
	const mlib_u8 *y,
	const mlib_u8 *u,
	const mlib_u8 *v,
	...
	mlib_s32 uv_stride)
{
	...
	if (((((mlib_addr)y | (mlib_addr)u | (mlib_addr)v |
		(mlib_addr)argb) & 0xf) == 0) &&
		((((y_stride | uv_stride | argb_stride) & 0xf) == 0) ||
		(height == 1))) {
		return mlib_VideoColorYUV2ARGB422_aligned(argb, y, u, v,
			width, height, argb_stride, y_stride, uv_stride);
	} else {
		return mlib_VideoColorYUV2ARGB422_naligned(argb, y, u, v,
			width, height, argb_stride, y_stride, uv_stride);
	}
}
