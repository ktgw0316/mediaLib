...
#pragma ident	"@(#)mlib_s_VideoColorARGB2JFIFYCC420.c	9.6	07/11/05 SMI"
...
/* *********************************************************** */
mlib_status
mlib_VideoColorARGB2JFIFYCC420_aligned(
	mlib_u8 *y0,
	mlib_u8 *y1,
	mlib_u8 *cb,
	mlib_u8 *cr,
...
mlib_status
mlib_VideoColorARGB2JFIFYCC420_naligned(
	mlib_u8 *y0,
	mlib_u8 *y1,
	mlib_u8 *cb,
	mlib_u8 *cr,
...
__mlib_VideoColorARGB2JFIFYCC420(
	...
	mlib_s32 n)
{
	if (argb0 == NULL || argb1 == NULL || y0 == NULL || y1 == NULL ||
		cb == NULL || cr == NULL)
	...
	if (0 == (0xf & ((mlib_addr)argb0 | (mlib_addr)argb1 |
			(mlib_addr)y0 | (mlib_addr)y1 |
			(mlib_addr)cb | (mlib_addr)cr))) {
			return mlib_VideoColorARGB2JFIFYCC420_aligned(
				y0, y1, cb, cr, argb0, argb1, n);
	} else {
			return mlib_VideoColorARGB2JFIFYCC420_naligned(
				y0, y1, cb, cr, argb0, argb1, n);
	}
}
	...
	mlib_s32 n)
{
	...
	__m128i *pargb0, *pargb1, *py0, *py1, *pcb, *pcr;

	pargb0 = (__m128i*)argb0;
	pargb1 = (__m128i*)argb1;
	py0 = (__m128i*)y0;
	py1 = (__m128i*)y1;
	pcb = (__m128i*)cb;
	pcr = (__m128i*)cr;
...
#pragma pipeloop(0)
#endif /* __SUNPRO_C */
	for (; i <= size - 64; i += 64)	{
		x_argb0 = _mm_load_si128(pargb0++);
		x_argb1 = _mm_load_si128(pargb0++);
		x_argb2 = _mm_load_si128(pargb0++);
		x_argb3 = _mm_load_si128(pargb0++);
		...
		/* load */
		x_argb0 = _mm_load_si128(pargb1++);
		x_argb1 = _mm_load_si128(pargb1++);
		x_argb2 = _mm_load_si128(pargb1++);
		x_argb3 = _mm_load_si128(pargb1++);
	...
	if (i <= size - 8) {
		/* 1st r/g/b */
		iTemp = *((mlib_s32 *)pargb0);
		ir1 = (iTemp >> 8) & 0xff;
		ig1 = (iTemp >> 16) & 0xff;
		ib1 = (iTemp >> 24) & 0xff;
		pargb0 = (__m128i*) (((mlib_u8 *)pargb0) + 4);
		...
		/* 1st r/g/b */
		iTemp = *((mlib_s32 *)pargb1);
		ir1 = (iTemp >> 8) & 0xff;
		ig1 = (iTemp >> 16) & 0xff;
		ib1 = (iTemp >> 24) & 0xff;
		pargb1 = (__m128i*) (((mlib_u8 *)pargb1) + 4);
...
#endif /* __SUNPRO_C */
	for (; i <= size - 64; i += 64)	{
		x_argb0 = _mm_loadu_si128(pargb0++);
		x_argb1 = _mm_loadu_si128(pargb0++);
		x_argb2 = _mm_loadu_si128(pargb0++);
		x_argb3 = _mm_loadu_si128(pargb0++);
		...
		/* load */
		x_argb0 = _mm_loadu_si128(pargb1++);
		x_argb1 = _mm_loadu_si128(pargb1++);
		x_argb2 = _mm_loadu_si128(pargb1++);
		x_argb3 = _mm_loadu_si128(pargb1++);
