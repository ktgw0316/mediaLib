...
#pragma ident	"@(#)mlib_s_VideoColorYUV2RGB422.c	9.5	07/11/05 SMI"
...
/* *********************************************************** */
mlib_status
mlib_VideoColorYUV2RGB422_aligned(
	mlib_u8 *rgb,
	const mlib_u8 *y,
	const mlib_u8 *u,
	const mlib_u8 *v,
...
mlib_status
mlib_VideoColorYUV2RGB422_naligned(
	mlib_u8 *rgb,
	const mlib_u8 *y,
	const mlib_u8 *u,
	const mlib_u8 *v,
	...
	mlib_s32 uv_stride)
{
	...
	if (((((mlib_addr)y | (mlib_addr)u | (mlib_addr)v) & 0xf) == 0) &&
		((((y_stride | uv_stride) & 0xf) == 0)	|| (height == 1))) {
		return mlib_VideoColorYUV2RGB422_aligned(rgb, y, u, v,
				width, height, rgb_stride, y_stride, uv_stride);
	} else {
		return mlib_VideoColorYUV2RGB422_naligned(rgb, y, u, v,
		width, height, rgb_stride, y_stride, uv_stride);
	}
}
...
/* variables */
	...
	__m128i ddy1, ddy2, ddy3, ddy4, drgl, drgh, dbzl, dbzh, drgbh, drgbl;
...
#pragma pipeloop(0)
		...
		for (; iW <= width - 32; iW += 32) {
			...
			drgl = _mm_unpacklo_epi8(dr1, dg1);
			drgh = _mm_unpackhi_epi8(dr1, dg1);
			dbzl = _mm_unpacklo_epi8(db1, db1);
			dbzh = _mm_unpackhi_epi8(db1, db1);

			drgbl = _mm_unpacklo_epi16(drgl, dbzl);
			PACK_RGB1(drgbl);

			drgbh = _mm_unpackhi_epi16(drgl, dbzl);
			PACK_RGB1(drgbh);

			drgbl = _mm_unpacklo_epi16(drgh, dbzh);
			PACK_RGB1(drgbl);

			drgbh = _mm_unpackhi_epi16(drgh, dbzh);
			PACK_RGB1(drgbh);

			/* create rgb sequences : db/dr/dg[2] */
			drgl = _mm_unpacklo_epi8(dr2, dg2);
			drgh = _mm_unpackhi_epi8(dr2, dg2);
			dbzl = _mm_unpacklo_epi8(db2, db2);
			dbzh = _mm_unpackhi_epi8(db2, db2);
