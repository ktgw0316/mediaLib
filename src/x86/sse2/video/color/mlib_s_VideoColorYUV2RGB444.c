...
#pragma ident	"@(#)mlib_s_VideoColorYUV2RGB444.c	9.6	07/11/05 SMI"
...
mlib_status
mlib_VideoColorYUV2RGB444_aligned(
	mlib_u8 *rgb,
	const mlib_u8 *y,
	const mlib_u8 *u,
	const mlib_u8 *v,
...
mlib_status
mlib_VideoColorYUV2RGB444_naligned(
	mlib_u8 *rgb,
	const mlib_u8 *y,
	const mlib_u8 *u,
	const mlib_u8 *v,
	...
	mlib_s32 yuv_stride)
{
	...
	if (((((mlib_addr)y | (mlib_addr)u | (mlib_addr)v) &
		0xf) == 0) &&
		(((yuv_stride & 0xf) == 0) || (ysize == 1))) {
		return mlib_VideoColorYUV2RGB444_aligned(
			rgb, y, u, v, xsize, ysize, rgb_stride, yuv_stride);
	} else {
		return mlib_VideoColorYUV2RGB444_naligned(
			rgb, y, u, v, xsize, ysize, rgb_stride, yuv_stride);
	}
}
...
/* -222.9952 * 32 */
	...
	__m128i *px_y, *px_u, *px_v;
	mlib_u8 *pY, *pU, *pV, *pD, *pd;
