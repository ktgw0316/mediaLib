...
#pragma ident	"@(#)mlib_s_VideoColorYUV2ARGB420.c	9.5	07/11/05 SMI"
 ...
 * SYNOPSIS
 *
 *      mlib_status mlib_VideoColorYUV2ARGB420(mlib_u8       *argb,
 *                                             const mlib_u8 *y,
 *                                             const mlib_u8 *u,
 *                                             const mlib_u8 *v,
 *                                             mlib_s32      width,
...
mlib_status
mlib_VideoColorYUV2ARGB420_aligned(
	mlib_u8 *argb,
	const mlib_u8 *y,
	const mlib_u8 *u,
	const mlib_u8 *v,
...
mlib_status
mlib_VideoColorYUV2ARGB420_naligned(
	mlib_u8 *argb,
	const mlib_u8 *y,
	const mlib_u8 *u,
	const mlib_u8 *v,
	...
	mlib_s32 uv_stride)
{
	...
	if ((0 == (0xf & ((mlib_addr)y | (mlib_addr)argb))) &&
		(0 == (0xf & (y_stride | argb_stride)))) {
		return mlib_VideoColorYUV2ARGB420_aligned(argb, y, u, v,
			width, height, argb_stride, y_stride, uv_stride);
	} else {
		return mlib_VideoColorYUV2ARGB420_naligned(argb, y, u, v,
			width, height, argb_stride, y_stride, uv_stride);
	}
}
...
#pragma pipeloop(0)
		...
		for (; iW <= width - 16; iW += 16) {
			...
			x_temp = _mm_unpacklo_epi64(x_rgbl, x_rgbh);
			x_temp1 = _mm_unpackhi_epi64(x_rgbl, x_rgbh);
			...
			x_zrl = _mm_unpacklo_epi8(x_zero, x_r1);
			x_zrh = _mm_unpackhi_epi8(x_zero, x_r1);
...
#pragma pipeloop(0)
		...
		for (; iW <= width - 16; iW += 16) {
			...
			x_temp = _mm_unpacklo_epi64(x_rgbl, x_rgbh);
			x_temp1 = _mm_unpackhi_epi64(x_rgbl, x_rgbh);
			SAVE_ARGB2(x_temp, pd1);
			SAVE_ARGB2(x_temp1, pd2);
