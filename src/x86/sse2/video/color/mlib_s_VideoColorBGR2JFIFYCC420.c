...
#pragma ident	"@(#)mlib_s_VideoColorBGR2JFIFYCC420.c	9.7	07/11/05 SMI"
...
/* *********************************************************** */
mlib_status
mlib_VideoColorBGR2JFIFYCC420_aligned(
	mlib_u8 *y0,
	mlib_u8 *y1,
	mlib_u8 *cb,
	mlib_u8 *cr,
...
mlib_status
mlib_VideoColorBGR2JFIFYCC420_naligned(
	mlib_u8 *y0,
	mlib_u8 *y1,
	mlib_u8 *cb,
	mlib_u8 *cr,
...
__mlib_VideoColorBGR2JFIFYCC420(
	...
	mlib_s32 n)
{
	if (bgr0 == NULL || bgr1 == NULL || y0 == NULL || y1 == NULL ||
		cb == NULL || cr == NULL)
	...
	if (0 == (0xf & ((mlib_addr)bgr0 | (mlib_addr)bgr1 |
			(mlib_addr)y0 | (mlib_addr)y1 |
			(mlib_addr)cb | (mlib_addr)cr))) {
			return mlib_VideoColorBGR2JFIFYCC420_aligned(
				y0, y1, cb, cr, bgr0, bgr1, n);
	} else {
			return mlib_VideoColorBGR2JFIFYCC420_naligned(
				y0, y1, cb, cr, bgr0, bgr1, n);
	}
}
	...
	mlib_s32 n)
{
	...
	__m128i *pbgr0, *pbgr1, *py0, *py1, *pcb, *pcr;
...
#pragma pipeloop(0)
#endif /* __SUNPRO_C */
	for (; i <= size - 48; i += 48)	{
		x_bgr0 = _mm_load_si128(pbgr0++);
		x_bgr1 = _mm_load_si128(pbgr0++);
		x_bgr2 = _mm_load_si128(pbgr0++);
		...
		/* load */
		x_bgr0 = _mm_load_si128(pbgr1++);
		x_bgr1 = _mm_load_si128(pbgr1++);
		x_bgr2 = _mm_load_si128(pbgr1++);
	...
	if (i <= size - 24)	{
		x_bgr0 = _mm_load_si128(pbgr0);
		x_bgr1 = _mm_loadl_epi64(pbgr0+1);
		pbgr0 = (__m128i*) (((__m64*)pbgr0) + 3);
		...
		/* load */
		x_bgr0 = _mm_load_si128(pbgr1);
		x_bgr1 = _mm_loadl_epi64(pbgr1+1);
		pbgr1 = (__m128i*) (((__m64*)pbgr1) + 3);
	...
	/* r/g/b: 4 bytes */
	if (i <= size - 12)	{
		x_bgr0 = _mm_loadl_epi64(pbgr0);
		x_temp = _mm_cvtsi32_si128(((mlib_s32*)pbgr0)[2]);
		x_temp = _mm_slli_si128(x_temp, 8);
		x_bgr0 = _mm_or_si128(x_temp, x_bgr0);
		pbgr0 = (__m128i*) (((mlib_s32*)pbgr0) + 3);
		...
		/* load */
		x_bgr0 = _mm_loadl_epi64(pbgr1);
		x_temp = _mm_cvtsi32_si128(((mlib_s32*)pbgr1)[2]);
		x_temp = _mm_slli_si128(x_temp, 8);
		x_bgr0 = _mm_or_si128(x_temp, x_bgr0);
		pbgr1 = (__m128i*) (((mlib_s32*)pbgr1) + 3);
	...
	if (i <= size - 6) {
		/* 1st r/g/b */
		iTemp = *((mlib_s16 *)pbgr0);
		ir1 = ((mlib_u8 *)pbgr0)[2];
		pbgr0 = (__m128i*) (((mlib_u8 *)pbgr0) + 3);
		...
		/* 1st r/g/b */
		iTemp = *((mlib_s16 *)pbgr1);
		ir1 = ((mlib_u8 *)pbgr1)[2];
		pbgr1 = (__m128i*) (((mlib_u8 *)pbgr1) + 3);
...
#endif /* __SUNPRO_C */
	for (; i <= size - 48; i += 48)	{
		x_bgr0 = _mm_loadu_si128(pbgr0++);
		x_bgr1 = _mm_loadu_si128(pbgr0++);
		x_bgr2 = _mm_loadu_si128(pbgr0++);
		...
		/* load */
		x_bgr0 = _mm_loadu_si128(pbgr1++);
		x_bgr1 = _mm_loadu_si128(pbgr1++);
		x_bgr2 = _mm_loadu_si128(pbgr1++);
