...
#pragma ident	"@(#)mlib_s_VideoColorABGR2JFIFYCC444.c	9.5	07/11/05 SMI"
 ...
 *
 * SYNOPSIS
 *      mlib_status mlib_VideoColorABGR2JFIFYCC444(mlib_u8        *y,
 *                                                 mlib_u8        *cb,
 *                                                 mlib_u8        *cr,
 *                                                 const mlib_u8  *abgr,
 *                                                 mlib_s32        n)
...
/* abgr: 64 bytes */
#define	SeparateABGR64(x_abgr0, x_abgr1, x_abgr2, x_abgr3, x_b, x_g, x_r) \
{	\
...
/* abgr: 32 bytes */
#define	SeparateABGR32(x_abgr0, x_abgr1, x_b, x_g, x_r) \
{	\
	x_t1 = _mm_unpacklo_epi8(x_abgr0, x_abgr1);	\
	x_t2 = _mm_unpackhi_epi8(x_abgr0, x_abgr1);	\
							\
...
/* abgr: 16 bytes */
#define	SeparateABGR16(x_abgr0, x_b, x_g, x_r) \
{	\
...
/* *********************************************************** */
mlib_status
mlib_VideoColorABGR2JFIFYCC444_aligned(
	mlib_u8 *y,
	mlib_u8 *cb,
	mlib_u8 *cr,
	const mlib_u8 *abgr,
...
mlib_status
mlib_VideoColorABGR2JFIFYCC444_naligned(
	mlib_u8 *y,
	mlib_u8 *cb,
	mlib_u8 *cr,
	const mlib_u8 *abgr,
	...
	mlib_s32 n)
{
	...
	if (0 == (0xf & ((mlib_addr)abgr | (mlib_addr)y |
		...
		return mlib_VideoColorABGR2JFIFYCC444_aligned(
			y, cb, cr, abgr, n);
	} else {
		return mlib_VideoColorABGR2JFIFYCC444_naligned(
			y, cb, cr, abgr, n);
	}
}
...
#pragma pipeloop(0)
	...
	if (i <= size - 8)	{
		x_abgr0 = _mm_load_si128(pabgr);
		x_abgr1 = _mm_load_si128(pabgr+1);
		pabgr += 2;

		SeparateABGR32(x_abgr0, x_abgr1, x_b, x_g, x_r);
	...
	if (i <= size - 4)	{
		...
		SeparateABGR16(x_abgr0, x_b, x_g, x_r);
	...
	/* r/g/b: <=3 bytes */
	for (; i < size; i ++) {
		iTemp = *((mlib_s16 *)(((mlib_u8 *)pabgr)+1));
		ir1 = ((mlib_u8 *)pabgr)[3];
...
#pragma pipeloop(0)
	...
	if (i <= size - 8)	{
		x_abgr0 = _mm_loadu_si128(pabgr);
		x_abgr1 = _mm_loadu_si128(pabgr+1);
		pabgr += 2;
