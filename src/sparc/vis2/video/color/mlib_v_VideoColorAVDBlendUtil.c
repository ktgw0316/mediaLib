...
#pragma pipeloop(0)
			for (i = 0; i < (w >> 1); i++) {
				...
				alpha3 = vis_fpsub16(done,
					vis_fmul8x16al(vis_read_lo(dpix1),
					fone));
				alpha4 = mlib_v_blend_alpha64[256 -
					psrc2[8 * i + 4 + off]];
...
{
	mlib_d64 *dpdst, *dpsrc1, *dpsrc2;
	mlib_d64 alpha1, alpha2, pix1, pix2, res1, res2, alpha;
	mlib_f32 fone = vis_to_float(0x1000);
	mlib_d64 dzero = vis_fzero();
	mlib_s32 off = 0;
	mlib_s32 ow = w;
...
#pragma unroll(3)
			...
			if (w & 1) {
				pix2 = (*dpsrc2++);
				alpha = vis_bshuffle(dzero, pix2);
				alpha1 = vis_fexpand(vis_read_hi(alpha));
				res1 = vis_fmul8x16al(vis_read_hi(pix2),
					vis_read_lo(alpha1));
				res1 = vis_fpadd16(vis_fexpand(*((mlib_f32 *)
					dpsrc1)), res1);
