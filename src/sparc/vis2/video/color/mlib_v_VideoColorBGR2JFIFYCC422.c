 ...
 * SYNOPSIS
 *
 *      mlib_status mlib_v_VideoColorBGR2JFIFYCC422(mlib_u8       *y,
 *                                                  mlib_u8       *cb,
 *                                                  mlib_u8       *cr,
 *                                                  const mlib_u8 *rgb,
 *                                                  mlib_s32      n);
...
#pragma pipeloop(0)
	for (i = 0; i < n; i++) {
		...
		CHANNELWEIGHT_U8_2p(vis_read_hi(r0b1), vis_read_lo(b0g0),
			...
			vis_read_lo(r0b1), k11, k12, k13, off0, z0, z1);
		z1 = vis_fpadd16(z1, off0);
		py[0] = vis_fpmerge(vis_fpack16(z0), vis_fpack16(z1));

		CHANNELWEIGHT_U8_2p(vis_read_hi(r0b1), vis_read_lo(b0g0),
			vis_read_hi(b0g0), vis_read_lo(g1r1), vis_read_hi(g1r1),
			vis_read_lo(r0b1), k21, k22, k23, off128, z0, z1);
		pcb[0] = vis_fpack16(vis_fpadd16(z0, z1));

		CHANNELWEIGHT_U8_2p(vis_read_hi(r0b1), vis_read_lo(b0g0),
			vis_read_hi(b0g0), vis_read_lo(g1r1), vis_read_hi(g1r1),
			vis_read_lo(r0b1), k31, k32, k33, off128, z0, z1);
