...
#pragma ident	"@(#)mlib_v_VideoColorRGB2JFIFYCC444.c	9.2	07/11/05 SMI"
 ...
 * SYNOPSIS
 *
 *      mlib_status mlib_v_VideoColorRGB2JFIFYCC444(mlib_u8 *y,
 *                                                  mlib_u8 *cb,
 *                                                  mlib_u8 *cr,
 *                                                  const mlib_u8 *rgb,
 *                                                  mlib_s32 n);
...
#pragma pipeloop(0)
	for (i = 0; i < n; i++) {
		...
		CHANNELWEIGHT_U8(rdd, gdd, bdd, k11, k12, k13, off0, py[0]);
		CHANNELWEIGHT_U8(rdd, gdd, bdd, k21, k22, k23, off128, pcb[0]);
		CHANNELWEIGHT_U8(rdd, gdd, bdd, k31, k32, k33, off128, pcr[0]);
		py++;
		pcb++;
		pcr++;
	}
	...
	if ((mlib_u8 *)pcb <= cbend) {
		...
		CHANNELSEPARATE_U8(sd0, sd1, sd2, _rdd, _gdd, _bdd);
