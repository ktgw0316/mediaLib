	...
	mlib_s32 height)
{
	...
	mlib_d64 sum0hi, sum0lo, sum1hi, sum1lo, sum2hi, sum2lo, xy_hi, xy_lo;
	mlib_s32 y;
	mlib_d64 stx0, stx1, sty0, sty1, stz0, stz1;
 ...
 */
	...
	MLIB_V_VIDEOINTERP1(stx0, a0, ax, 2);
	dlX[0] = vis_bshuffle(stx0, a0);
	dlX += stride;
	for (y = 0; y <= (height - 2 - 1); y += 2) {
		a0 = sl0[0];
		b0 = sl0[stride];
 ...
 * Y-Interpolation
 */
		MLIB_V_VIDEOINTERP(sty0, a0, b0, 1);
		MLIB_V_VIDEOINTERP(sty1, b0, c0, 1);

/*
 * X-Interpolation
 */
		MLIB_V_VIDEOINTERP(stx0, b0, bx, 1);
		MLIB_V_VIDEOINTERP1(stx1, c0, cx, 2);
 ...
 */
		...
		xy_hi = vis_fmul8x16(scale, xy_hi);
		xy_lo = vis_fmul8x16(scale, xy_lo);
		stz0 = vis_fpack16_pair(xy_hi, xy_lo);

		xy_hi = vis_fpadd16(sum1hi, sum2hi);
		xy_lo = vis_fpadd16(sum1lo, sum2lo);
		xy_hi = vis_fmul8x16(scale, xy_hi);
		xy_lo = vis_fmul8x16(scale, xy_lo);
		stz1 = vis_fpack16_pair(xy_hi, xy_lo);

		dlX[0] = vis_bshuffle(stx0, b0);
		dlX[stride] = vis_bshuffle(stx1, c0);
		dlY[0] = sty0;
		dlY[stride] = sty1;
		dlZ[0] = vis_bshuffle(stz0, sty0);
		dlZ[stride] = vis_bshuffle(stz1, sty1);
 ...
 */
	...
	dlY[stride] = b0;
	dlZ[0] = vis_bshuffle(stz0, sty0);
	dlZ[stride] = vis_bshuffle(stx0, b0);
	...
	mlib_s32 height)
{
	...
	mlib_s32 y;
	mlib_d64 stx1, stx2, sty1, sty2, stz1, stz2;
 ...
 */
	...
	MLIB_V_VIDEOINTERP1(stx1, a1, ay, 5);

	dlX[1] = vis_bshuffle(stx1, a1);
	dlX += stride;
#pragma pipeloop(0)
#pragma unroll(3)
	for (y = 0; y <= (height - 2 - 1); y += 2) {
 ...
 */
		...
		MLIB_V_VIDEOINTERP(sty1, a1, b1, 2);
		MLIB_V_VIDEOINTERP(dlY[stride], b0, c0, 2);
		MLIB_V_VIDEOINTERP(sty2, b1, c1, 2);

/*
 * X-Interpolation
 */
		MLIB_V_VIDEOINTERP(dlX[0], b0, bx, 2);
		MLIB_V_VIDEOINTERP(stx1, b1, by, 3);
		MLIB_V_VIDEOINTERP1(dlX[stride], c0, cx, 4);
		MLIB_V_VIDEOINTERP1(stx2, c1, cy, 5);
 ...
 */
		...
		xy_hi = vis_fmul8x16(scale, xy_hi);
		xy_lo = vis_fmul8x16(scale, xy_lo);
		stz2 = vis_fpack16_pair(xy_hi, xy_lo);

		dlX[1] = vis_bshuffle(stx1, b1);
		dlX[stride + 1] = vis_bshuffle(stx2, c1);
		dlY[1] = sty1;
		dlY[stride + 1] = sty2;
		dlZ[1] = vis_bshuffle(stz1, sty1);
		dlZ[stride + 1] = vis_bshuffle(stz2, sty2);
 ...
 * X-Interpolation
 */
	MLIB_V_VIDEOINTERP(stx2, b0, bx, 2);
	MLIB_V_VIDEOINTERP(stx1, b1, by, 3);
 ...
 */
	...
	dlY[stride] = b0;
	dlY[stride + 1] = b1;
	dlZ[stride] = dlX[0] = stx2;

	dlX[1] = vis_bshuffle(stx1, b1);
	dlY[1] = sty1;
	dlZ[1] = vis_bshuffle(stz1, sty1);
	dlZ[stride + 1] = vis_bshuffle(stx1, b1);
