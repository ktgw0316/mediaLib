...
#define	PACK_S32_S16_MOD                                        \
	{                                                       \
	    t = vis_bshuffle(dr, dr1);                          \
	    vis_pst_16(t, dpz, emask);                          \
	}
...
#define	EXPAND_U8C_FOR_00_MUL                                             \
	 ...
	 * y_r * 16                                                       \
	 */                                                               \
	    dy = vis_fmul8x16al(vis_read_lo(dy), vis_to_float(0x1000));   \
	}
...
#define	EXPAND_U8C_FOR_MUL_                                           \
	dr2 = vis_faligndata(dx, dx);                                 \
	/* x_r */                                                     \
	dy  = vis_fpmerge(vis_read_hi(dy), vis_read_lo(dy));          \
	dy  = vis_fpmerge(vis_read_hi(dy), vis_read_lo(dy));          \
	dr3 = vis_fmul8x16al(vis_read_hi(dy), vis_to_float(0x1000));  \
	/* y_i * 16 */                                                \
	dy = vis_fexpand(vis_read_lo(dy));
...
#define	COMPOSE_S16C_FROM32_MOD_                                \
	{                                                       \
	    dr = vis_fpack32(dr, dzero);                        \
	    dr = vis_fpack32(dr, dzero);                        \
	    dr1 = vis_fpack32(dr1, dzero);                      \
...
#define	COMPOSE_S16C_FROM32_MOD_WITH_ODD                        \
	{                                                       \
	    COMPOSE_S16C_FROM32_MOD_;                           \
	    dr = vis_bshuffle(rest, dzh);                       \
	                                                        \
	    rest = dzl;                                         \
	    dzl = vis_bshuffle(dzh, rest);                      \
	    dzh = dr;                                           \
	}
...
#define	PACK_S32C_S16C1_MOD	dr = vis_bshuffle(dr1, dr);
...
#define	OVER_MUL_S16C_SAT                                       \
	mask_under = (vis_fcmplt32(dr, d_min16) << 2) |         \
		vis_fcmplt32(dr1, d_min16);                     \
	mask_over =                                             \
...
#pragma pipeloop(0)
		...
		if ((((mlib_addr)y ^ (mlib_addr)z) & 7) == 0) {
...
#pragma pipeloop(0)
			...
			dpz = dpzend;
			dy = vis_ld_d64_nf(dpy + len);
			dx0 = vis_ld_d64_nf(dpx + len);
			dx1 = vis_ld_d64_nf(dpx + len + 1);
			dx = vis_faligndata(dx0, dx1);
			MUL_U8_U16;
		} else {
			if ((off_x - off_z) > 0)
				vis_write_bmask(0x01234567,
					(off_x - off_z) * 0x11111111);
			if ((off_x - off_z) < 0)
				vis_write_bmask(0x01234567,
					(8 + off_x - off_z) * 0x11111111);
...
/* offset of address alignment in destination */
	...
	mlib_u64 gsr_mul = 0x02461357;
	mlib_u64 gsr_pack = 0x04152637;
	mlib_u64 gsr_pack2 = 0x30415263;

	if (n <= 0)
		return (MLIB_FAILURE);

	gsr_mul = ((gsr_mul << 32) | (gsr_for_mul));
	gsr_pack = ((gsr_pack << 32) | (gsr_for_pack));
	gsr_pack2 = ((gsr_pack2 << 32) | (gsr_for_pack));
...
#pragma pipeloop(0)
				...
				for (i = 0; i < 256; i++) {
					...
					dr1 = vis_fpackfix_pair(dr2, dr3);
					dr2 = vis_bshuffle(rest, dr1);
					dr = vis_fpmerge(vis_read_hi(dr),
						vis_read_lo(dr));
					dr = vis_fpmerge(vis_read_hi(dr),
						vis_read_lo(dr));
					dr3 = vis_fpmerge(vis_read_hi(dr2),
						vis_read_hi(dr));
					dr2 = vis_fpmerge(vis_read_lo(dr2),
						vis_read_lo(dr));
					dzh = vis_fpmerge(vis_read_hi(dr3),
						vis_read_hi(dr2));
					dzl = vis_fpmerge(vis_read_lo(dr3),
						vis_read_lo(dr2));
					rest = dr1;
					dpz[512 * k + 2 * i] = dzh;
					dpz[512 * k + 2 * i + 1] = dzl;
	...
	mlib_s32 n)
{
	mlib_d64 *dpz, *dpx, *dpy;
	mlib_d64 dx, dy, dy0, dy1;
	mlib_d64 dr, dr1, dzh, dzl, re2, im2, im3, im4;
	mlib_s8 *px, *py;
	mlib_s16 *pz;
 ...
 */
		...
		vis_alignaddr(py, 0);
		vis_write_bmask(0x01234567, off_x * 0x11111111);
