...
#pragma ident	"@(#)mlib_v_ImageBlend_OMSC_SAS.c	9.2	07/11/05 SMI"
...
#define	BLEND_OMSC_SAS(alpha, src_1, src_2, src_3, res)         \
	sum1 = vis_fpsub16(alpha, src_1);                       \
	res1 = vis_fmul8x16(src_2, sum1);                       \
	res = vis_fpadd16(res1, src_3)
    ...
    mlib_s32 cmask)
{
	...
	mlib_d64 pix1hi, pix1lo, pix1, pix2;
	mlib_d64 sum1, sum2;
...
#pragma pipeloop(0)
			for (i = 0; i < (dst_width - 1); i += 2) {
				...
				BLEND_OMSC_SAS(alpha1, pix1hi,
				    vis_read_hi(pix2), pix1hi, res2);
				...
				BLEND_OMSC_SAS(alpha2, pix1lo,
				    vis_read_lo(pix2), pix1lo, res4);
...
#pragma pipeloop(0)
			for (i = 0; i < (dst_width - 1); i += 2) {
				...
				alpha1 = table64[min];
				sum1 = vis_fpsub16(alpha1, pix1hi);
				res1 = vis_fmul8x16(vis_read_hi(pix2), sum1);
				res2 = vis_fpadd16(res1, pix1hi);

				pix1lo = vis_fmul8x16(vis_read_lo(pix1), d256);
				min = MIN(idx1 & 0x1ff, idx2 & 0x1ff);
				alpha2 = table64[min];
				sum2 = vis_fpsub16(alpha2, pix1lo);
				res3 = vis_fmul8x16(vis_read_lo(pix2), sum2);
				res4 = vis_fpadd16(res3, pix1lo);
