...
#pragma pipeloop(0)
		for (i = 0; i < (dst_width - 1); i += 2) {
			...
			idx1 = (*(mlib_u64 *)dpsrc2 >> (24 - 8 * off)) & mask;
			idx1 = idx1 << 3;

			dpix = vis_bshuffle(pix2, pix2);

			alpha1 = vis_fexpand(vis_read_hi(dpix));
			sum1 =
			    vis_fpadd16(vis_fmul8x16al(vis_read_hi(pix1), fone),
			    alpha1);
			...
			alpha2 =
			    *(mlib_d64 *)((mlib_u8 *)mlib_v_blend_alpha64 +
			    (idx1 & 0xff8));
			sum2 =
			    vis_fpadd16(vis_fmul8x16al(vis_read_lo(pix1), fone),
			    alpha2);
