...
#define	BLEND_ONE_SAS1(index1, index2, alpha, src_1, src_2,     \
	d_res)                                                  \
...
#define	BLEND_ONE_SAS(alpha, off, src_1, src_2, d_res)             \
	min = MIN(256 - psrc1[4 * i + off], psrc2[4 * i + off]);   \
	alpha = table64[min];                                      \
	res = vis_fexpand(src_1);                                  \
	d_res = vis_fmul8x16(src_2, alpha);                        \
	d_res = vis_fpadd16(res, d_res)
    ...
    mlib_s32 cmask)
{
	...
	mlib_s32 width, off = 0, off_4, min;
...
#pragma pipeloop(0)
			for (i = 0; i < (dst_width - 1); i += 2) {
				...
				mlib_u64 mask = 0x000000ff000000ff;
				mlib_u64 one = 0x0000010000000100;
				mlib_u64 idx1, idx2;
				...
				BLEND_ONE_SAS1(idx1 >> 32, idx2 >> 32, alpha1,
				    vis_read_hi(pix1), vis_read_hi(pix2), res1);
				BLEND_ONE_SAS1(idx1 & 0x1ff, idx2 & 0x1ff,
				    alpha2, vis_read_lo(pix1),
				    vis_read_lo(pix2), res2);
		...
		for (; i < dst_width; i++) {
			...
			BLEND_ONE_SAS1(256 - ((mlib_u8 *)dpsrc1)[off],
			    ((mlib_u8 *)dpsrc2)[off], alpha1,
			    *(mlib_f32 *)dpsrc1, *(mlib_f32 *)dpsrc2, res1);
