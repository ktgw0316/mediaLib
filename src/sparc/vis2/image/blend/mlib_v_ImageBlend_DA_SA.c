...
#pragma ident	"@(#)mlib_v_ImageBlend_DA_SA.c	9.2	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_ImageBlend_DA_SA - blend two images with
 *      SA source blend factor and DA destination blend factor
 *
 * SYNOPSIS
 *      mlib_status mlib_ImageBlend_DA_SA(mlib_image       *dst,
...
#pragma weak mlib_ImageBlend_DA_SA = __mlib_ImageBlend_DA_SA

#elif defined(__GNUC__)	/* defined(__SUNPRO_C) */
__typeof__(__mlib_ImageBlend_DA_SA) mlib_ImageBlend_DA_SA
    __attribute__((weak, alias("__mlib_ImageBlend_DA_SA")));
...
#pragma pipeloop(0)
		for (i = 0; i < (dst_width - 1); i += 2) {
			...
			alpha1 = vis_fmul8x16al(vis_read_hi(dpix1), fone);
		...
		for (; i < dst_width; i++) {
			...
			alpha1 =
			    ...
			    + off)));
			alpha2 =
			    vis_fexpand(vis_read_lo(vis_ld_u8((mlib_u8 *)dpsrc2
			    + off)));
			res1 =
