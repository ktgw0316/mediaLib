...
#pragma ident	"@(#)mlib_v_ImageBlend_OMSC_DA.c	9.2	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_ImageBlend_OMSC_DA - blend two images with
 *      DA source blend factor and OMSC destination blend factor
 *
 * SYNOPSIS
 *      mlib_status mlib_ImageBlend_OMSC_DA(mlib_image       *dst,
...
#pragma weak mlib_ImageBlend_OMSC_DA = __mlib_ImageBlend_OMSC_DA

#elif defined(__GNUC__)	/* defined(__SUNPRO_C) */
__typeof__(__mlib_ImageBlend_OMSC_DA) mlib_ImageBlend_OMSC_DA
    __attribute__((weak, alias("__mlib_ImageBlend_OMSC_DA")));
    ...
    mlib_s32 cmask)
{
	...
	mlib_d64 pix1hi, pix1lo, pix1, pix2, sum1, sum2, dpix;
	mlib_d64 res1, res2, res3, res4;
	mlib_s32 width, mask1, off = 0;
...
#pragma pipeloop(0)
		for (i = 0; i < (dst_width - 1); i += 2) {
			...
			alpha1 = vis_fmul8x16al(vis_read_hi(dpix), fone);
			...
			alpha2 = vis_fmul8x16al(vis_read_lo(dpix), fone);
		...
		for (; i < dst_width; i++) {
			...
			alpha1 = mlib_v_blend_alpha64[((mlib_u8 *)dpsrc1)[off]];
			sum1 =
