...
#define	BLEND_ZERO_SAS(alpha, off, src_2, res)                  \
	min = MIN(256 - psrc1[4 * i + off], idx2);              \
	alpha = table64[min];                                   \
	res = vis_fmul8x16(src_2, alpha)

/* *********************************************************** */

#define	BLEND_ZERO_SAS1(alpha, off, src_2, res)                    \
...
#pragma pipeloop(0)
			for (i = 0; i < (dst_width - 1); i += 2) {
				...
				BLEND_ZERO_SAS1(alpha1, off, vis_read_hi(pix2),
				    res1);
				BLEND_ZERO_SAS1(alpha2, off_4,
...
#pragma pipeloop(0)
			for (i = 0; i < (dst_width - 1); i += 2) {
				...
				idx2 = idx20 >> 32;
				BLEND_ZERO_SAS(alpha1, off, vis_read_hi(pix2),
				    res1);

				idx2 = idx20 & 0x1ff;
				BLEND_ZERO_SAS(alpha2, off_4, vis_read_lo(pix2),
				    res2);
		...
		for (; i < dst_width; i++) {
			...
			min =
			    ...
			    ((mlib_u8 *)dpsrc2)[off]);
			alpha1 = table64[min];
			res1 = vis_fmul8x16(*(mlib_f32 *)dpsrc2, alpha1);
