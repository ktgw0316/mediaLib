...
#define	BILINEAR_U8_1_Y_D                                                \
	dx1 = vis_fand(dmask, dx1);                                      \
	dx2 = vis_fand(dmask, dx2);                                      \
	dx3 = vis_fand(dmask, dx3);                                      \
	dx4 = vis_fand(dmask, dx4);                                      \
	dr =                                                             \
	vis_fpack16_pair(vis_fpadd16(dx1, dx2), vis_fpadd16(dx3, dx4))
    ...
    BILINEAR_PARAMETERS(mlib_u8))
{
	...
	mlib_d64 *dsp0, *dpd,
	    done = vis_to_double_dup(0x1000100),
	    dmask = vis_to_double_dup(0xff00ff);
	mlib_f32 fr;
	...
	if (dw) {
		if (((((mlib_s32)p_dst | dst_stride) & 3) | (((mlib_s32)p_src |
			...
			if (((dst_stride & 3) == 0) || (dst_width <= 4)) {
				beg =
				    (dst_width <=
				    4) ? dst_width : (4 - (mlib_u32)p_dst) & 3;
				for (j = 0; j < beg; j++) {
					for (i = 0; i < dst_height; i++) {
...
#pragma pipeloop(0)
					...
					off = (mlib_s32)dp & 7;
					pdend = (mlib_u8 *)dp + n - 1;
					emask = vis_edge8(dp, pdend);
					dp = (mlib_d64 *)((mlib_addr)dp & ~7);

					vis_write_bmask((8 - off) * 0x11111111,
					    0x01234567);
					j = 0;
					...
					if ((mlib_s32)sp0 & 1) {
						...
						dx3 = ap0[1];
						da0 = ap1[1];
						dx4 = vis_faligndata(da1, da0);
						BILINEAR_U8_1_Y_D;
						vis_pst_8(vis_bshuffle(dr, dr),
						    dp, emask);
						d_old = dr;
						j = 1;
					    }
#pragma pipeloop(0)
						...
						dx1 = vis_fmul8sux16(dx1, done);
						dx2 = vis_fmul8sux16(dx2, done);
						dx3 = vis_fmul8sux16(dx3, done);
						dx4 = vis_fmul8sux16(dx4, done);
						BILINEAR_U8_1_Y_D;
						vis_pst_8(vis_bshuffle(dr, dr),
						    dp, emask);
						d_old = dr;
...
/* averaging along both axes */
	...
	if (((((mlib_u32)p_dst | dst_stride) & 3) |
	    (((mlib_u32)p_src | src_stride) & 7)) == 0) {
	...
	for (i = 0; i < dst_height; i++) {
		...
		if ((((mlib_s32)sp0 | src_stride) & 7) == 0) {
			for (; j <= (dst_width - 4); j += 4) {
				...
				dx1 = ((mlib_d64 *)sp0)[4];
				dx2 = ((mlib_d64 *)sp0)[5];
				dx3 = ((mlib_d64 *)(sp0 + src_stride))[3];
				dx4 = ((mlib_d64 *)(sp0 + src_stride))[4];
				dx5 = ((mlib_d64 *)(sp0 + src_stride))[5];
    ...
    BILINEAR_PARAMETERS(mlib_s16))
{
	...
	mlib_f32 scale025 = vis_to_float(0x40404040),
	    scale05 = vis_to_float(0x80808080), fr;
...
/* common case */
		for (i = 0; i < dst_height; i++) {
			...
			dpd = (mlib_d64 *)vis_alignaddr(dp, -1);
			dsp0 = (mlib_d64 *)vis_alignaddr(sp0, 0);
			emask_hi = 0xff >> ((mlib_u32)dp - (mlib_u32)dpd);
			emask_lo = ~emask_hi;
			all_z = 8 - (mlib_u32)dp;
			dx2l = dsp0[0];
			dx2 = dsp0[1];
