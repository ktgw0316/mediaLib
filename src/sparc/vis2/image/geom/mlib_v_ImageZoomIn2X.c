...
#define	SWITCH_FUNC_BL(TT)                                        \
	...
	if (channels == 2 || channels == 3) {                     \
	    ...
	    case 4:                                               \
		res =                                             \
		    mlib_v_ImageZoomIn2X_##TT##_2BL_FL00          \
		    (PARAM1);                                     \
		break;                                            \
	    case 5:                                               \
		res =                                             \
		    mlib_v_ImageZoomIn2X_##TT##_2BL_FL01          \
		    (PARAM1);                                     \
		break;                                            \
	    case 6:                                               \
		res =                                             \
		    mlib_v_ImageZoomIn2X_##TT##_2BL_FL10          \
		    (PARAM1);                                     \
		break;                                            \
	    case 12:                                              \
		res =                                             \
		    mlib_v_ImageZoomIn2X_##TT##_4BL_FL00          \
		    (PARAM1);                                     \
		break;                                            \
	    case 14:                                              \
		res =                                             \
		    mlib_v_ImageZoomIn2X_##TT##_4BL_FL10          \
		    (PARAM1);                                     \
		break;                                            \
	    }                                                     \
	    if (res != MLIB_SUCCESS)                              \
    ...
    mlib_s32 n)
{
	...
	for (i = 0; i < n / 4; i++) {
		s0 = vis_freg_pair(((mlib_f32 *)sp0)[0], ((mlib_f32 *)sp0)[1]);
		s1 = vis_freg_pair(((mlib_f32 *)sp0)[1], ((mlib_f32 *)sp0)[2]);
		s2 = vis_freg_pair(((mlib_f32 *)sp1)[0], ((mlib_f32 *)sp1)[1]);
		s3 = vis_freg_pair(((mlib_f32 *)sp1)[1], ((mlib_f32 *)sp1)[2]);
		vis_write_bmask(0x01234589, 0);
		((mlib_d64 *)dp)[0] = vis_bshuffle(s0, s2);
		vis_write_bmask(0x2345ABCD, 0);
		((mlib_d64 *)dp)[1] = vis_bshuffle(s2, s1);
		vis_write_bmask(0x67ABCDEF, 0);
		((mlib_d64 *)dp)[2] = vis_bshuffle(s1, s3);
...
#define	MERGE_CH3(dp, sp0, sp1)                                           \
	...
	s2 = vis_freg_pair(((mlib_f32 *)sp1)[0], ((mlib_f32 *)sp1)[1]);   \
	s3 = vis_freg_pair(((mlib_f32 *)sp1)[1], ((mlib_f32 *)sp1)[2]);   \
	vis_write_bmask(0x01289A34, 0);                                   \
	((mlib_d64 *)dp)[0] = vis_bshuffle(s0, s2);                       \
	vis_write_bmask(0x1BCD234E, 0);                                   \
	((mlib_d64 *)dp)[1] = vis_bshuffle(s1, s2);                       \
	vis_write_bmask(0xBC567DEF, 0);                                   \
	((mlib_d64 *)dp)[2] = vis_bshuffle(s1, s3)
...
/* handle MLIB_BYTE data type of image */
		case MLIB_BYTE:
			...
			case MLIB_BILINEAR:
				...
				if (flag == 2 && channels == 4) {
					mlib_c_ImageZoomIn2X_U8_BL_FL2(PARAM2);
					break;
				}

				if (flag == 3 && channels == 2) {
					mlib_v_ImageZoomIn2X_U8_BL_FL3(PARAM2);
					break;
				}
...
/* handle MLIB_SHORT data type of image */
		case MLIB_SHORT:
			...
			case MLIB_BILINEAR:
				...
				if (flag == 3 && channels == 3) {
					mlib_v_ImageZoomIn2X_S16_BL_FL3(PARAM2);
					break;
				}

				if (flag == 2 && channels == 4) {
					mlib_v_ImageZoomIn2X_S16_BL_FL2(PARAM2);
					break;
				}
		...
		case MLIB_USHORT:
			...
			case MLIB_BILINEAR:
				...
				if (flag == 3 && channels == 3) {
					mlib_v_ImageZoomIn2X_U16_BL_FL3(PARAM2);
					break;
				}

				if (flag == 2 && channels == 4) {
					mlib_v_ImageZoomIn2X_U16_BL_FL2(PARAM2);
					break;
				}

				if (channels <= 3 || (flag == 0 &&
				    channels == 4)) {
					switch (flag) {
					case 0:
		...
		case MLIB_INT:
			switch (filter) {
			case MLIB_NEAREST:
				mlib_ImageZoomIn2X_S32_NN(MLIB_ZOOMIN2XCALLPARAM
				    (mlib_s32));
				break;
			case MLIB_BILINEAR:
				switch (flag) {
