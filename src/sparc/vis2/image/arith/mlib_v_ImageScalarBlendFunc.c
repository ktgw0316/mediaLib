    ...
    mlib_d64 ad)
{
	mlib_d64 *sp1, *sp2, *dp;
	mlib_d64 sd1, sd2, dd;
	mlib_d64 sd3, sd4, sd5, sd6, sd7, sd8;
...
#pragma pipeloop(0)
	for (j = 0; j <= (dsize - 32); j += 32) {
		...
		*(dp) = dd;
		...
		sd3 = *(sp1 + 1);
		sd4 = *(sp2 + 1);
		MLIB_V_IMAGESCALARBLEND_U8(sd3, sd4, ad, bd, ad, bd);
		*(dp + 1) = dd;

		sd5 = *(sp1 + 2);
		sd6 = *(sp2 + 2);
		MLIB_V_IMAGESCALARBLEND_U8(sd5, sd6, ad, bd, ad, bd);
		*(dp + 2) = dd;

		sd7 = *(sp1 + 3);
		sd8 = *(sp2 + 3);
		MLIB_V_IMAGESCALARBLEND_U8(sd7, sd8, ad, bd, ad, bd);
		*(dp + 3) = dd;
...
/* last 8-pixel column */
		...
		if ((mlib_addr)dp <= (mlib_addr)dend) {
			sd10 = vis_faligndata(s11, sp1[1]);
			vis_alignaddr(sa2, off);
			sd20 = vis_faligndata(s21, sp2[1]);
			MLIB_V_IMAGESCALARBLEND_U8(sd10, sd20, ad, bd, ad, bd);
...
/* last 8-pixel column */
		...
		if ((mlib_addr)dp <= (mlib_addr)dend) {
			...
			if ((mlib_addr)dp <= (mlib_addr)dend) {
				...
				if ((mlib_addr)dp <= (mlib_addr)dend) {
					sd20 = vis_faligndata(s21, sp2[3]);
					vis_alignaddr(sa1, off);
					sd10 = vis_faligndata(s11, sp1[3]);
					MLIB_V_IMAGESCALARBLEND_U8(sd10, sd20,
					    a1, b1, a2, b2);
					emask = vis_edge8((mlib_u8 *)dp, dend);
					vis_pst_8(dd, dp, emask);
...
/* aligned dst address */
	mlib_d64 *dp;
	mlib_d64 sd1, sd2, sd3, sd4, sd5, sd6, sd7, sd8;
...
#pragma pipeloop(0)
	for (j = 0; j <= (dsize - 16); j += 16) {
		...
		*(dp) = dd;
		...
		sd3 = *(sp1 + 1);
		sd4 = *(sp2 + 1);
		MLIB_V_IMAGESCALARBLEND_S16(sd3, sd4, ad, bd);
		*(dp + 1) = dd;

		sd5 = *(sp1 + 2);
		sd6 = *(sp2 + 2);
		MLIB_V_IMAGESCALARBLEND_S16(sd5, sd6, ad, bd);
		*(dp + 2) = dd;

		sd7 = *(sp1 + 3);
		sd8 = *(sp2 + 3);
		MLIB_V_IMAGESCALARBLEND_S16(sd7, sd8, ad, bd);
		*(dp + 3) = dd;
...
#pragma pipeloop(0)
	for (j = 0; j <= (dsize - 16); j += 16) {
		...
		*dp = dd;
		...
		sd3 = *(sp1 + 1);
		sd4 = *(sp2 + 1);
		MLIB_V_IMAGESCALARBLEND_U16(sd3, sd4, ad, bd);
		*(dp + 1) = dd;

		sd5 = *(sp1 + 2);
		sd6 = *(sp2 + 2);
		MLIB_V_IMAGESCALARBLEND_U16(sd5, sd6, ad, bd);
		*(dp + 2) = dd;

		sd7 = *(sp1 + 3);
		sd8 = *(sp2 + 3);
		MLIB_V_IMAGESCALARBLEND_U16(sd7, sd8, ad, bd);
		*(dp + 3) = dd;
...
#pragma pipeloop(0)
		for (i = 0; i < xsize - 3; i += 4) {
			dl[i] = (mlib_s32)(a0 * (mlib_d64)sl1[i] +
			    (1 - a0) * (mlib_d64)sl2[i]);
			dl[i + 1] = (mlib_s32)(a0 * (mlib_d64)sl1[i + 1] +
			    (1 - a0) * (mlib_d64)sl2[i + 1]);
			dl[i + 2] = (mlib_s32)(a0 * (mlib_d64)sl1[i + 2] +
			    (1 - a0) * (mlib_d64)sl2[i + 2]);
			dl[i + 3] = (mlib_s32)(a0 * (mlib_d64)sl1[i + 3] +
			    (1 - a0) * (mlib_d64)sl2[i + 3]);
		}
