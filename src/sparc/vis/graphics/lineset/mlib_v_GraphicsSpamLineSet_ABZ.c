...
#pragma ident	"@(#)mlib_v_GraphicsSpamLineSet_ABZ.c	9.2	07/11/05 SMI"
...
#define	MLIB_MAKE_ALPHA8                                                   \
	...
	zdata[xz] = stz;                                                   \
	w_resbg =                                                          \
	vis_fpadd16(vis_fmul8x16(vis_read_lo(w_bg), da),                   \
	    vis_fmul8x16(fa, w_resbg));                                    \
	vis_st_u8_i(w_resbg, data, xz);                                    \
	w_uresbg =                                                         \
	vis_fpadd16(vis_fmul8x16(vis_read_lo(w_ubg), da),                  \
	    vis_fmul8x16(fa, w_uresbg));                                   \
	vis_st_u8_i(w_uresbg, udata, ux);                                  \
	w_dresbg =                                                         \
	vis_fpadd16(vis_fmul8x16(vis_read_lo(w_dbg), da),                  \
	    vis_fmul8x16(fa, w_dresbg));                                   \
    ...
    mlib_s32 height)
{
	...
	fa1 = vis_read_lo(vis_fpmerge(vis_fzeros(), vis_to_float(a)));

	da = vis_to_double(a1, a1);
	da1 = vis_to_double((a1 << 23) | (a1 << 7), (a1 << 23) | (a1 << 7));
...
#pragma pipeloop(0)
					for (j = 0; j <= n; j++, x++) {
						...
						w_uresbg =
						    vis_fpadd16(vis_fmul8x16
						    (vis_read_lo(w_ubg), da),
						    vis_fmul8x16(fa, w_uresbg));
						vis_st_u8_i(w_uresbg, udata,
						    ux);
						w_dresbg =
						    vis_fpadd16(vis_fmul8x16
						    (vis_read_lo(w_dbg), da),
						    vis_fmul8x16(fa, w_dresbg));
    ...
    mlib_s32 height)
{
	...
	da = vis_to_double((a1), (a1 << 16) | (a1));
	d_c = vis_fpmerge(vis_fzeros(), vis_to_float(c));
