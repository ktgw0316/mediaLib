...
#pragma ident	"@(#)mlib_v_GraphicsDrawEllipse_AB.c	9.3	07/11/05 SMI"
    ...
    mlib_s32 alpha)
{
	...
	aA1 = ~alpha & 0xff;
	fFa = vis_to_float((alpha << 24) |
	    (alpha << 16) | (alpha << 8) | alpha);

	dDa = vis_to_double(alpha, alpha);
    ...
    mlib_s32 alpha)
{
	...
	aA1 = ~alpha & 0xff;
	fFa = vis_to_float((alpha << 16) | (alpha << 8) | alpha);
	dDa = vis_to_double((alpha << 6), (alpha << 22) | (alpha << 6));
	...
	if (a == b)
		return (__mlib_GraphicsDrawCircle_AB_32(buffer, x, y, a, c,
		    alpha));
