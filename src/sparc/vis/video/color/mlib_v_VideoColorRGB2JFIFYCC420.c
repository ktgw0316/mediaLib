...
#pragma ident	"@(#)mlib_v_VideoColorRGB2JFIFYCC420.c	9.2	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_VideoColorRGB2JFIFYCC420 - color conversion with
 *                                      420 downsampling with bilinear filter
 *                                      Only Cb and Cr components
	...
	mlib_s32 n)
{
	...
	mlib_u8 *y0end = y0 + n, *cbend = cb + (n >> 1);
	mlib_d64 sd0, sd1, sd2, se0, se1, se2;
	mlib_d64 sda, sdb, sdc, sea, seb, sec;
	mlib_d64 r0g0, b0r1, g1b1, z0, z1;
	mlib_d64 _r0g0, _b0r1, _g1b1;
	...
	sd0 = vis_ld_d64_nf(sp0);
	sp0++;
	sd1 = vis_ld_d64_nf(sp0);
	sp0++;
	sd2 = vis_ld_d64_nf(sp0);
	sp0++;
	se0 = vis_ld_d64_nf(sp1);
	sp1++;
	se1 = vis_ld_d64_nf(sp1);
	sp1++;
	se2 = vis_ld_d64_nf(sp1);
	sp1++;
	CHANNELSEPARATE_U8_422(sd0, sd1, sd2, r0g0, b0r1, g1b1);
	CHANNELSEPARATE_U8_422(se0, se1, se2, _r0g0, _b0r1, _g1b1);
#pragma pipeloop(0)
	for (i = 0; i < n; i++) {
		sd0 = vis_ld_d64_nf(sp0);
		sp0++;
		...
		CHANNELWEIGHT_U8_2p(vis_read_hi(r0g0), vis_read_lo(r0g0),
			vis_read_hi(b0r1), vis_read_lo(b0r1),
			vis_read_hi(g1b1), vis_read_lo(g1b1), k11, k12, k13,
			z0, z1);
		...
		CHANNELWEIGHT_U8_2p(vis_read_hi(_r0g0), vis_read_lo(_r0g0),
			vis_read_hi(_b0r1), vis_read_lo(_b0r1),
			vis_read_hi(_g1b1), vis_read_lo(_g1b1), k11, k12, k13,
			z0, z1);
		z0 = vis_fpadd16(z0, off0);
		z1 = vis_fpadd16(z1, off0);
		...
		sda = vis_fpmerge(vis_read_hi(sd0), vis_read_lo(sd1));
		sea = vis_fpmerge(vis_read_hi(se0), vis_read_lo(se1));

		CHANNELWEIGHT_U8_2p(vis_read_hi(r0g0), vis_read_lo(r0g0),
			vis_read_hi(b0r1), vis_read_lo(b0r1),
			vis_read_hi(g1b1), vis_read_lo(g1b1), k21, k22, k23,
			z0, z1);
		cb0 = vis_fpadd16(z0, z1);
		CHANNELWEIGHT_U8_2p(vis_read_hi(_r0g0), vis_read_lo(_r0g0),
			vis_read_hi(_b0r1), vis_read_lo(_b0r1),
			vis_read_hi(_g1b1), vis_read_lo(_g1b1), k21, k22, k23,
			z0, z1);
		...
		sdb = vis_fpmerge(vis_read_lo(sd0), vis_read_hi(sd2));
		sdc = vis_fpmerge(vis_read_hi(sd1), vis_read_lo(sd2));
		seb = vis_fpmerge(vis_read_lo(se0), vis_read_hi(se2));
		sec = vis_fpmerge(vis_read_hi(se1), vis_read_lo(se2));

		CHANNELWEIGHT_U8_2p(vis_read_hi(r0g0), vis_read_lo(r0g0),
			vis_read_hi(b0r1), vis_read_lo(b0r1),
			vis_read_hi(g1b1), vis_read_lo(g1b1), k31, k32, k33,
			z0, z1);
		...
		CHANNELWEIGHT_U8_2p(vis_read_hi(_r0g0), vis_read_lo(_r0g0),
			vis_read_hi(_b0r1), vis_read_lo(_b0r1),
			vis_read_hi(_g1b1), vis_read_lo(_g1b1), k31, k32, k33,
			z0, z1);
		cr1 = vis_fpadd16(z0, z1);
		cr0 = vis_fpadd16(cr0, off128);
		cr0 = vis_fpadd16(cr0, cr1);
		pcr[0] = vis_fpack16(cr0);
		_r0g0 = vis_fpmerge(vis_read_hi(sea), vis_read_lo(seb));
		_b0r1 = vis_fpmerge(vis_read_lo(sea), vis_read_hi(sec));
		_g1b1 = vis_fpmerge(vis_read_hi(seb), vis_read_lo(sec));
