...
#pragma ident	"@(#)mlib_v_VideoColorYUV2ABGR411.c	9.3	07/11/05 SMI"
...
static mlib_status mlib_v_VideoColorYUV2ABGR411_dst_align(
	mlib_u8 *abgr,
	const mlib_u8 *y,
	const mlib_u8 *u,
	const mlib_u8 *v,
	...
	mlib_s32 uv_stride)
{
	...
	if ((((mlib_addr)abgr & 7) == 0) && (((abgr_stride & 7) == 0) ||
		(height == 1))) {
		...
		return mlib_v_VideoColorYUV2ABGR411_dst_align(abgr, y, u, v,
			width, height, abgr_stride, y_stride, uv_stride);
	} else {
		return mlib_v_VideoColorYUV2ABGR411_dst_nonalign(abgr, y, u, v,
			width, height, abgr_stride, y_stride, uv_stride);
	}
}
...
static mlib_status
mlib_v_VideoColorYUV2ABGR411_dst_align(
	mlib_u8 *abgr,
	const mlib_u8 *y,
	const mlib_u8 *u,
	const mlib_u8 *v,
