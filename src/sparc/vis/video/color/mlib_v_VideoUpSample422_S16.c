...
#pragma pipeloop(0)
	for (i = 0; i <= (n - 8); i += 8) {
		...
		((mlib_s32 *)dp)[4] =
			((mlib_u32)sa_i0 >> 16) + (sa_i0 & ~0xFFFF);
		((mlib_s32 *)dp)[5] = (sa_i0 << 16) + (sa_i0 & 0xFFFF);
		((mlib_s32 *)dp)[6] =
			((mlib_u32)sa_i1 >> 16) + (sa_i1 & ~0xFFFF);
		((mlib_s32 *)dp)[7] = (sa_i1 << 16) + (sa_i1 & 0xFFFF);
		sp += 2;
		dp += 4;
	}
	...
	if ((mlib_s16 *)dp <= dend) {
		...
		if ((mlib_s16 *)dp <= dend) {
			...
			if ((mlib_s16 *)dp <= dend) {
				sa = vis_ld_d64_nf(sp + 1);
				dr = vis_fpmerge(vis_read_hi(sa),
					vis_read_lo(sa));
				dr = vis_fpmerge(vis_read_hi(dr),
					vis_read_lo(dr));
				dr1 = vis_fpmerge(vis_read_lo(dr),
					vis_read_lo(dr));
				dr = vis_fpmerge(vis_read_hi(dr),
					vis_read_hi(dr));
				da = vis_fpmerge(vis_read_hi(dr),
					vis_read_hi(dr1));
				emask = vis_edge16(dp, dend);
				vis_pst_16(da, dp, emask);
				dp++;
