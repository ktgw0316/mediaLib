...
/* Y*1.1644 */
			...
			dy = vis_ld_d64_nf((mlib_d64 *)y0 + i);
...
/* Y*1.1644 */
			...
			dz = vis_ld_d64_nf((mlib_d64 *)y1 + i);
...
/* V*(-0.8132); */
			...
			da0 = vis_ld_d64_nf((mlib_d64 *)a0 + i);
			((mlib_f32 *)dpp0)[8 * i + 0] = vis_read_hi(dd);
			((mlib_f32 *)dpp0)[8 * i + 1] = vis_read_lo(dd);
			((mlib_f32 *)dpp0)[8 * i + 2] = vis_read_hi(dd1);
			((mlib_f32 *)dpp0)[8 * i + 3] = vis_read_lo(dd1);
...
/* Y*1.1644 */
			...
			da1 = vis_ld_d64_nf((mlib_d64 *)a1 + i);
 ...
 */
	...
	for (i = 0; i < h;
		...
		if (((mlib_addr)(y + aylb) & 7) == 0)
			...
			__mlib_VectorCopy_U8((mlib_u8 *)y1_, (y + aylb), w);
		...
		if (a_array) {
			...
			if (((mlib_addr)a_array & 7) == 0)
				...
				__mlib_VectorCopy_U8((mlib_u8 *)a1, a_array, w);
