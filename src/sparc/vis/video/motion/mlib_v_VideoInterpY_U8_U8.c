...
#pragma pipeloop(0)
			for (y = 0; y < (height >> 1); ++y) {
				...
				sf0 = sfd[0];
				sf1 = sfd[1];
				MLIB_V_VIDEOINTERP2(d1, vis_faligndata(s0, s1),
					vis_faligndata(sf0, sf1));
				sd = (mlib_d64 *)((mlib_u8 *)sd + field_stride);
				sfd = (mlib_d64 *)((mlib_u8 *)sfd +
					field_stride);
...
#pragma pipeloop(0)
			for (y = 0; y < height; ++y) {
				...
				sf0 = sfd[0];
				sf1 = sfd[1];
				MLIB_V_VIDEOINTERP2(d0, vis_faligndata(s0, s1),
					vis_faligndata(sf0, sf1));
				s2 = sd[2];
				sf2 = sfd[2];
