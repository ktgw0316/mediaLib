...
#pragma ident	"@(#)mlib_v_ImageZoomIn2X_BL_S16.c	9.3	07/11/05 SMI"
...
#define	LOAD_1S16A(l0, l1, psrc, index)	LOAD_1U8A(l0, l1, psrc, index)
#define	STORE_1S16A(pdst, index, r0, r1)	STORE_1U8A(pdst, index, r0, r1)

#define	LOAD_1S16G(l0, l1, psrc, index)	LOAD_1U8G(l0, l1, psrc, 2*(index))

#define	LOAD_1S16G_NF(l0, l1, psrc, index)                                   \
	LOAD_1U8G_NF(l0, l1, psrc, 2*(index))

#define	STORE_1S16G(pdst, index, r0, r1)	STORE_1U8G(pdst, index, r0, r1)

#define	STORE_1S16G_NF(pdst, index, r0, r1)                                  \
	STORE_1U8G_NF(pdst, index, r0, r1)

#define	STORE_1S16G_LAST(pdst, index, r0, r1)	STORE_1U8G_LAST(pdst, index, \
						r0, r1)

/* *********************************************************** */

#define	LOAD_4S16A(l0, l1, psrc, index)                         \
	{                                                       \
	    mlib_f32 *ps = (mlib_f32 *)(psrc + index);          \
	                                                        \
	    l0 = vis_freg_pair(ps[0], ps[1]);                   \
...
#define	LOAD_4S16G(l0, l1, psrc, index)                         \
	{                                                       \
...
#define	LOAD_4S16G_NF(l0, l1, psrc, index)                      \
	{                                                       \
	    mlib_d64 *ps = VIS_ALIGNADDR(psrc, 2 * (index));    \
	                                                        \
	    l0 = ps[0];                                         \
...
void
mlib_v_ImageZoomIn2X_S16_1BL_FL0(
    MLIB_ZOOMIN2XDEFPARAM_BL(mlib_s16))
{
/* indices */
	...
	while (num_of_rows > 0) {
		...
		if (((((mlib_addr)psrc | src_stride) & 3) == 0) &&
			...
			for (j = 0; j < src_width / 2 - 3; j += 4) {
				...
				LOAD_1S16A(w_src10, w_src11, psrc, j);
...
#pragma pipeloop(0)
				for (i = 0; i < src_height / 2; i++) {
					...
					LOAD_1S16A(w_src10, w_src11, psrc,
					    (i + 1) * src_stride + j);

					MAKE_1S16_FL0;

					TAIL_1S16_FL0(RESULT_1S16);
			...
			for (j = 0; j < src_width / 2 - 3; j += 4) {
				...
				LOAD_1S16G(w_src10, w_src11, psrc, j);
				...
				if (src_height / 2 > 0) {
					...
					LOAD_1S16G_NF(w_src10, w_src11, psrc,
					    src_stride + j);
					...
					STORE_1S16G_NF(pdst, 2 * j, w_res0,
					    w_res2);

					STORE_1S16G(pdst, dst_stride + 2 * j,
					    w_res1, w_res3);
				}
		...
		if (j < src_width / 2) {
			...
			LOAD_1S16G_NF(w_src10, w_src11, psrc, j);
...
void
mlib_v_ImageZoomIn2X_S16_2BL_FL0(
    MLIB_ZOOMIN2XDEFPARAM_BL(mlib_s16))
{
/* indices */
...
void
mlib_v_ImageZoomIn2X_S16_3BL_FL0(
    MLIB_ZOOMIN2XDEFPARAM_BL(mlib_s16))
{
/* indices */
	...
	while (num_of_rows > 0) {
		...
		for (j = 0; j < src_width / 2 - 2; j += 3) {
			...
			for (i = 0; i < src_height / 2; i++) {
				...
				LOAD_1S16G(w_src10, w_src11, psrc,
				    (i + 1) * src_stride + j);

				MAKE_1S16_FL0;

				TAIL_1S16_FL0(RESULT_3S16);
...
void
mlib_v_ImageZoomIn2X_S16_4BL_FL0(
    MLIB_ZOOMIN2XDEFPARAM_BL(mlib_s16))
{
/* indices */
	...
	while (num_of_rows > 0) {
		...
		if (((((mlib_addr)psrc | src_stride) & 3) == 0) &&
			...
			for (j = 0; j < src_width / 2 - 3; j += 4) {
				...
				LOAD_4S16A(w_src10, w_src11, psrc, j);
...
#pragma pipeloop(0)
				for (i = 0; i < src_height / 2; i++) {
					...
					LOAD_4S16A(w_src10, w_src11, psrc,
					    (i + 1) * src_stride + j);

					MAKE_1S16_FL0;

					TAIL_1S16_FL0(RESULT_4S16);
			...
			for (j = 0; j < src_width / 2 - 3; j += 4) {
				...
				LOAD_4S16G(w_src10, w_src11, psrc, j);
				...
				if (src_height / 2 > 0) {
					...
					LOAD_4S16G_NF(w_src10, w_src11, psrc,
					    src_stride + j);
		...
		if (j < src_width / 2) {
			...
			LOAD_4S16G_NF(w_src10, w_src11, psrc, j);
...
#define	MOVE_1S16_FL1	w_old_33 = w_tmp_33; w_old_30 = w_tmp_30

/* *********************************************************** */

#define	MAKE_1S16_FL1                                           \
	w_src10 = vis_fand(w_src10, w_mask2);                   \
	w_src11 = vis_fand(w_src11, w_mask2);                   \
...
void
mlib_v_ImageZoomIn2X_S16_1BL_FL1(
    MLIB_ZOOMIN2XDEFPARAM_BL(mlib_s16))
{
/* indices */
	mlib_s32 i, j, channels = 1;
...
void
mlib_v_ImageZoomIn2X_S16_2BL_FL1(
    MLIB_ZOOMIN2XDEFPARAM_BL(mlib_s16))
{
/* indices */
	mlib_s32 i, j, channels = 2;
...
void
mlib_v_ImageZoomIn2X_S16_4BL_FL1(
    MLIB_ZOOMIN2XDEFPARAM_BL(mlib_s16))
{
/* indices */
	mlib_s32 i, j, channels = 4;
...
void
mlib_v_ImageZoomIn2X_S16_1BL_FL2(
    MLIB_ZOOMIN2XDEFPARAM_BL(mlib_s16))
{
/* indices */
	mlib_s32 i, j, channels = 1;
...
void
mlib_v_ImageZoomIn2X_S16_2BL_FL2(
    MLIB_ZOOMIN2XDEFPARAM_BL(mlib_s16))
{
/* indices */
	mlib_s32 i, j, channels = 2;
...
void
mlib_v_ImageZoomIn2X_S16_3BL_FL2(
    MLIB_ZOOMIN2XDEFPARAM_BL(mlib_s16))
{
/* indices */
	mlib_s32 i, j, channels = 3;
...
void
mlib_v_ImageZoomIn2X_S16_4BL_FL2(
    MLIB_ZOOMIN2XDEFPARAM_BL(mlib_s16))
{
/* indices */
	mlib_s32 i, j, channels = 4;
...
#define	STORE_RES_S16_1(dr, dr1, pz)                            \
	{                                                       \
	    mlib_d64 dd0, dd1;                                  \
	                                                        \
...
#define	BILINEAR_S16_124                                        \
	dr = vis_fpadd16(da2, dd2);                             \
	dr1 = vis_fpadd16(da3, dd3)

/* *********************************************************** */

#define	BILINEAR_S16_124_TAIL                                   \
	da = dd;                                                \
	da1 = dd1;                                              \
	da2 = dd2;                                              \
	da3 = dd3
...
#define	GET_STR_S16_1_BILINEAR(ind, da, da1, da2, da3)          \
...
void
mlib_v_ImageZoomIn2X_S16_1BL_FL3(
    MLIB_ZOOMIN2XDEFPARAM_BL(mlib_s16))
{
/* indices */
	mlib_s32 i, j;
	...
	while (num_of_rows > 0) {
		...
		for (j = 0; j <= ((src_width / 2) - 4); j += 4) {
			...
			GET_STR_S16_1_BILINEAR(0, da, da1, da2, da3);
			prow += src_stride;

			for (i = 0; i < (src_height / 2); i++) {
				GET_STR_S16_1_BILINEAR(0, dd, dd1, dd2, dd3);
				STORE_RES_S16_1(da, da1, pz_row);
				BILINEAR_S16_124;
				pz_row += dst_stride - 8;
				STORE_RES_S16_1(dr, dr1, pz_row);
				BILINEAR_S16_124_TAIL;
				prow += src_stride;
				pz_row += dst_stride - 8;
			}
		}
...
#define	GET_STR_S16_2_BILINEAR(ind, da, da1, da2, da3)          \
	dprow = (mlib_d64 *)VIS_ALIGNADDR(prow, ind);           \
	da = dprow[0];                                          \
	da1 = dprow[1];                                         \
	da3 = vis_ld_d64_nf(dprow + 2);                         \
...
void
mlib_v_ImageZoomIn2X_S16_2BL_FL3(
    MLIB_ZOOMIN2XDEFPARAM_BL(mlib_s16))
{
/* indices */
	mlib_s32 i, j, l;
...
/* aligned case */
		if (!(((mlib_addr)pdst & 3) | (dst_stride & 1))) {
			...
			for (j = 0; j <= ((src_width / 2) - 2); j += 2) {
				...
				GET_STR_S16_2_BILINEAR(0, da, da1, da2, da3);
				prow += src_stride;
				for (i = 0; i < (src_height / 2); i++) {
					GET_STR_S16_2_BILINEAR(0, dd, dd1, dd2,
					    dd3);
					((mlib_f32 *)pz_row)[0] =
					    vis_read_hi(da);
					((mlib_f32 *)pz_row)[1] =
...
#define	COMPOSE_S16_3(pointer, dx, dx1, dx2, dy, dy1, dy2,      \
...
#define	GET_STR_S16_3_BILINEAR(ind, dah, dam, dal, da1h,        \
...
#define	STORE_RES_S16_3_COMMON(pointer, res1, res2, res3,             \
	res4, res5, res6)                                             \
	{                                                             \
	    mlib_d64 *dpz = (mlib_d64 *)VIS_ALIGNADDR(pointer, -1);   \
	    mlib_s32 emask =                                          \
...
#define	STORE_RES_S16_3_COMMON_NF(pointer, res1, res2, res3,          \
	res4, res5, res6)                                             \
	{                                                             \
	    mlib_d64 *dpz = (mlib_d64 *)VIS_ALIGNADDR(pointer, -1);   \
	    mlib_s32 emask =                                          \
...
void
mlib_v_ImageZoomIn2X_S16_3BL_FL3(
    MLIB_ZOOMIN2XDEFPARAM_BL(mlib_s16))
{
/* indices */
	mlib_s32 i, j, l;
	...
	while (num_of_rows > 0) {
		...
		for (j = 0; j <= ((src_width / 2) - 4); j += 4) {
			...
			GET_STR_S16_3_BILINEAR(0, dah, dam, dal, da1h, da1m,
			    da1l, da2h, da2m, da2l, da3h, da3m, da3l);
			prow += src_stride;

			if (src_height > 0) {
				COMPOSE_S16_3(dprow, dah, dam, dal, da1h, da1m,
				    da1l, drh, drm, drl, dr1h, dr1m, dr1l);
				STORE_RES_S16_3_COMMON_NF(pz_row, drh, drm, drl,
				    dr1h, dr1m, dr1l);
				pz_row += dst_stride;
...
#pragma pipeloop(0)
					for (i = 0; i < ((src_height / 2) - 1);
					    i++) {
						GET_STR_S16_3_BILINEAR(0, ddh,
						    ddm, ddl, dd1h, dd1m, dd1l,
						    dd2h, dd2m, dd2l, dd3h,
						    dd3m, dd3l);
						BILINEAR_S16_3;
						COMPOSE_S16_3(dprow, da2h, da2m,
						    da2l, da3h, da3m, da3l, drh,
						    drm, drl, dr1h, dr1m, dr1l);
						STORE_RES_S16_3_COMMON(pz_row,
...
/* aligned case */
					for (i = 0; i < ((src_height / 2) - 1);
						...
						((mlib_d64 *)pz_row)[3] = dr1h;
						((mlib_d64 *)pz_row)[1] = drm;
						((mlib_d64 *)pz_row)[4] = dr1m;
						((mlib_d64 *)pz_row)[2] = drl;
						((mlib_d64 *)pz_row)[5] = dr1l;
						pz_row += dst_stride;
				...
				GET_STR_S16_3_BILINEAR(0, ddh, ddm, ddl, dd1h,
				    dd1m, dd1l, dd2h, dd2m, dd2l, dd3h, dd3m,
				    dd3l);
				BILINEAR_S16_3;
				COMPOSE_S16_3(dprow, da2h, da2m, da2l, da3h,
				    da3m, da3l, drh, drm, drl, dr1h, dr1m,
				    dr1l);
				STORE_RES_S16_3_COMMON(pz_row, drh, drm, drl,
				    dr1h, dr1m, dr1l);
			}
		}
...
#define	GET_STR_S16_4_BILINEAR(ind, da, da1, da2, da3)          \
	dprow = (mlib_d64 *)VIS_ALIGNADDR(prow, ind);           \
	da = dprow[0];                                          \
	da1 = dprow[1];                                         \
	da3 = vis_ld_d64_nf(dprow + 2);                         \
...
void
mlib_v_ImageZoomIn2X_S16_4BL_FL3(
    MLIB_ZOOMIN2XDEFPARAM_BL(mlib_s16))
{
/* indices */
	mlib_s32 i, j;
	...
	while (num_of_rows > 0) {
		...
		for (j = 0; j < (src_width / 2); j++) {
			...
			GET_STR_S16_4_BILINEAR(0, da, da1, da2, da3);
			prow += src_stride;

/* common case */
			if (((mlib_addr)pdst & 7) | (dst_stride & 3)) {
				if (src_height / 2 > 0) {
					GET_STR_S16_4_BILINEAR(0, dd, dd1, dd2,
					    dd3);
...
void
mlib_v_ImageZoomIn2X_S16_BL_FL0(
...
/* left bound of image */
	...
	if (i && j)
		...
		case 1:
			mlib_v_ImageZoomIn2X_S16_1BL_FL0
			    (MLIB_ZOOMIN2XCALLPARAM_BL(mlib_s16));
			break;
		case 2:
			mlib_v_ImageZoomIn2X_S16_2BL_FL0
			    (MLIB_ZOOMIN2XCALLPARAM_BL(mlib_s16));
...
void
mlib_v_ImageZoomIn2X_S16_BL_FL1(
    MLIB_ZOOMIN2XDEFPARAM(mlib_s16))
{
/* indices */
	mlib_s32 i, j, k;
...
void
mlib_v_ImageZoomIn2X_S16_BL_FL2(
    MLIB_ZOOMIN2XDEFPARAM(mlib_s16))
{
/* indices */
	mlib_s32 i, j, k;
...
void
mlib_v_ImageZoomIn2X_S16_BL_FL3(
    MLIB_ZOOMIN2XDEFPARAM(mlib_s16))
{
/* indices */
	mlib_s32 i, j, k;
...
#define	SUMV_A0(a, b)                                                   \
	vis_fpadd16(vis_fmul8x16(vis_read_hi(q), vis_fand(a, mask2)),   \
	    vis_fmul8x16(vis_read_lo(q), vis_fand(b, mask2)))

/* *********************************************************** */

#define	SUMV_B0(a, b)                                                   \
	vis_fpadd16(vis_fmul8x16(vis_read_lo(q), vis_fand(a, mask2)),   \
	    vis_fmul8x16(vis_read_hi(q), vis_fand(b, mask2)))

#define	SUMV_A1(a, b)	vis_fand(a, mask2)
...
FUNC_ZOOM_BL(S16, 2, 0, 0)
    FUNC_ZOOM_BL(S16, 2, 0, 1)
    FUNC_ZOOM_BL(S16, 2, 1, 0)
    FUNC_ZOOM_BL(S16, 4, 0, 0)
    FUNC_ZOOM_BL(S16, 4, 1, 0)
