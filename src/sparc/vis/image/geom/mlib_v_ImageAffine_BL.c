...
#pragma ident	"@(#)mlib_v_ImageAffine_BL.c	9.2	07/11/05 SMI"
...
static mlib_status FUN_NAME(2ch_na) (
    mlib_affine_param *param);
static mlib_status FUN_NAME(4ch_na) (
    mlib_affine_param *param);
...
#define	GET_FILTER_XY()                                                   \
	...
	filterx =                                                         \
	    *((mlib_d64 *)((mlib_u8 *)mlib_filters_u8_bl + filterpos));   \
...
#define	LOAD_1CH()                                                     \
	...
	s2 =                                                           \
	vis_fpmerge(LD_U8(sp0, srcYStride), LD_U8(sp2, srcYStride));   \
	s3 =                                                           \
	vis_fpmerge(LD_U8(sp0, srcYStride + 1), LD_U8(sp2,             \
	    srcYStride + 1));                                          \
	t0 = vis_fpmerge(LD_U8(sp1, 0), LD_U8(sp3, 0));                \
	t1 = vis_fpmerge(LD_U8(sp1, 1), LD_U8(sp3, 1));                \
	t2 =                                                           \
	vis_fpmerge(LD_U8(sp1, srcYStride), LD_U8(sp3, srcYStride));   \
	t3 =                                                           \
	vis_fpmerge(LD_U8(sp1, srcYStride + 1), LD_U8(sp3,             \
	    srcYStride + 1));                                          \
...
#define	PREPARE_DELTAS                                                     \
	if (warp_tbl != NULL) {                                            \
	    ...
	    dY = warp_tbl[2 * j + 1];                                      \
	    dx64 =                                                         \
		vis_to_double_dup((((dX << 1) & 0xFFFF) << 16) | ((dX <<   \
		1) & 0xFFFF));                                             \
	    dy64 =                                                         \
		vis_to_double_dup((((dY << 1) & 0xFFFF) << 16) | ((dY <<   \
		1) & 0xFFFF));                                             \
	}

/* *********************************************************** */

mlib_status FUN_NAME(1ch) (
    mlib_affine_param *param)
{
	DECLAREVAR();
	...
	dx64 =
	    vis_to_double_dup((((dX << 1) & 0xFFFF) << 16) | ((dX << 1) &
	    0xFFFF));
	dy64 =
	    vis_to_double_dup((((dY << 1) & 0xFFFF) << 16) | ((dY << 1) &
	    0xFFFF));
	...
	for (j = yStart; j <= yFinish; j++) {
		...
		if (off) {
			...
			s0 = vis_freg_pair(df, df);
			vis_pst_8(s0, (mlib_d64 *)((mlib_addr)dp & ~7),
			    emask << (4 - ((mlib_addr)dp & 7)));
			dp++;
...
#define	LOAD_2CH()                                                        \
	...
	s2 =                                                              \
	vis_bshuffle(LD_U16(sp0, srcYStride), LD_U16(sp1, srcYStride));   \
	s3 =                                                              \
	vis_bshuffle(LD_U16(sp0, srcYStride + 2), LD_U16(sp1,             \
	    srcYStride + 2))
...
#define	PREPARE_DELTAS                                            \
	if (warp_tbl != NULL) {                                   \
	    ...
	    dY = warp_tbl[2 * j + 1];                             \
	    dx64 =                                                \
		vis_to_double_dup(((dX & 0xFFFF) << 16) | (dX &   \
		0xFFFF));                                         \
	    dy64 =                                                \
		vis_to_double_dup(((dY & 0xFFFF) << 16) | (dY &   \
		0xFFFF));                                         \
	}

/* *********************************************************** */

mlib_status FUN_NAME(2ch) (
    mlib_affine_param *param)
{
	DECLAREVAR();
	mlib_d64 mask_7fff = vis_to_double_dup(0x7FFF7FFF);
	...
	if (((mlib_s32)lineAddr[0] | (mlib_s32)dstData | srcYStride |
	    dstYStride) & 1) {
...
#define	LOAD_2CH_NA()                                                  \
	...
	s2 =                                                           \
	vis_fpmerge(LD_U8(sp0, srcYStride), LD_U8(sp1, srcYStride));   \
	s3 =                                                           \
	vis_fpmerge(LD_U8(sp0, srcYStride + 2), LD_U8(sp1,             \
	    srcYStride + 2));                                          \
	t0 = vis_fpmerge(LD_U8(sp0, 1), LD_U8(sp1, 1));                \
	t1 = vis_fpmerge(LD_U8(sp0, 3), LD_U8(sp1, 3));                \
	t2 =                                                           \
	vis_fpmerge(LD_U8(sp0, srcYStride + 1), LD_U8(sp1,             \
	    srcYStride + 1));                                          \
	t3 =                                                           \
	vis_fpmerge(LD_U8(sp0, srcYStride + 3), LD_U8(sp1,             \
	    srcYStride + 3));                                          \
	s0 = vis_fpmerge(vis_read_lo(s0), vis_read_lo(t0));            \
	s1 = vis_fpmerge(vis_read_lo(s1), vis_read_lo(t1));            \
	s2 = vis_fpmerge(vis_read_lo(s2), vis_read_lo(t2));            \
    ...
    mlib_affine_param *param)
{
	...
	if (bsize > BUF_SIZE) {
		pbuff = __mlib_malloc(bsize * sizeof (mlib_f32));
    ...
    mlib_affine_param *param)
{
	...
	if (max_xsize > BUF_SIZE) {
		pbuff = __mlib_malloc(max_xsize * sizeof (mlib_f32));
...
#pragma pipeloop(0)
		for (i = 0; i < size; i++) {
			...
			d0 = vis_fmul8x16au(vis_read_hi(s0),
			    vis_read_hi(filterxy));
			d1 = vis_fmul8x16al(vis_read_lo(s0),
			    vis_read_hi(filterxy));

			vis_alignaddr(sp, srcYStride);
			sp1 = AL_ADDR(sp, srcYStride);
			s1 = vis_faligndata(sp1[0], sp1[1]);
			d2 = vis_fmul8x16au(vis_read_hi(s1),
			    vis_read_lo(filterxy));
			d3 = vis_fmul8x16al(vis_read_lo(s1),
			    vis_read_lo(filterxy));
		...
		mlib_v_ImageChannelExtract_U8_43L_D1((mlib_u8 *)pbuff, dl,
		    size);
	}
...
#pragma pipeloop(0)
		for (i = 0; i < size; i++) {
			...
			sp = *(mlib_f32 **)((mlib_u8 *)lineAddr +
			    PTR_SHIFT(Y)) + (X >> MLIB_SHIFT);
			s0 = sp[0];
			s1 = sp[1];
			s2 = sp[srcYStride];
			s3 = sp[srcYStride + 1];
...
#pragma pipeloop(0)
		for (i = 0; i < size; i++) {
			...
			PROCESS_4CH(vis_read_hi(s0), vis_read_lo(s0),
			    vis_read_hi(s1), vis_read_lo(s1));
    ...
    const void *colormap)
{
	...
	mlib_f32 *plut =
	    (mlib_f32 *)mlib_ImageGetLutNormalTable(colormap) - lut_off;
	mlib_s32 max_xsize = param->max_xsize;
	mlib_f32 buff[BUF_SIZE], *pbuff = buff;
...
#pragma pipeloop(0)
		for (i = 0; i < size; i++) {
			...
			sp = *(DTYPE **) ((mlib_u8 *)lineAddr + PTR_SHIFT(Y)) +
			    (X >> MLIB_SHIFT);
			s0 = LUT(sp[0]);
			s1 = LUT(sp[1]);
		...
		if (nchan == 3) {
			mlib_ImageColorTrue2IndexLine_U8_U8_3_in_4((void *)
			    pbuff, (void *)dl, size, colormap);
		} else {
			mlib_ImageColorTrue2IndexLine_U8_U8_4((void *)pbuff,
			    (void *)dl, size, colormap);
		}
	}
...
#pragma pipeloop(0)
		...
		if (nchan == 3) {
			mlib_ImageColorTrue2IndexLine_U8_S16_3_in_4((void *)
			    pbuff, (void *)dl, size, colormap);
		} else {
			mlib_ImageColorTrue2IndexLine_U8_S16_4((void *)pbuff,
			    (void *)dl, size, colormap);
		}
	}
