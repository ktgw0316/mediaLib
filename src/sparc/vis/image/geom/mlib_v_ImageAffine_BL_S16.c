...
#pragma ident	"@(#)mlib_v_ImageAffine_BL_S16.c	9.2	07/11/05 SMI"
...
#define	FMUL_16x16(x, y)                                          \
	vis_fpadd16(vis_fmul8sux16(y, x), vis_fmul8ulx16(y, x))
...
#define	DOUBLE_4U16(x0, x1, x2, x3)                                    \
	vis_to_double(((((x0) & 0xFFFE) << 15) | (((x1) & 0xFFFE) >>   \
...
#define	LOAD_1CH()                                                        \
	...
	s2 =                                                              \
	vis_bshuffle(LD_U16(sp0, srcYStride), LD_U16(sp2, srcYStride));   \
	s3 =                                                              \
	vis_bshuffle(LD_U16(sp0, srcYStride + 2), LD_U16(sp2,             \
	    srcYStride + 2));                                             \
	t0 = vis_bshuffle(LD_U16(sp1, 0), LD_U16(sp3, 0));                \
	t1 = vis_bshuffle(LD_U16(sp1, 2), LD_U16(sp3, 2));                \
	t2 =                                                              \
	vis_bshuffle(LD_U16(sp1, srcYStride), LD_U16(sp3, srcYStride));   \
	t3 =                                                              \
	vis_bshuffle(LD_U16(sp1, srcYStride + 2), LD_U16(sp3,             \
	    srcYStride + 2));                                             \
...
#pragma pipeloop(0)
		...
		if (off) {
			...
			vis_pst_16(dd, dp + i, (0xF0 >> off) & 0x0F);
		}
	}
...
#undef  GET_POINTER
#define	GET_POINTER(sp)                                             \
	sp = *(mlib_f32 **)((mlib_u8 *)lineAddr + PTR_SHIFT(Y)) +   \
	    (X >> MLIB_SHIFT);                                      \
	X += dX;                                                    \
	Y += dY
...
#define	LOAD_2CH_NA()                                                     \
	...
	s3 =                                                              \
	vis_bshuffle(LD_U16(sp0, srcYStride + 4), LD_U16(sp1,             \
	    srcYStride + 4));                                             \
	...
	vis_bshuffle(LD_U16(sp0, srcYStride + 2), LD_U16(sp1,             \
	    srcYStride + 2));                                             \
	t3 =                                                              \
	vis_bshuffle(LD_U16(sp0, srcYStride + 6), LD_U16(sp1,             \
	    srcYStride + 6));                                             \
	s0 = vis_bshuffle(s0, t0);                                        \
	s1 = vis_bshuffle(s1, t1);                                        \
	s2 = vis_bshuffle(s2, t2);                                        \
    ...
    mlib_affine_param *param)
{
	...
	if (bsize > BUF_SIZE) {
		pbuff = __mlib_malloc(bsize * sizeof (mlib_d64));
    ...
    mlib_affine_param *param)
{
	...
	if (max_xsize > BUF_SIZE) {
		pbuff = __mlib_malloc(max_xsize * sizeof (mlib_d64));
...
#pragma pipeloop(0)
		...
		mlib_v_ImageChannelExtract_S16_43L_D1((void *)pbuff, (void *)dl,
		    size);
	}
    ...
    const void *colormap)
{
	...
	mlib_d64 *plut =
	    (mlib_d64 *)mlib_ImageGetLutNormalTable(colormap) - lut_off;
	mlib_s32 max_xsize = param->max_xsize;
	mlib_d64 buff[BUF_SIZE], *pbuff = buff;
...
#pragma pipeloop(0)
		...
		if (nchan == 3) {
			mlib_ImageColorTrue2IndexLine_S16_S16_3_in_4((void *)
			    pbuff, (void *)dl, size, colormap);
		} else {
			mlib_ImageColorTrue2IndexLine_S16_S16_4((void *)pbuff,
...
#pragma pipeloop(0)
		...
		if (nchan == 3) {
			mlib_ImageColorTrue2IndexLine_S16_U8_3_in_4((void *)
			    pbuff, (void *)dl, size, colormap);
		} else {
			mlib_ImageColorTrue2IndexLine_S16_U8_4((void *)pbuff,
			    (void *)dl, size, colormap);
		}
	}
