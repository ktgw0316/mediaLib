...
#pragma pipeloop(0)
		for (i = 0; i <= (size - 4); i += 4) {
			...
			ss = vis_faligndata(vis_ld_u16_i(sp3,
			    SHIFT1(X + 3 * dX)), ss);
			ss = vis_faligndata(vis_ld_u16_i(sp2,
			    SHIFT1(X + 2 * dX)), ss);
			ss = vis_faligndata(vis_ld_u16_i(sp1, SHIFT1(X + dX)),
			    ss);
    ...
    mlib_affine_param *param)
{
	...
	if (max_xsize > BUFF_SIZE) {
		pbuff = __mlib_malloc(sizeof (mlib_d64) * max_xsize);
	}
...
#pragma pipeloop(0)
		...
		if ((mlib_s32)dp & 7) {
			mlib_ImageCopy_na((mlib_u8 *)buff, (mlib_u8 *)dp,
			    8 * size);
		}
	}
