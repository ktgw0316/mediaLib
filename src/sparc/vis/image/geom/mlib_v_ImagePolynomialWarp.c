...
#pragma ident	"@(#)mlib_v_ImagePolynomialWarp.c	9.2	07/11/05 SMI"
...
/* check for obvious errors */
	...
	memory = __mlib_malloc(sizeof (mlib_s32) * 3 * len +
	...
	if (type == MLIB_BYTE) {
		pws.filter_table = (void *)mlib_filters_u8_bc;
		pws.filter_table_vis = (void *)mlib_filters_u8_bc_3;

		if (channels == 4)
			pws.filter_table_vis = (void *)mlib_filters_u8_bc_4;
	} else {
	...
	if (filter == MLIB_BICUBIC2) {
		...
		if (type == MLIB_BYTE) {
			pws.filter_table = (void *)mlib_filters_u8_bc2;
			pws.filter_table_vis = (void *)mlib_filters_u8_bc2_3;
