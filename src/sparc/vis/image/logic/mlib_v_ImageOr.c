...
#pragma ident	"@(#)mlib_v_ImageOr.c	9.2	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_ImageOr       - or two images     (VIS version)
 *
 * SYNOPSIS
 ...
 *
 * DESCRIPTION
 *      Or two images for each band:      dst = src1 | src2
 */
...
#define	VIS_LOGIC(a1, a2)	vis_for(a1, a2)
    ...
    const mlib_image *src2)
{
	...
	if ((mlib_ImageGetType(src1) == MLIB_BYTE) ||
		...
		if ((mlib_ImageGetBitOffset(src1) ==
		    ...
		    mlib_ImageGetBitOffset(dst))) {
			return (mlib_v_ImageOr_Bit(dst, src1, src2));
		} else {
