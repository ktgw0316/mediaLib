...
#pragma ident	"@(#)mlib_v_ImageConstNotAnd.c	9.2	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_ImageConstNotAnd - image logical operation with constant
 *
 * SYNOPSIS
 *      mlib_status mlib_ImageConstNotAnd(mlib_image       *dst,
...
#pragma weak mlib_ImageConstNotAnd = __mlib_ImageConstNotAnd

#elif defined(__GNUC__)	/* defined(__SUNPRO_C) */
__typeof__(__mlib_ImageConstNotAnd) mlib_ImageConstNotAnd
    __attribute__((weak, alias("__mlib_ImageConstNotAnd")));
...
#define	VIS_CONSTLOGIC(c, a)	vis_fnand(a, c)
    ...
    const mlib_s32 *c)
{
	...
	if ((mlib_ImageGetType(src) == MLIB_BYTE) ||
		...
		if (mlib_ImageGetBitOffset(src) ==
			...
			return (mlib_v_ImageConstNotAnd_Bit(dst, src, c));
		} else {
			return (mlib_ImageConstNotAnd_Bit(dst, src, c));
		}
