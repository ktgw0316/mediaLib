...
#pragma ident	"@(#)mlib_v_ImageAnd.c	9.2	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_ImageAnd     - and two images  (C version)
 *
 * SYNOPSIS
 *      mlib_status mlib_ImageAnd(mlib_image       *dst,
...
#pragma weak mlib_ImageAnd = __mlib_ImageAnd

#elif defined(__GNUC__)	/* defined(__SUNPRO_C) */
__typeof__(__mlib_ImageAnd) mlib_ImageAnd
    __attribute__((weak, alias("__mlib_ImageAnd")));
...
#define	VIS_LOGIC(a1, a2)	vis_fand(a1, a2)
    ...
    const mlib_image *src2)
{
	...
	if ((mlib_ImageGetType(src1) == MLIB_BYTE) ||
		...
		if ((mlib_ImageGetBitOffset(src1) ==
		    ...
		    mlib_ImageGetBitOffset(dst))) {
			return (mlib_v_ImageAnd_Bit(dst, src1, src2));
		} else {
			return (mlib_ImageAnd_Bit(dst, src1, src2));
		}
