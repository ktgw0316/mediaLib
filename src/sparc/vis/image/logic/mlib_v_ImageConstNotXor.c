...
#pragma ident	"@(#)mlib_v_ImageConstNotXor.c	9.2	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_ImageConstNotXor - image logical operation with constant
 *
 * SYNOPSIS
 *      mlib_status mlib_ImageConstNotXor(mlib_image       *dst,
...
#pragma weak mlib_ImageConstNotXor = __mlib_ImageConstNotXor

#elif defined(__GNUC__)	/* defined(__SUNPRO_C) */
__typeof__(__mlib_ImageConstNotXor) mlib_ImageConstNotXor
    __attribute__((weak, alias("__mlib_ImageConstNotXor")));
...
#define	VIS_CONSTLOGIC(c, a)	vis_fxnor(a, c)
    ...
    const mlib_s32 *c)
{
	...
	if ((mlib_ImageGetType(src) == MLIB_BYTE) ||
		...
		if (mlib_ImageGetBitOffset(src) ==
			...
			return (mlib_v_ImageConstNotXor_Bit(dst, src, c));
		} else {
			return (mlib_ImageConstNotXor_Bit(dst, src, c));
		}
