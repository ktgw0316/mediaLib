...
#pragma ident	"@(#)mlib_v_ImageConstOr.c	9.2	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_ImageConstOr - image logical operation with constant
 *
 * SYNOPSIS
 *      mlib_status mlib_ImageConstOr(mlib_image       *dst,
...
#pragma weak mlib_ImageConstOr = __mlib_ImageConstOr

#elif defined(__GNUC__)	/* defined(__SUNPRO_C) */
__typeof__(__mlib_ImageConstOr) mlib_ImageConstOr
    __attribute__((weak, alias("__mlib_ImageConstOr")));
...
#define	VIS_CONSTLOGIC(c, a)	vis_for(a, c)
    ...
    const mlib_s32 *c)
{
	...
	if ((mlib_ImageGetType(src) == MLIB_BYTE) ||
		...
		if (mlib_ImageGetBitOffset(src) ==
			...
			return (mlib_v_ImageConstOr_Bit(dst, src, c));
		} else {
			return (mlib_ImageConstOr_Bit(dst, src, c));
		}
