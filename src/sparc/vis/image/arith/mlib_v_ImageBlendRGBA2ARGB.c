...
#pragma ident	"@(#)mlib_v_ImageBlendRGBA2ARGB.c	9.2	07/11/05 SMI"
...
#pragma weak mlib_ImageBlendRGBA2ARGB = __mlib_ImageBlendRGBA2ARGB

#elif defined(__GNUC__)	/* defined(__SUNPRO_C) */
__typeof__(__mlib_ImageBlendRGBA2ARGB) mlib_ImageBlendRGBA2ARGB
    __attribute__((weak, alias("__mlib_ImageBlendRGBA2ARGB")));
...
#pragma pipeloop(0)
		for (i = 0; i < (width / 2); i++) {
			aa = sp[i];
			ss = vis_faligndata(aa, aa);
			dd = dp[i];
			BLEND(d_h, vis_read_hi(aa), vis_read_hi(ss),
			    vis_read_hi(dd));
			BLEND(d_l, vis_read_lo(aa), vis_read_lo(ss),
			    vis_read_lo(dd));
			dd = vis_fpack16_pair(d_h, d_l);
			vis_pst_8(dd, dp + i, 0x77);
		}
