...
#pragma ident	"@(#)mlib_v_ImageConvMxN_8ext.c	9.2	07/11/05 SMI"
...
static mlib_status mlib_convMxN_8ext_f(
    mlib_image *dst,
...
static mlib_status mlib_convMxN_8ext_mask(
    mlib_image *dst,
    const mlib_image *src,
    mlib_s32 m,
    mlib_s32 n,
    ...
    mlib_s32 cmask)
{
	...
	if ((cmask & amask) == amask) {
		return mlib_convMxN_8ext_f(dst, src, kwid, khgt, dx_l, dx_r,
		    dy_t, dy_b, kernel, discardbits);
	} else {
		return mlib_convMxN_8ext_mask(dst, src, kwid, khgt, dx_l, dx_r,
		    dy_t, dy_b, kernel, discardbits, cmask);
	}
}
    ...
    mlib_s32 scale)
{
	...
	for (l = 0; l < n; l++) {
		...
		mlib_ImageCopy_na((mlib_u8 *)sl,
		    (mlib_u8 *)buffn + dx_l * nchan, ssize);

		for (i = 0; i < nchan; i++) {
			for (ii = 0; ii < dx_l; ii++) {
				*((mlib_u8 *)buffn + i + nchan * ii) =
				    *((mlib_u8 *)buffn + i + nchan * dx_l);
			}
		}

		for (i = 0; i < nchan; i++) {
			for (ii = 0; ii < dx_r; ii++) {
				*((mlib_u8 *)buffn + i + nchan * ii + ssize +
				    dx_l * nchan) =
				    *((mlib_u8 *)buffn + i + nchan * (dx_l -
				    1) + ssize);
			}
    ...
    mlib_s32 cmask)
{
	...
	for (chan = 0; chan < nchan; chan++) {
		...
		for (l = 0; l < n; l++) {
			...
			(*p_proc_load) ((mlib_u8 *)sl, (mlib_u8 *)buffn + dx_l,
			    ssize, testchan);
...
#pragma pipeloop(0)
				for (i = 0; i < (xsize + 7) / 8; i++) {
					...
					d21 =
					    vis_fmul8x16au(vis_read_lo(s2), k2);
					d30 =
					    vis_fmul8x16au(vis_read_hi(s3), k3);
					d31 =
					    vis_fmul8x16au(vis_read_lo(s3), k3);
			...
			(*p_proc_store) ((mlib_u8 *)buffe, (mlib_u8 *)dl, xsize,
			    testchan);
