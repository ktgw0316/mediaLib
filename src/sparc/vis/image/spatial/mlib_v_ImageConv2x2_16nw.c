...
#pragma ident	"@(#)mlib_v_ImageConv2x2_16nw.c	9.3	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_conv2x2_16nw.c - convolve an 16-bit image with a 2x2 kernel
 *                      (edge = no write)
 *
 *
 * SYNOPSIS
...
static mlib_status mlib_v_conv2x2_16nw_1(
    mlib_image *dst,
    const mlib_image *src,
    const mlib_s32 *kernel,
    mlib_s32 scalef_expon);

static mlib_status mlib_v_conv2x2_16nw_2(
    mlib_image *dst,
    const mlib_image *src,
    const mlib_s32 *kernel,
    mlib_s32 scalef_expon);

static mlib_status mlib_v_conv2x2_16nw_3(
    mlib_image *dst,
    const mlib_image *src,
    const mlib_s32 *kernel,
    mlib_s32 scalef_expon);

static mlib_status mlib_v_conv2x2_16nw_4(
    mlib_image *dst,
    const mlib_image *src,
    const mlib_s32 *kernel,
    mlib_s32 scalef_expon);

static mlib_status mlib_v_conv2x2_16nw_mask(
    mlib_image *dst,
    const mlib_image *src,
    const mlib_s32 *kernel,
    mlib_s32 scalef_expon,
    ...
    mlib_s32 cmask)
{
	...
	if (mlib_ImageConvVersion(2, 2, scalef_expon, MLIB_SHORT) == 0)
		return mlib_c_conv2x2nw_s16(dst, src, kernel, scalef_expon,
		    cmask);
	...
	if ((chan == nchannel)) {
		...
		case 1:
			res =
			    mlib_v_conv2x2_16nw_1(dst, src, kernel,
			    scalef_expon);
			break;
		case 2:
			res =
			    mlib_v_conv2x2_16nw_2(dst, src, kernel,
			    scalef_expon);
			break;
		case 3:
			res =
			    mlib_v_conv2x2_16nw_3(dst, src, kernel,
			    scalef_expon);
			break;
		case 4:
			res =
			    mlib_v_conv2x2_16nw_4(dst, src, kernel,
			    scalef_expon);
			break;
		default:
			res = MLIB_FAILURE;
		...
		res =
		    mlib_v_conv2x2_16nw_mask(dst, src, kernel, scalef_expon,
		    cmask);
	return (res);
}
