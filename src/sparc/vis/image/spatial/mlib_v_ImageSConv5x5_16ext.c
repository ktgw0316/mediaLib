...
#pragma ident	"@(#)mlib_v_ImageSConv5x5_16ext.c	9.3	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_sconv5x5_16ext - separable convolve of an image with a 5x5 kernel
 *                              edge = MLIB_EDGE_SRC_EXTEND
 *
 * SYNOPSIS
 *      mlib_status mlib_sconv5x5ext_s16(mlib_image       *dst,
...
static mlib_status mlib_v_sconv5x5_16ext_1(
    mlib_image *dst,
    const mlib_image *src,
    mlib_s32 dx_l,
    mlib_s32 dx_r,
...
static mlib_status mlib_v_sconv5x5_16ext_2(
    mlib_image *dst,
    const mlib_image *src,
    mlib_s32 dx_l,
    mlib_s32 dx_r,
...
static mlib_status mlib_v_sconv5x5_16ext_3(
    mlib_image *dst,
    const mlib_image *src,
    mlib_s32 dx_l,
    mlib_s32 dx_r,
...
static mlib_status mlib_v_sconv5x5_16ext_4(
    mlib_image *dst,
    const mlib_image *src,
    mlib_s32 dx_l,
    mlib_s32 dx_r,
...
static mlib_status mlib_v_sconv5x5_16ext_mask(
    mlib_image *dst,
    const mlib_image *src,
    mlib_s32 dx_l,
    mlib_s32 dx_r,
    ...
    mlib_s32 cmask)
{
	...
	if ((nchannel == chan) || ((nchannel == 4) && (chan == 3))) {
		...
		case 1:
			res =
			    mlib_v_sconv5x5_16ext_1(dst, src, dx_l, dx_r, dy_t,
			    dy_b, hkernel, vkernel, scalef_expon);
			break;
		case 2:
			res =
			    mlib_v_sconv5x5_16ext_2(dst, src, dx_l, dx_r, dy_t,
			    dy_b, hkernel, vkernel, scalef_expon);
			break;
		case 3:
			res =
			    mlib_v_sconv5x5_16ext_3(dst, src, dx_l, dx_r, dy_t,
			    dy_b, hkernel, vkernel, scalef_expon);
			break;
		case 4:
			res =
			    mlib_v_sconv5x5_16ext_4(dst, src, dx_l, dx_r, dy_t,
		...
		res =
		    mlib_v_sconv5x5_16ext_mask(dst, src, dx_l, dx_r, dy_t, dy_b,
...
#pragma pipeloop(0)
		...
		for (i = 0; i < dx_r; i++) {
			*(mlib_u16 *)((mlib_u16 *)sbuf5 + dw + LEFT + RIGHT -
			    dx_r + i) =
			    *(mlib_u16 *)((mlib_u16 *)sbuf5 + dw + LEFT +
			    RIGHT - dx_r - 1);
		}
...
#pragma pipeloop(0)
		...
		for (i = 0; i < dx_r; i++) {
			*(mlib_f32 *)((mlib_f32 *)sbuf5 + (dw >> 1) + (LEFT +
			    RIGHT - dx_r) + i) =
			    *(mlib_f32 *)((mlib_f32 *)sbuf5 + (dw >> 1) - 1 +
			    (LEFT + RIGHT - dx_r));
		}
...
#pragma pipeloop(0)
		...
		for (i = 0; i < 3 * dx_r; i++) {
			*(mlib_u16 *)((mlib_u16 *)sbuf5 + dw + 3 * (LEFT +
			    RIGHT - dx_r) + i) =
			    *(mlib_u16 *)((mlib_u16 *)sbuf5 + dw - 3 +
			    3 * (LEFT + RIGHT - dx_r) + i);
		}
...
/* pointers to rows in interm. src buf */
	mlib_u16 *sbuf4, *sbuf5, *sbuf6, *s_buf1;
...
/* alloc. interm. src and dst buffer */
	buff_src =
	    (mlib_u16 *)__mlib_malloc(6 * buf_slb * sizeof (mlib_u8) + 16);
...
/* first iteration - no write data to dst (only in union) */
			t1 = vis_ld_u16_nf(sa_5);
			sa_5 += nchannel;
			t2 = vis_ld_u16_nf(sa_5);
			sa_5 += nchannel;
			t3 = vis_ld_u16_nf(sa_5);
			sa_5 += nchannel;
			t4 = vis_ld_u16_nf(sa_5);
			sa_5 += nchannel;
			(*s_buf1++) = t1;
			(*s_buf1++) = t2;
			(*s_buf1++) = t3;
