...
#pragma ident	"@(#)mlib_v_ImageSConv5x5_8ext.c	9.2	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_sconv5x5_8ext - separable convolve of an image with a 5x5 kernel
 *                             edge = MLIB_EDGE_SRC_EXTEND
 *
 * SYNOPSIS
 *      mlib_status mlib_sconv5x5ext_u8(mlib_image       *dst,
...
static mlib_status mlib_v_sconv5x5_8ext_1(
    mlib_image *dst,
    const mlib_image *src,
    mlib_s32 dx_l,
    mlib_s32 dx_r,
...
static mlib_status mlib_v_sconv5x5_8ext_2(
    mlib_image *dst,
    const mlib_image *src,
    mlib_s32 dx_l,
    mlib_s32 dx_r,
...
static mlib_status mlib_v_sconv5x5_8ext_3(
    mlib_image *dst,
    const mlib_image *src,
    mlib_s32 dx_l,
    mlib_s32 dx_r,
...
static mlib_status mlib_v_sconv5x5_8ext_4(
    mlib_image *dst,
    const mlib_image *src,
    mlib_s32 dx_l,
    mlib_s32 dx_r,
...
static mlib_status mlib_v_sconv5x5_8ext_mask(
    mlib_image *dst,
    const mlib_image *src,
    mlib_s32 dx_l,
    mlib_s32 dx_r,
    ...
    mlib_s32 cmask)
{
	...
	if ((nchannel == chan) || ((nchannel == 4) && (chan == 3))) {
		...
		case 1:
			res =
			    mlib_v_sconv5x5_8ext_1(dst, src, dx_l, dx_r, dy_t,
			    dy_b, hkernel, vkernel, scalef_expon);
			break;
		case 2:
			res =
			    mlib_v_sconv5x5_8ext_2(dst, src, dx_l, dx_r, dy_t,
			    dy_b, hkernel, vkernel, scalef_expon);
			break;
		case 3:
			res =
			    mlib_v_sconv5x5_8ext_3(dst, src, dx_l, dx_r, dy_t,
			    dy_b, hkernel, vkernel, scalef_expon);
			break;
		case 4:
			res =
			    mlib_v_sconv5x5_8ext_4(dst, src, dx_l, dx_r, dy_t,
		...
		res =
		    mlib_v_sconv5x5_8ext_mask(dst, src, dx_l, dx_r, dy_t, dy_b,
...
#define	VCONV(dsrc0, dsrc1, dsrc2, dsrc3, dsrc4, sum0, sum1)    \
...
#pragma pipeloop(0)
		...
		for (i = 0; i < dx_r; i++) {
			*(mlib_u8 *)((mlib_u8 *)sbuf5 + dw + LEFT + RIGHT -
			    dx_r + i) =
			    *(mlib_u8 *)((mlib_u8 *)sbuf5 + dw - 1 + LEFT +
			    RIGHT - dx_r);
		}
		...
		d4 = s4[0];
		d5 = s5[0];
		VCONV(d1, d2, d3, d4, d5, acc0, acc1);
		d1 = s1[1];
		d2 = s2[1];
		d3 = s3[1];
		d4 = s4[1];
...
#pragma pipeloop(0)
		for (i = 0; i < dw; i += 8) {
			VCONV(d1, d2, d3, d4, d5, acc2, acc3);
...
#pragma pipeloop(0)
		...
		for (i = 0; i < dx_r; i++) {
			*(mlib_u16 *)((mlib_u16 *)sbuf5 + (dw >> 1) + LEFT +
			    RIGHT - dx_r + i) =
			    *(mlib_u16 *)((mlib_u16 *)sbuf5 + (dw >> 1) - 1 +
			    LEFT + RIGHT - dx_r);
		}
...
#pragma pipeloop(0)
		...
		for (i = 0; i < 3 * dx_r; i++) {
			*(mlib_u8 *)((mlib_u8 *)sbuf5 + dw + 3 * (LEFT + RIGHT -
			    dx_r) + i) =
			    *(mlib_u8 *)((mlib_u8 *)sbuf5 + dw - 3 + 3 * (LEFT +
			    RIGHT - dx_r) + i);
		}
...
#pragma pipeloop(0)
		for (i = 0; i < dw; i += 8) {
			VCONV(d1, d2, d3, d4, d5, acc4, acc5);
			HCONV_START(acc0, acc1);
			HCONV_SRC1(src1_shift1, h1);
			HCONV_SRC2(src2_shift1, h1);
			HCONV_SRC1(src1_shift2, h2);
...
#pragma pipeloop(0)
		...
		for (i = 0; i < dx_r; i++) {
			*(mlib_f32 *)((mlib_f32 *)sbuf5 + (dw >> 2) + (LEFT +
			    RIGHT - dx_r) + i) =
			    *(mlib_f32 *)((mlib_f32 *)sbuf5 + (dw >> 2) - 1 +
			    (LEFT + RIGHT - dx_r));
		}
...
/* pointers to src rows */
	mlib_u8 *sa, *sa1, *sa2, *sa3, *sa4, *sa_5, *sa5;

/* pointers to rows in interm. src buf */
	mlib_u8 *buff_src, *sbuf1, *sbuf2, *sbuf3, *prow;

/* pointers to rows in interm. src buf */
	mlib_u8 *sbuf4, *sbuf5, *s_buf1;
...
/* alloc. interm. src and dst buffer */
	buff_src1 = (mlib_d64 *)__mlib_malloc(6 * buf_slb * sizeof (mlib_d64));
...
#pragma pipeloop(0)
			for (i = 0; i < dw; i += 8) {
				t0 = vis_ld_u8_nf(sa_5);
				sa_5 += nchannel;
				t1 = vis_ld_u8_nf(sa_5);
				sa_5 += nchannel;
				t2 = vis_ld_u8_nf(sa_5);
				sa_5 += nchannel;
				t3 = vis_ld_u8_nf(sa_5);
				sa_5 += nchannel;
				t4 = vis_ld_u8_nf(sa_5);
				sa_5 += nchannel;
				t5 = vis_ld_u8_nf(sa_5);
				sa_5 += nchannel;
				t6 = vis_ld_u8_nf(sa_5);
				sa_5 += nchannel;
				t7 = vis_ld_u8_nf(sa_5);
				sa_5 += nchannel;
				VCONV(d1, d2, d3, d4, d5, acc2, acc3);
				(*s_buf1++) = t0;
				HCONV_START(acc0, acc1);
