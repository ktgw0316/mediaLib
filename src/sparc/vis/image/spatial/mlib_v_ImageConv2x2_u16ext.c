...
#pragma ident	"@(#)mlib_v_ImageConv2x2_u16ext.c	9.3	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_conv2x2ext_u16 - convolve an 16-bit image with a 2x2 kernel
 *                            (edge = src extended)
 *
 * SYNOPSIS
 *      mlib_statu mlib_conv2x2ext_u16(mlib_image       *dst,
...
static mlib_status mlib_v_conv2x2_u16ext_1(
    mlib_image *dst,
    const mlib_image *src,
    mlib_s32 dx_l,
    mlib_s32 dx_r,
...
static mlib_status mlib_v_conv2x2_u16ext_2(
    mlib_image *dst,
    const mlib_image *src,
    mlib_s32 dx_l,
    mlib_s32 dx_r,
...
static mlib_status mlib_v_conv2x2_u16ext_3(
    mlib_image *dst,
    const mlib_image *src,
    mlib_s32 dx_l,
    mlib_s32 dx_r,
...
static mlib_status mlib_v_conv2x2_u16ext_4(
    mlib_image *dst,
    const mlib_image *src,
    mlib_s32 dx_l,
    mlib_s32 dx_r,
...
static mlib_status mlib_v_conv2x2_u16ext_mask(
    mlib_image *dst,
    const mlib_image *src,
    mlib_s32 dx_l,
    mlib_s32 dx_r,
...
#define	LOAD_LINE_INTO_BUFFER(n, end)                                   \
	...
	if (!((mlib_s32)dsa & 7)) {                                     \
		...
		mlib_ImageCopy_na((void*)dsa, (void*)prow, 2*(dw + n)); \
	}
...
#pragma pipeloop(0)
	LOAD_LINE_INTO_BUFFER(delta, 2 * end);
	j++;
    ...
    mlib_s32 cmask)
{
	...
	if (mlib_ImageConvVersion(2, 2, scalef_expon, MLIB_USHORT) == 0)
		return mlib_c_conv2x2ext_u16(dst, src, dx_l, dx_r, dy_t, dy_b,
	...
	if ((chan == nchannel)) {
		...
		case 1:
			res =
			    mlib_v_conv2x2_u16ext_1(dst, src, dx_l, dx_r, dy_t,
			    dy_b, kernel, scalef_expon);
			break;
		case 2:
			res =
			    mlib_v_conv2x2_u16ext_2(dst, src, dx_l, dx_r, dy_t,
			    dy_b, kernel, scalef_expon);
			break;
		case 3:
			res =
			    mlib_v_conv2x2_u16ext_3(dst, src, dx_l, dx_r, dy_t,
			    dy_b, kernel, scalef_expon);
			break;
		case 4:
			res =
			    mlib_v_conv2x2_u16ext_4(dst, src, dx_l, dx_r, dy_t,
			    dy_b, kernel, scalef_expon);
			break;
		default:
			res = MLIB_FAILURE;
		...
		res =
		    mlib_v_conv2x2_u16ext_mask(dst, src, dx_l, dx_r, dy_t, dy_b,
		    kernel, scalef_expon, cmask);
	return (res);
}
