...
#pragma ident	"@(#)mlib_v_ImageConv7x7_8nw.c	9.3	07/11/05 SMI"
...
/* pointers to src rows */
	mlib_u8 *sa, *sa2, *sa3, *sa4, *sa5, *sa6, *sa7, *prow;
...
/* shifted data */
	...
	if ((cmask & amask) == amask) {
		return mlib_convMxN_8nw_f(dst, src, 7, 7, 3, 3, kernel,
		    scalef_expon);
	}
...
/* load interm. src buff */
		...
		(*p_proc_load) (sa, sbuf2, dw + 6, testchan);
		(*p_proc_load) (sa2, sbuf3, dw + 6, testchan);
		(*p_proc_load) (sa3, sbuf4, dw + 6, testchan);
		(*p_proc_load) (sa4, sbuf5, dw + 6, testchan);
		(*p_proc_load) (sa5, sbuf6, dw + 6, testchan);
		(*p_proc_load) (sa6, sbuf7, dw + 6, testchan);

		for (j = 0; j < dh; j++) {
/* load next row */
			(*p_proc_load) (sa7, sbuf1, dw + 6, testchan);
			ddst = (mlib_d64 *)(dbuf);
			prow = sbuf1;
			sbuf1 = sbuf2;
			sbuf2 = sbuf3;
