...
static mlib_status mlib_convMxN_8nw_mask(
    mlib_image *dst,
    const mlib_image *src,
    mlib_s32 m,
    mlib_s32 n,
    ...
    mlib_s32 cmask)
{
	...
	if ((cmask & amask) == amask) {
		return mlib_convMxN_8nw_f(dst, src, kwid, khgt, khw, khh,
		    kernel, discardbits);
	} else {
		return mlib_convMxN_8nw_mask(dst, src, kwid, khgt, khw, khh,
		    kernel, discardbits, cmask);
	}
}
    ...
    mlib_s32 cmask)
{
	...
	for (chan = 0; chan < nchan; chan++) {
		...
		for (l = 0; l < n; l++) {
			...
			(*p_proc_load) ((mlib_u8 *)sp, (mlib_u8 *)buffn, ssize,
			    testchan);
		}
