    ...
    mlib_s32 cmask)
{
	...
	mlib_d64 pix1hi, pix1lo, pix1, pix2, sum1, sum2;
...
#pragma pipeloop(0)
		for (i = 0; i < (dst_width >> 1); i++) {
			...
			alpha1 = mlib_v_blend_alpha64[psrc1[8 * i + off]];
			...
			alpha2 = mlib_v_blend_alpha64[psrc1[8 * i + 4 + off]];
		...
		if (dst_width & 1) {
			...
			alpha1 = mlib_v_blend_alpha64[psrc1[i * 8 + off]];
			sum1 =
