...
#pragma ident	"@(#)mlib_v_ImageBlend_ZERO_OMDC.c	9.2	07/11/05 SMI"

/*
 * FUNCTION
 *      mlib_ImageBlend_ZERO_OMDC - blend two images with
 *      OMDC source blend factor and ZERO destination blend factor
 *
 * SYNOPSIS
 *
 *      mlib_status mlib_ImageBlend_ZERO_OMDC(mlib_image       *dst,
...
#pragma weak mlib_ImageBlend_ZERO_OMDC = __mlib_ImageBlend_ZERO_OMDC

#elif defined(__GNUC__)	/* defined(__SUNPRO_C) */
__typeof__(__mlib_ImageBlend_ZERO_OMDC) mlib_ImageBlend_ZERO_OMDC
    __attribute__((weak, alias("__mlib_ImageBlend_ZERO_OMDC")));
...
#define	BLEND_ZERO_OMDC                                         \
	sum1 = vis_fexpand(vis_read_hi(pix1));                  \
	res1 = vis_fpsub16(d256, sum1);                         \
	res1 = vis_fmul8x16(vis_read_hi(pix2), res1);           \
	sum2 = vis_fexpand(vis_read_lo(pix1));                  \
	res2 = vis_fpsub16(d256, sum2);                         \
	res2 = vis_fmul8x16(vis_read_lo(pix2), res2);           \
	res = vis_fpack16_pair(res1, res2)

/* *********************************************************** */

mlib_status
__mlib_ImageBlend_ZERO_OMDC(
    mlib_image *dst,
    const mlib_image *src1,
    const mlib_image *src2,
    mlib_s32 cmask)
{
	...
	for (j = 0; j < dst_height; j++) {
		...
		len = (mlib_u8 *)pdstend + 1 - (mlib_u8 *)dpdst;
...
#pragma pipeloop(0)
			...
			if ((mlib_u8 *)dpdst <= (mlib_u8 *)pdstend) {
...
/* src1, src2 the some alignment */
				...
				if (off) {
					mask = vis_edge8(pdst, pdstend);
					tmp0 = vis_ld_d64_nf(dpsrc1); dpsrc1++;
					tmp1 = vis_ld_d64_nf(dpsrc1);
					pix1 = vis_faligndata(tmp0, tmp1);
					tmp2 = vis_ld_d64_nf(dpsrc2); dpsrc2++;
