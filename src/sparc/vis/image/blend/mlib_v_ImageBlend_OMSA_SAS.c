...
#pragma ident	"@(#)mlib_v_ImageBlend_OMSA_SAS.c	9.2	07/11/05 SMI"
...
#pragma pipeloop(0)
		for (i = 0; i < (dst_width >> 1); i++) {
			...
			res1 =
			    vis_fpadd16(vis_fmul8x16(vis_read_hi(pix1), alpha2),
			    vis_fmul8x16(vis_read_hi(pix2), alpha1));

			min =
			    MIN(256 - psrc1[8 * i + 4 + off],
			    psrc2[8 * i + 4 + off]);
			alpha3 = table64[min];
			alpha4 =
			    mlib_v_blend_alpha64[256 - psrc2[8 * i + 4 + off]];
			res2 =
			    vis_fpadd16(vis_fmul8x16(vis_read_lo(pix1), alpha4),
			    vis_fmul8x16(vis_read_lo(pix2), alpha3));
