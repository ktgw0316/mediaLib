...
#define	MUL_S_ADD_U8C_SAT_SAT_1                                 \
	dr9 = vis_faligndata(dy, dy);                           \
	...
	dx_r_hi = vis_fpadd16(dx_r_hi, dx_r_hi);                \
	dx_r_lo = vis_fpmerge(dzero, vis_read_lo(dx_r));        \
	dx_r_lo = vis_fpadd16(dx_r_lo, dx_r_lo);                \
	dx_i_hi = vis_fmuld8ulx16(fone, vis_read_hi(dx_i));     \
	dx_i_hi = vis_fpadd16(dx_i_hi, dx_i_hi);                \
	dx_i_lo = vis_fmuld8ulx16(fone, vis_read_lo(dx_i));     \
	dx_i_lo = vis_fpadd16(dx_i_lo, dx_i_lo);

#define	MUL_S_ADD_U8C_SAT_SAT_2                                       \
 ...
 */
	...
	if ((uc_r + uc_i) > MLIB_S8_MAX) {
		mlib_f32 fc_r = vis_to_float((uc_r << 17) | (uc_r << 1));
		mlib_f32 fc_i = vis_to_float((uc_i << 17) | (uc_i << 1));

		mlib_s32 gsr_for_pack16 = (7 << 3) | 7, gsr_for_packfix =
			(15 << 3) | 7;

		vis_write_gsr(gsr_for_pack16);
		MUL_S_ADD_U8C_SAT_SAT_1;
		vis_write_gsr(gsr_for_packfix);
		MUL_S_ADD_U8C_SAT_SAT_2;
 ...
 */
	...
	if ((abs(((mlib_s8 *)c)[0]) + abs(((mlib_s8 *)c)[1])) >
		...
		mlib_d64 dover_r = vis_to_double_dup(0x80808080),
			dover_i = vis_to_double_dup(0x81008100);
...
#define	MUL_S_ADD_S16C_SAT_OVER                                  \
	...
	mask_over = vis_fcmplt32(dr5, d_min) |                   \
		vis_fcmpgt32(dr5, d_max16);                      \
	mask_over1 = vis_fcmplt32(dr6, d_min) |                  \
		vis_fcmpgt32(dr6, d_max16);                      \
 ...
 * uc0 <<= 7; uc1 <<= 7;
 */
	fc0 = vis_to_float((uc0 << 17) | (uc0 << 1));
	fc1 = vis_to_float((uc1 << 17) | (uc1 << 1));
...
#define	EVEN_S16C_U8C_MOD                                       \
	dr = vis_fand(dr, const_clear1);                        \
	dr1 = vis_fand(dr1, const_clear1);                      \
	(void *)vis_alignaddr(dpx, 2);                          \
	dr = vis_faligndata(dr, dr);                            \
	dr1 = vis_faligndata(dr1, dr1);                         \
	dr2 = vis_fand(dr2, const_clear1);                      \
	dr3 = vis_fand(dr3, const_clear1);                      \
	dzh = vis_for(dr, dr2);                                 \
	dzl = vis_for(dr1, dr3);
	...
	mlib_s32 n)
{
	...
	mlib_d64 const_clear1 = vis_to_double_dup(0xffff);
