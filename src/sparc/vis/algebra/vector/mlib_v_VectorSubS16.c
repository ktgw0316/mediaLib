...
#pragma ident	"@(#)mlib_v_VectorSubS16.c	9.3	07/11/05 SMI"

/*
 * FUNCTIONS
 *      mlib_VectorSubS16 - subtraction of signed 16-bit format vector
 *                         from a scalar
 *
 * SYNOPSIS
 *      mlib_status mlib_VectorSubS_S16_[Sat|Mod](mlib_s16       *xz,
 *                                                const mlib_s16 *c,
 *                                                mlib_s32       n);
 ...
 *                                                    mlib_s32       n);
 *
 *      mlib_status mlib_VectorSubS_S32_S16_Mod(mlib_s32       *z,
 *                                              const mlib_s16 *x,
 *                                              const mlib_s16 *c,
 *                                              mlib_s32       n);
 *
...
/*
 * FUNCTIONS
 *      mlib_VectorSubS_S16C - subtraction of signed 16-bit format
 *                            complex vector from a scalar
 *
 * SYNOPSIS
 *      mlib_status mlib_VectorSubS_S16C_[Sat|Mod](mlib_s16       *xz,
 *                                                 const mlib_s16 *c,
 *                                                 mlib_s32       n);
 ...
 *                                                      mlib_s32       n);
 *
 *      mlib_status mlib_VectorSubS_S32C_S16C_Mod(mlib_s32       *z,
 *                                                const mlib_s16 *x,
 *                                                const mlib_s16 *c,
 *                                                mlib_s32       n);
 *
...
#define	SUBS16_SAT                                              \
	dz = vis_fpsub16(dc, dx);                               \
	dr0 = vis_fxor(dx, dc);                                 \
	dr1 = vis_fxor(dc, dz);                                 \
	dr2 = vis_fand(dr0, dr1);                               \
...
#define	SUBS_S32_S16_MOD                                        \
	dr1 = vis_fmuld8ulx16(fone, vis_read_hi(dx));           \
	dr2 = vis_fmuld8ulx16(fone, vis_read_lo(dx));           \
	dzh = vis_fpsub32(dc, dr1);                             \
	dzl = vis_fpsub32(dc, dr2)

/* *********************************************************** */

#define	SUBS_S16C_S32C                                          \
	dr1 = vis_fmuld8ulx16(fone, vis_read_hi(dx));           \
	dr2 = vis_fmuld8ulx16(fone, vis_read_lo(dx));           \
	dzh = vis_fpsub32(dc, dr1);                             \
	dzl = vis_fpsub32(dc, dr2)
...
#define	SUBS_S32C_S16C_WITH_ODD                                 \
	dr1 = vis_fmuld8ulx16(fone, vis_read_hi(dx));           \
	dr2 = vis_fmuld8ulx16(fone, vis_read_lo(dx));           \
	dzh = vis_fpsub32(dc_odd, dr1);                         \
	dzl = vis_fpsub32(dc_odd, dr2)

/* *********************************************************** */

#define	SUBS_S32_S16_IN_C                                       \
	{                                                       \
	    for (i = 0; i < n; i++)                             \
		(*pz++) = uc - (*px++);                         \
	    return (MLIB_SUCCESS);                              \
